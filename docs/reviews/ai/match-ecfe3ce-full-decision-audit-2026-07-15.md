...
Remote-Pfad rechnet die sichtbare Rex-Vermeidung und den folgenden
Krash-Break nicht in dasselbe Credit-Budget. Krash deckt die Wall tatsächlich
ab; die Vorabquote unterschlägt aber vier Credits garantierte Rex-Vermeidung.
Der spätere Jack-out D63 ist wegen der danach unterschrittenen Reserve korrekt;
der Fehler entsteht bereits in der zu günstigen Startquote.
...
| D59 Remote 1                 | D60–D63   | Rex-Trace verhindert, dann Jack-out vor bekannter Wall                        | F1 beginnt in D59: Rex-Vermeidung und Krash-Break werden nicht gemeinsam budgetiert; D60–D63 schützen danach korrekt die Credit-Reserve     |
...
## F1 – Vorab-Pfadprüfung teilt sichtbare Kosten auf zwei Budgets

### Historischer Zustand

- D59 / StateVersion 113 / Turn 18 / letzter Klick
- Runner: 8 Credits; Rig enthält Krash und Loony Goon
- Krash ist ein aktuell installierter universeller Breaker und deckt die Wall;
  ein Dwarf liegt im Heap, eine weitere Kopie in der Grip
...
Krash-Break der Data Wall, führt die sichtbare Rex-Gefahr aber in einem
separaten `creditsAfterAvoidingVisibleIceHazards`-Wert. Die dafür ermittelte
Vermeidung wird weder vom gemeinsamen `creditBudget` abgezogen noch zu
`visibleBreakCost` addiert.

Mit sichtbarer Corp-Bid-Kapazität 6 ist die billigste garantierte
Rex-Vermeidung ein Krash-Break für 4 Credits; die anschließende Data Wall kostet
weitere 4. Der sichere Pfad kostet damit 8 und lässt 0 Credits. Im konkreten
Replay bietet die Corp weniger, sodass der Runner den Trace für 3 verhindert;
zusammen mit dem späteren 4-Credit-Wall-Break bliebe dennoch nur 1 Credit und
damit weniger als die aktive Reserve 2. Der RunnerRunPlan jackt deshalb in D63
korrekt aus.

### Bessere legale Folge

Der freie HQ-Run ist bereits historisch legal, raw besser und frei von
sichtbaren ICE-Risiken. Alternativ ist die im reparierten Checkpoint gewählte
Dwarf-Installation ein plausibles Setup. Der bekannte Remote-Run darf mit
vollständig eingerechneten sichtbaren Kosten nicht als finanzierter
Trash-Pfad den Plan absolut binden.

### Generische Maßnahme nach Freigabe

- sichtbare ICE-Breaks, Trace-Vermeidung, Restricted Credits und
  Breaker-Nebenfolgen über genau ein gemeinsames Pfadbudget verbuchen;
- `visibleBreakCost`, `creditsAfterPath` und Hazard-Evidence aus derselben
  Zahlungsprojektion ableiten;
- historischer D59-Checkpoint muss Kosten 8, Restcredits 0 und den Remote-Run
  als verbotene Auswahl ausweisen;
- Gegenprobe: mit 10 Credits bleibt derselbe Pfad erreichbar, kostet weiterhin
  8 und lässt 2 Credits.
...
und ein neues unbekanntes äußeres ICE; der einzige installierte Dwarf kostet 6. Der Score berücksichtigt nur das bekannte ICE und keinen side-safen
...
| D48/D61 Rex                     | Trace 3; bei Erfolg ETR plus 2-Credit-Run-Sperre, kein Tag                        | aktiver und kompilierter Hint modellieren Trace, conditional ETR und Run-Lock; der korrigierte Tag-Consumer verlangt echte `tag_source`-Semantik                         | exakte Bid-Choices funktionieren; F1 entsteht erst bei der getrennten Verbuchung von Hazard- und ICE-Kosten                                                   | korrekt nach FD7671-Remediation                                                                |
...
| Broker über das Spiel           | pro Kopie und Zug genau eine der beiden Actions: 3 Credits laden oder alle nehmen | finite Economy-Pool und action economy korrekt kompiliert                                                                                                                | D156 verpasst einen letzten-Klick-Load; D179 installiert eine zweite Kopie zu früh; der `firstLoad`-Bonus priorisiert D180/D191 die schlechtere Kopie         | Hint/Engine korrekt; Mehr-Kopien- und Gesamtwert-Consumer fehlerhaft                           |
...
- D150 und D154 zeigen dagegen, dass ein richtiger Plan spezifische
  ...
  Warnsignal relevant.
- D59 ist der einzige belegte Planfehler vor F2: ein unvollständiger
  Path-Quote-Budget-Input
  lässt `runner.contest_remote` den um 1126 besseren HQ-Raw-Winner absolut
  blockieren.

## Freigabeliste für ein Folgepaket

### Punkt 1: Sichtbare Pfadkosten müssen ein gemeinsames Budget teilen

- Beschreibung Spielfehler: D59 besitzt mit Krash Wall-Coverage, aber die
  Vorabquote addiert die garantierte Rex-Vermeidung 4 nicht zum folgenden
  Wall-Break 4. Sie meldet Kosten 4/Rest 4 statt Kosten 8/Rest 0; D63 schützt
  später korrekt die Reserve. HQ ist legal und raw um 1126 besser.
- Dafür geplante Anpassungsmaßnahme: gemeinsame Zahlungsprojektion für Hazard-
  und ICE-Kosten; historischer Checkpoint plus finanzierte Gegenprobe.
