...
| AI-COMPLETE-15 | Text-/Regex-/Label-Fallbacks aus produktiver Entscheidung lösen. | `IN_PROGRESS` | `action.label` und Regex werden in produktiven Pfaden verwendet. | Text-/Regex-/Label-Fallbacks sind nur diagnostisch und erzeugen Coverage-Gaps. |
...
Nächstes aktives Ziel: `AI-COMPLETE-14`.
...
- `AI-COMPLETE-12` zweiter Hard-Gate-Vertragsschnitt:
  - `action-goal-fit.test.ts` vereinheitlicht die Gate-Stop-Erwartung für `not_in_legal_actions`, `hidden_info_required` und `cannot_pay`: blockierter Fit, Score `0`, nur Hard-Gate-Evidence, keine positiven Score-Komponenten.
  - `semantic-shadow-decision.test.ts` schützt zusätzlich den Hidden-Info-End-to-End-Fall: ein Kandidat mit `hidden_info_blocked` wird vor Run-Payoff-Scoring abgelehnt, nicht gerankt und mit `hidden_info_required` plus Gate-Evidence erklärt.
...
- `AI-COMPLETE-13` Abschlussaudit:
  - Produktive Practical-Tactic- und Practical-Micro-Pfade erzeugen keine Action-Ersetzung mehr; Evidence meldet `*_actual_override:false`.
  - Produktive Suche nach alten Apply-/Overlay-/Actual-Override-Evidence-Pfaden ist sauber; verbleibende `*_applied`-Strings sind Negativ-Erwartungen in Tests.
  - TacticalPlan-Mapping verwendet `tactical_plan_mapping_outcome:*` und neutrale Semantic-Choice-Evidence statt `tactical_plan_mapping_overridden:true` oder `tactical_plan_override_reason:*`.
  - Diagnose-/Mining-Pfade werten jetzt `tactical_plan_mapping_outcome:semantic_choice_selected` aus.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `corepack pnpm --filter @netgrid/ai test` grün, 155 Dateien, 1680 Tests.
...
- `AI-COMPLETE-14` Abschlussaudit und achter Rückbau-Schnitt:
  - `packages/ai/src/simulation/corp-tag-punish-action-context.ts` entfernt die verbliebenen Tag-/Punish-Fallback-Sets für einzelne DefinitionIds; produktive Tag-Source- und Punish-Payoff-Klassifikation läuft über Ontologie, scored-agenda Adapter oder generische Rollen.
  - `packages/ai/src/simulation/tag-punish-card-sets.ts` enthält keine Corp-Tag-/Punish-DefinitionId-Sets mehr; übrig bleiben nur Runner-Survival-Countercontext-Sets außerhalb des auditierten Corp-Punish-Pfads.
  - `packages/ai/src/runtime/practical-micro-candidates-context.ts` und `practical-tactic-overlay.ts` entfernen `Closed Accounts`-/`Scorched`-/`Power Grid Overload`-Namen aus den Punish-Erkennungen und nutzen nur generische Punish-/Tag-Rollenbegriffe.
  - `packages/ai/src/simulation/corp-tag-punish-visible-payoff.ts` erkennt sichtbaren Corp-Meat-Damage-Payoff und Runner-Damage-/Trace-Defense-Ressourcen über Hint-/Rollen-/Ontology-Semantik statt über konkrete DefinitionIds wie Full Body Conversion, Dermatech Bodyplating, Diplomatic Immunity oder Submarine Uplink.
  - `packages/ai/src/runtime/runner-semantic-card-ids.ts` entfernt ungenutzte Full-Body-/Dermatech-ID-Exporte.
  - `packages/ai/src/index.test.ts` schützt die neuen generischen Evidence-Namen für Damage-/Meat-Prevention und setzt `resetTacticalPlanMemory()` im globalen `beforeEach`, damit Full-Suite-Order keine persistenten Plan-Memory-Artefakte erzeugt.
  - `packages/ai/src/tactical-plans.test.ts` nutzt ein neutrales Bank-Cashout-Label statt Broker-Text für die Bank-Capability-Evidence.
  - Audit: `rg -n "broker|Broker|closed accounts|city surveillance|chance observation|project babylon|project venice|project zurich" packages\ai\src\runtime packages\ai\src\plans packages\ai\src\actions packages\ai\src\decision packages\ai\src\simulation --glob "*.ts" --glob "!**/*.test.ts" --glob "!**/legacy/**"` ohne Treffer.
  - Audit: `rg -n "onr_v1_313_city-surveillance|onr_v1_284_chance-observation|onr_v1_285_closed-accounts|onr_v1_154_broker|onr_v1_214_project-babylon|onr_proteus_008_project-zurich|onr_proteus_007_project-venice|onr_v1_127_full-body-conversion|onr_v1_125_dermatech-bodyplating|onr_v1_160_diplomatic-immunity|onr_v1_182_submarine-uplink|closed accounts|city surveillance|chance observation|project babylon|project venice|project zurich|runner_resource_diplomatic_immunity|runner_full_body_conversion|runner_dermatech" packages\ai\src\runtime packages\ai\src\plans packages\ai\src\actions packages\ai\src\decision packages\ai\src\simulation --glob "*.ts" --glob "!**/*.test.ts" --glob "!**/legacy/**"` ohne Treffer.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/corp-tag-source-payoff-context.test.ts src/runtime/ai-feature-server.test.ts src/runtime/runner-bank-investment-context.test.ts src/runtime/semantic-runtime-corp-advancement-counter.test.ts src/runtime/semantic-runtime-plan-memory-exclusion.test.ts src/runtime/runner-visible-breaker-coverage.test.ts` grün, 6 Dateien, 11 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/index.test.ts src/semantic-ai-runtime-cutover.test.ts -t "immediate trace tag source|visible-payoff tag-source|persistent tag-asset|separates Broker pool loading|exact overadvance threshold|Broker build|Broker cashout|does not cash out Broker immediately|devalues Broker install|Diplomatic Immunity|Schlaghund tagged meat damage|endgame tag-punish resource trash"` grün, 2 Dateien, 20 relevante Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/index.test.ts -t "defers Loan from Chiba|penalizes Loan from Chiba|allows Loan from Chiba when it funds a concrete remote contest"` grün, 3 relevante Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/tactical-plans.test.ts -t "uses bank capability evidence for runner cashout plans"` grün, 1 relevanter Test.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.
...
- `AI-COMPLETE-15` siebenundvierzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/runtime/semantic-runtime-corp-effective-defense.ts` ersetzt `variableRezKind.includes("x"|"trace"|"paid_end_the_run")` durch eine explizite Allowlist der Variable-Rez-Kinds, die einen Mindestwert brauchen.
  - `semantic-runtime-corp-effective-defense.test.ts` schützt, dass x-artiger Freitext wie `xylophone_noise` keinen `minimumUsefulX`-Vertrag auslöst.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/semantic-runtime-corp-effective-defense.test.ts src/runtime/semantic-runtime-corp-score.test.ts` grün, 2 Dateien, 17 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.
...
- `AI-COMPLETE-15` fünfzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/decision/tactical-goal-utility.ts` ersetzt freie GoalId-Substring-Kategorisierung durch strukturierte Goal-Family-Auswertung und Segment-/Compound-Boundary-Matches auf GoalIds.
  - Goal-Familien wie `corp_scoreline`, `tag_punish` und `damage_pressure` werden explizit als strukturierte Quelle genutzt; IDs wie `corp.score_agenda` bleiben Treffer, während `outscoreboard_noise`, `microeconomy_noise` und `tagalong_punishment_noise` nicht mehr durch Substrings falsch kategorisiert werden.
  - `tactical-goal-utility.test.ts` schützt strukturierte Familien, echte GoalId-Terms und Substring-Negativfälle.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/decision/tactical-goal-utility.test.ts src/decision/action-goal-fit.test.ts src/decision/semantic-shadow-decision.test.ts` grün, 3 Dateien, 34 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.

- `AI-COMPLETE-15` einundfünfzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/runtime/discard-keep-score.ts` nutzt für interne Economy-, Score-, Remote-, Breaker- und Tempo-Rollenprüfungen den gehärteten `rolesMatch`-Helper statt freier Rollen-Substring-Prüfungen.
  - Corp-Discard-Keep-Score wertet strukturierte Rollen wie `economy_asset` weiter, ignoriert aber substringartiges Rollenrauschen wie `microeconomy` und `remotecontrol_noise`.
  - `discard-keep-score.test.ts` schützt strukturierte Rollentreffer und Substring-Negativfälle.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/discard-keep-score.test.ts src/runtime/role-match.test.ts src/runtime/discard-plan.test.ts` grün, 3 Dateien, 7 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.
...
- `AI-COMPLETE-15` vierundfünfzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/runtime/runner-junkyard-bbs-recovery-target.ts` nutzt für Junkyard-BBS-Recovery-Zielrollen wie `breaker_`, `memory`, `memory_support`, `setup` und `build_rig` den gehärteten `rolesMatch`-Helper statt freier Rollen-Substring-Prüfungen.
  - Strukturierte Recovery-Zielrollen bleiben wertend, substringartiges Rauschen wie `setupsomething_noise` und `memoryless_noise` löst keine Target-Klasse mehr aus.
  - `runner-junkyard-bbs-recovery-target.test.ts` schützt strukturierte Zielrollen und Substring-Negativfälle.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/runner-junkyard-bbs-recovery-target.test.ts src/runtime/role-match.test.ts src/runtime/runner-role-classification.test.ts` grün, 3 Dateien, 7 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/index.test.ts --testNamePattern "Junkyard BBS|junkyard"` grün, 1 Datei, 6 Tests, 528 skipped.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.

- `AI-COMPLETE-15` fünfundfünfzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/runtime/runner-loan-spend-candidate.ts` nutzt für Loan-Spend-Kandidatenrollen wie `breaker_`, `draw` und `search` den gehärteten `rolesMatch`-Helper statt freier Rollen-Substring-Prüfungen.
  - Strukturierte Rollen wie `breaker_fracter`, `card_draw` und `stack_search` bleiben Treffer; substringartiges Rauschen wie `withdrawal_noise`, `searchlight_noise` und `breakerish_noise` wird nicht als Spend-Kategorie gewertet.
  - `runner-loan-spend-candidate.test.ts` schützt strukturierte Treffer und Substring-Negativfälle.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/runner-loan-spend-candidate.test.ts src/runtime/role-match.test.ts src/runtime/runner-role-classification.test.ts src/runtime/runner-loan-context.test.ts` grün, 3 tatsächlich vorhandene Dateien, 7 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.
...
- `AI-COMPLETE-15` siebenundfünfzigster Label-Fallback-Rückbau-Schnitt:
  - `packages/ai/src/runtime/semantic-runtime-corp-score.ts` nutzt für Corp-Install-Protection- und Economy-Rollen den gehärteten `rolesMatch`-Helper statt freier Rollen-Substring-Prüfungen.
  - Strukturierte Rollen wie `ice_tax`, `remote_protect` und `economy_asset` bleiben wertend; substringartiges Rauschen wie `nice_noise`, `protectorate_noise` und `microeconomy_noise` löst keine Install-Protection-/Economy-Komponente mehr aus.
  - `semantic-runtime-corp-score.test.ts` schützt strukturierte Install-Rollentreffer und Substring-Negativfälle.
  - Status bleibt `IN_PROGRESS`, weil weitere generische Text-/Regex-Heuristiken auf strukturierte Ownership geprüft und gegebenenfalls in Folgepaketen abgebaut werden müssen.
  - Verifikation: `corepack pnpm --filter @netgrid/ai exec vitest run src/runtime/semantic-runtime-corp-score.test.ts src/runtime/semantic-runtime-corp-effective-defense.test.ts src/runtime/role-match.test.ts` grün, 3 Dateien, 20 Tests.
  - Verifikation: `corepack pnpm --filter @netgrid/ai typecheck` grün.
  - Verifikation: `git diff --check` grün.
