...
- Lokaler `main` wurde konfliktfrei in `codex/deck-strategy-runtime-hardening` gemergt; spätere zusätzliche Main-Refactors wurden ebenfalls integriert, zuletzt mit `3a1fa580`.
...
- `corepack pnpm --filter @netgrid/ai typecheck`: grün nach letztem Main-Sync.
- `corepack pnpm --filter @netgrid/ai test`: grün nach vorletztem Main-Sync, 148 Testdateien und 1643 Tests; vollständiges Abschlussgate folgt auf `main`.
- `git diff --check -- packages/ai docs KI-Wissen-NETGRID`: grün.
