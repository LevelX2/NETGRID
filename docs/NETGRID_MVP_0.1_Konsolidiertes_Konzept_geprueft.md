# NETGRID-Webapplikation – Konsolidiertes Entwicklungskonzept
...
Der MVP beweist nicht, dass alle NETGRID-Regeln und alle Karten funktionieren. Er beweist, dass die technische Basis tragfähig ist:
...
- Für spätere Kartendatenquellen wird ein versionierter lokaler Snapshot bevorzugt; NETGRIDDB/API oder offizielle JSON-Daten können später als Quelle dienen, aber nicht als direkte Regelinterpretation.
...
- NETGRID hat hohe Regel- und Karteneffektkomplexität.
...
- Externe Kartendaten werden gecacht und nicht unnötig häufig abgerufen; bei späterer Nutzung von NETGRIDDB-/NSG-Daten sind HTTP-Caching und die jeweilige Nutzungsfreigabe zu beachten.
...
Tests sind bei diesem Projekt nicht optional. NETGRID ist wegen Timing, verdeckter Information, Karteneffekten und verschachtelten Runs zu komplex, um die Engine nur manuell zu prüfen.
...
Private NETGRID-Webapp = deterministische Regel-Engine + serverseitiger Spielzustand + gefilterte Spieleransichten + Human/KI-Controller + EventLog + StateHash + umfassende Tests + schrittweise Kartenerweiterung.

MVP 0.1 ist erfolgreich, wenn eine kleine, kontrollierte NETGRID-Grundpartie stabil, reproduzierbar, hidden-info-sicher und regelgeführt funktioniert. Alles Weitere baut darauf auf.
