...
Nachtest nach Nutzer-Rueckmeldung: Die erste Regression lief zunaechst nicht auf der V0.97+/V0.99-Run-Baseline und erzeugte deshalb kein `run.jack_out_window` vor dem naechsten ICE. Der Test nutzt jetzt `MVP_0_99_BASELINE` und deckt beide kritischen Movement-Fenster ab: vor dem sichtbaren, bezahlbaren inneren `Wall of Static` und nach dem gebrochenen letzten ICE vor dem Zugriff. `safe_probe_run` priorisiert in einem Movement-Fenster mit sichtbarem, bezahlbarem gerezztem ETR-ICE nun `continue_run` statt `jack_out`.
