...
checks:
  - corepack pnpm --filter @netgrid/web test -- action-board-ui.test.ts -t "card choice"
...
- [x] `Technician Lover` zeigt beim Blick auf die oberste R&D-Karte ein reines Anzeige-/Bestätigungsfenster ohne Plus-/Auswahlbutton.
- [x] `Fertig` schließt die Anzeige direkt, ohne vorher eine Karte markieren zu müssen.
- [x] Der angezeigte Kontext nennt verständlich Quelle und Zone, mindestens für den Fokusfall `Technician Lover` / oberste R&D-Karte.
- [x] Mehrkarten-Private-Look-Fälle wie `R&D-Protocol Files` bleiben lesbar und verwenden ebenfalls keine Auswahlmarkierung, solange keine Auswahlentscheidung existiert.
- [x] Echte Such-, Reorder-, Trash-, Access- und Auswahlfenster behalten ihre Auswahlinteraktion.
- [x] Hidden-Info-Grenzen bleiben unverändert: Nur der berechtigte Runner sieht die privaten Karten.
- [x] Fokussierte Web-Tests oder ein dokumentierter Browser-Smoke decken Ein-Karten- und Mehrkarten-Private-Look ab.
...
Umgesetzt: `p3_33.private_look:*`-Choices werden im Webclient als Read-only-Kartenanzeige erkannt. Die CardChoicePanel-Ansicht blendet Auswahl-/Plus-/Nur-ansehen-Buttons und den Auswählbar-Filter für diese Anzeige aus, zeigt einen kontextbezogenen Titel wie `Technician Lover: oberste R&D-Karte` und schließt über `Fertig` direkt mit der vorhandenen `done`-Choice. Echte Such-, Reorder-, Trash-, Access- und Auswahlfenster bleiben im Auswahlmodus. Fokussierte Helper-Regressionen decken Ein- und Mehrkarten-Private-Look ab.
