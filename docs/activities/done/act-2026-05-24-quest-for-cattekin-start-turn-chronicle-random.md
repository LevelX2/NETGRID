...
checks:
  - corepack pnpm --filter @netgrid/engine test -- index.test.ts -t "Quest for Cattekin"
  - corepack pnpm --filter @netgrid/web test -- chronicle.test.ts -t "Quest for Cattekin"
...
- [x] Bei jedem Runner-Zugbeginn mit installierter `Quest for Cattekin` entsteht ein deterministischer Würfelwurf mit passendem `RandomDrawRecord`.
- [x] Die öffentliche Chronik zeigt für `Quest for Cattekin` den gewürfelten Wert und das konkrete Ergebnis, auch bei `3`, `4` oder `5`.
- [x] Bei Wurf `6` wird die Resource getrasht, der Runner erhält die Extra-Aktion im aktuellen oder ab dem regelkonform erwarteten Turn, und der persistente Extra-Action-Zustand wirkt in späteren Runner-Zügen weiter.
- [x] Bei Wurf `1`/`2` wird der passende nicht verhinderbare Damage ausgelöst und in der Chronik verständlich dargestellt.
- [x] Replays bleiben deterministisch; StateHash und RandomDrawRecords bleiben stabil.
- [x] PublicEvent-/Reconnect-/Undo-/Replay-Payloads leaken keine verdeckten Kartendaten.
- [x] Fokussierte Engine- und, falls die Chronik im Webclient betroffen ist, Web-/UI-Tests sind ergänzt oder ein bewusst begründeter Ersatzcheck ist dokumentiert.
...
Umgesetzt: Der bestehende Start-of-Turn-Engine-Pfad würfelt weiter deterministisch und erhält einen öffentlichen, side-sicheren `questForCattekinOutcome` im `ResolvedEffect`. Die Regression prüft No-op-Würfe, Core Damage, Net Damage, den 6er-Trash mit dauerhafter Extra-Aktion sowie den späteren Runner-Zug ohne erneuten Quest-Wurf. Die Web-Chronik rendert `Quest for Cattekin` jetzt spezifisch mit Wurfwert und Ergebnis für `1`, `2`, `3`/`4`/`5` und `6`, inklusive nicht verhinderbarem Damage und dauerhaftem Aktionsgewinn.
