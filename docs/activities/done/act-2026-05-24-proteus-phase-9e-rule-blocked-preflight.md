...
Die lokale Primärquelle `docs/source/Proteusspoiler.txt` führt `Ice and Data Special Report` mit Text `Expose up to five cards installed in or on a single data fort.` und Kostenzeile `Cost: 3 (0)`. Der damalige Proteus-Import hat diese Kostenangabe bewusst nicht als eindeutigen numerischen Play-Cost übernommen; der zu diesem Zeitpunkt maßgebliche Kartenstand führte `numeric.cost: null` für `onr_proteus_111_ice-and-data-special-report`.
