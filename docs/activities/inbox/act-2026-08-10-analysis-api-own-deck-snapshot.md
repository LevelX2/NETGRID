...
- [ ] Ein neu gespeichertes Match liefert für die analysierte eigene Seite
      Identity, Definition-Counts, Gesamtzahl, Kartenpool-/Formatversion und
      Deck-Hash aus einer historisch gebundenen Quelle.
- [ ] Die Darstellung ist reihenfolgenneutral und enthält keine
      Karteninstanz-IDs, Shuffle-Ergebnisse oder Stackpositionen.
- [ ] Bundle und Decision-Detail weisen denselben Snapshot beziehungsweise
      dieselbe Signatur und eindeutige Provenance aus.
- [ ] Aus Snapshot und actor-sicheren eigenen Zonen kann nachvollzogen
      werden, welche Definitionen beziehungsweise Rollen noch im Stack
      möglich sind, ohne dessen Reihenfolge zu verraten.
- [ ] Die aktuelle Deckdatei wird nach einer nachträglichen Änderung nicht
      als historische Wahrheit ausgegeben.
- [ ] Bei fehlender historischer Bindung meldet
      `diagnostics.unavailableSections` einen strukturierten Grund wie
      `ownDeckSnapshot`; es gibt keinen stillen Fallback.
- [ ] Gegnerische Deckdefinitionen, Handkarten, Karteninstanzen,
      FullGameState, Sessions und Tokens bleiben redigiert.
- [ ] Bestehende Filter und Payloadgrenzen des Analyse-Bundles bleiben
      wirksam.
- [ ] Maintenance-API-Tests sichern Berechtigung, Side-Bindung, Redaction,
      Hash-/Versionsbindung und konsistente Bundle-/Detailwerte.
- [ ] Replay, StateHash und Matchdeterminismus bleiben unverändert; der
      Snapshot ist Analyse-/Provenance-Evidence, keine Regelautorität.
- [ ] Fokussierte Server-/Storage-/API-Tests, erforderliche Typechecks,
...
Noch offen. Die aktuelle API liefert nur Deckmetadaten und Stackanzahl, nicht
die historische eigene Deckzusammensetzung.
