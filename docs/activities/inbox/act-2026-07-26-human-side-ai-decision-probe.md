...
- [ ] Der Vorschlag kann nur von der authentifizierten menschlichen Seite und
      nur in ihrem aktuellen Entscheidungsfenster angefordert werden.
- [ ] Der KI-Input entspricht der eigenen `PlayerView`, den eigenen
      `LegalActions`, erlaubten `PublicEvents` und dem eigenen Decksnapshot.
- [ ] Die angezeigte direkte Aktion existiert unverändert in den aktuellen
      `LegalActions`; unbekannte oder inzwischen stale Actions werden nicht
      angezeigt.
- [ ] Zwei Aufrufe auf demselben Zustand und mit demselben Advisor-Profil
      liefern dasselbe Ergebnis, sofern die KI keinen ausdrücklich
      Engine-randomisierten Auswahlbefehl anfordert.
- [ ] Engine-randomisierte Auswahlpfade ziehen in der Vorschau keinen Zufall
      ...
      KI-Absicht.
- [ ] Der Aufruf verändert weder Match-/State-Version, Eventlog, StateHash,
      Zufallsnachweise noch produktives KI-Memory.
- [ ] Der Vorschlag verschwindet nach jeder Zustandsänderung und wird nie als
      Regelautorität oder garantiert beste Aktion bezeichnet.
- [ ] Der Meldeexport ist reproduzierbar, kompakt und enthält keine verbotenen
      Hidden-Info-, Token-, Decklisten-, Rohinput- oder FullState-Daten.
- [ ] Runner- und Korp-Fälle, Choice-Fenster, stale Versionen, unzulässige
      Fremdseitenaufrufe, Random-Auswahl und Hidden-Info-Sentinels sind
      getestet.
- [ ] Relevante AI-, Server- und Web-Checks sowie `git diff --check` sind grün.
