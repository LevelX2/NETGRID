...
- Queue- und Register-Dedupe geprüft gegen `docs/reviews/originalset-spotchecks/register.md`, `data/reports/originalset-card-spotcheck-register.json` sowie alle Markdown-Berichte unter `docs/derived/originalset-spotcheck-jobs/{inbox,in_progress,done,blocked}/`.
...
- `False Echo`: Successful-Run-LegalAction mit source-/side-/timinggebundener Revalidation; deterministische Rez-Schleife von äußerster zu innerster ICE, Kostenprüfung und öffentliche Payload ohne Hidden-Zone-Dumps.
- `Netspace Inverter`: Successful-Run-LegalAction zum stabilen Reversen der ICE-Liste des angegriffenen Forts; Payload beschränkt auf Serverlabel, ICE-Anzahl und Reversal-Flag.
