...
startedAt: 2026-05-15T08:46:33Z
completedAt: 2026-05-15T11:15:57+02:00
...
- `Rabbit`: installierter Runner-Rabbit reduziert Corp-Bid-Limit bei ICE-Traces um 1; Trace-State, PublicPayload und LegalAction-Revalidation tragen `corpBidMax` und `rabbitTraceLimitReduction`.
- `Artemis 2020`: Runtime-Definition auf +2 MU, 2 recurring Icebreaker-Credits und Deck-Einzigartigkeit korrigiert; Runner-Hardware-Install ersetzt ältere Deck-Hardware.
- `City Surveillance`: rezzed Asset belastet Runner-Draws deterministisch mit 1 Credit pro Quelle oder gibt bei fehlenden Credits Tags; PublicPayload nennt nur Counts und Endwerte.
- `South African Mining Corp`: rezzed Root-Asset bietet `[A], trash: Gain 8 credits.` als source-bound LegalAction mit Selftrash und PublicPayload.
- `Jerusalem City Grid`: rezzed Region gibt Wall-ICE im eigenen Fort +1 Stärke und 9 Credits Rez-Kostenreduktion, ohne fremde Server oder unrezzed Roots zu leaken.
- Bestehende Pfade für `Blink`, `Grubb`, `Incubator`, `Forgotten Backup Chip`, `Stumble through Wilderspace`, `Corporate Downsizing`, `Strike Force Kali`, `Superior Net Barriers`, `TKO 2.0` und `Zombie` wurden in der Engine-Abdeckung mitgeführt und blieben grün.
