...
Proteus-Release-Slicing ist planning-only geschnitten: `docs/derived/PROTEUS_RELEASE_SLICING_PLAN.md` priorisiert nach Import-/Coverage-Freeze zuerst sichtbare Baseline-Karten, Bad-Publicity-7+-Harness, variable ICE-Foundation und Hidden-Runner-Resource-Foundation, danach einfache Runner-/Agenda-/Access-Slices, Cybernetics-/Deck-Hardware, Virus-/Antibody-/Purge und Random-/Longtail-Familien. Erste Folgeactivities sind `act-2026-05-17-proteus-visible-baseline-card-slice`, `act-2026-05-17-proteus-bad-publicity-engine-harness`, `act-2026-05-17-proteus-variable-ice-harness-slice` und `act-2026-05-17-proteus-hidden-resource-foundation-slice`. Human-Spielbarkeit, Decklegalität und AI-Support bleiben getrennte Gates.

Proteus-Mechanikabdeckung ist als Planungsmatrix erfasst: `data/rules/proteus-mechanics-coverage-2026-05-17.json` und `docs/derived/PROTEUS_MECHANICS_COVERAGE_ANALYSIS.md` klassifizieren 154/154 importierte Proteus-Karten in `wahrscheinlich abgedeckt` (17), `kleiner Resolver nötig` (56), `Mechanikvertiefung nötig` (80) und `blockiert/Regelklärung nötig` (1). Die Analyse benennt 31 Mechanikcluster, darunter variable Proteus-ICE, verdeckte Runner-Resources, Bad-Publicity-Loss-Gate, Virus-/Antibody-Counter und Cybernetics-/Deck-Hardware. Fünf Folgeactivities sind im Inbox-Board angelegt: variable ICE-Verträge, Hidden Resources, Bad-Publicity-Loss-Gate, Virus-/Antibody-Verträge und Cybernetics-/Deck-Hardware. Keine Runtime-, Decklegalitäts-, AI- oder Kartenpromotion.

Proteus-Spoiler-Ingestion ist als blockierte Planungsdatenbasis abgeschlossen: `packages/catalog/src/proteus-spoiler.ts` parst `docs/source/Proteusspoiler.txt` in 154 display-only Catalog-Karten mit stabilen `onr_proteus_*`-IDs, Set `onr-proteus`, Rarität, Seite, Typ, Subtypen und Basiswerten. Die versionierten Artefakte liegen unter `data/card-import/proteus-card-basis-2026-05-17.json`, `data/card-import/source-registry-proteus-2026-05-17.json`, `data/reports/proteus-spoiler-import-report-2026-05-17.json` und `docs/derived/PROTEUS_SPOILER_IMPORT_REPORT.md`. Alle Proteus-Karten bleiben `blocked` und sind nicht `implemented`, `engine_supported`, `playable`, `human_playable`, `deck_legal`, `format_legal` oder `ai_supported`; der Runtime-Snapshot nimmt sie nicht auf. Der Report bestätigt 77 Korp-/77 Runner-Karten und Rarität 66 Common, 44 Uncommon, 44 Rare, 0 Vital; offene Reviewpunkte sind variable Stärke bei `Digiconda`/`Homing Missile`, `Cost 3 (0)` bei `Ice and Data Special Report` sowie die Quellenkopf-Abweichung bei Prep-/Hardware-Zählung. Verifikation: `corepack pnpm --filter @netgrid/catalog test`, `corepack pnpm --filter @netgrid/catalog typecheck`.
...
V1.9.22 Newsgroup-Filter-Preflight vom 2026-05-14 00:32 CEST: `docs/derived/V1_9_22_NEWSGROUP_FILTER_SLICE_PREFLIGHT.md` dokumentiert den kleinsten verbleibenden Runner-Programm-Kandidaten. Nach dem Install-only-Schnitt ist die aktivierbare Credit-Gain-Faehigkeit fachlich durch Nutzerklaerung bestaetigt: `[A]: Gain 2 Credits`, keine Ziele, Choices oder Hidden-Info. Runtime-Code bleibt bis zum technischen LegalAction-/applyAction-, PublicPayload- und Replay-/StateHash-Vertrag gesperrt.

V1.9.22 Remaining-Corp-ICE-Preflight vom 2026-05-14 00:35 CEST: `docs/derived/V1_9_22_REMAINING_CORP_ICE_PREFLIGHT.md` trennt die vier offenen Corp-ICE-Pfade. `Tutor` und `Virizz` sind die kleinsten naechsten ICE-Kandidaten, bleiben aber fuer Code gesperrt, bis run-weite Zusatzsubroutine bzw. Break-Kostenmodifikation als LegalAction-/applyAction-Vertrag bestaetigt sind.
...
V1.9.22 Security-Purge-Preflight vom 2026-05-13 23:59 CEST: `docs/derived/V1_9_22_SECURITY_PURGE_SLICE_PREFLIGHT.md` dokumentiert den nächsten Agenda-Kandidaten. Code bleibt bewusst gesperrt, bis Optionalität von "any ice", Serverzielwahl und kostenlose Install-/Rez-Reihenfolge lokal bestätigt sind.

V1.9.22 Corp-Longtail-Rest-Preflight vom 2026-05-13 23:59 CEST: `docs/derived/V1_9_22_REMAINING_CORP_LONGTAIL_PREFLIGHT.md` fasst die sechs verbleibenden Corp-Longtailkarten und die offenen Implementierungsvertraege zusammen. Der bestehende No-Playable-Runtime-Guard bleibt fuer diese sechs Karten korrekt.

V1.9.22 Runner-Programm-Preflight vom 2026-05-14 00:27 CEST, durch den 08:50-Newsgroup-Schnitt teilweise ueberholt: `docs/derived/V1_9_22_RUNNER_PROGRAM_IMPLEMENTATION_PREFLIGHT.md` markierte nach dem Shield-Schnitt `Newsgroup Filter` als kleinsten verbleibenden Programm-Kandidaten. Installation und `[A]: Gain 2 Credits` sind inzwischen umgesetzt. Nach den install-only-Schnitten bleiben fuenf Programmkarten durch No-Playable-/No-LegalAction-Guards geschuetzt.
