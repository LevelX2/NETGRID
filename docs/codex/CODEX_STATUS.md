...
V1.9.22 Remaining-Runner-Program-Preflight vom 2026-05-14 01:05 CEST: `docs/derived/V1_9_22_REMAINING_RUNNER_PROGRAM_PREFLIGHT.md` trennt die fuenf verbleibenden Runner-Programm-Zielkarten. `Flak`, `Hammer`, `Japanese Water Torture` und `Reflector` bleiben fuer Code gesperrt, bis Breaker-Subroutine-Taxonomie, Pump-/Break-LegalActions und applyAction-Revalidation feststehen. `Zetatech Software Installer` bleibt wegen Installkosten-Bestaetigung, restricted Programinstallations-Credits und Overlay-Vertrag blockiert.
...
V1.9.22 Runner-Event-Runtime-Resolver-WIP vom 2026-05-13 21:47 CEST: `misc.for-sale` ist als weiterer enger Runner-Event-Resolver umgesetzt, ohne Catalog-, AI- oder Release-Promotion. Die Karte nutzt einen privaten Installed-Trash-Choice fuer eigene installierte Runner-Karten und gewinnt 1 Credit pro getrashter Karte. Der erste Engine-Check war rot, weil der Test auf eine nicht exportierte interne Hilfsfunktion zugriff; nach Testkorrektur sind `engine` 287, `catalog` 44 und `typecheck` gruen. V1.9.22 bleibt `blocked_open`, weil weitere Resolver, AI-Promotion-Artefakte, Webclient-Version und Final Review offen sind.

V1.9.22 Runner-Event-Runtime-Resolver-WIP vom 2026-05-13 21:40 CEST: `Organ Donor` ist als weiterer enger Runner-Event-Resolver umgesetzt, ohne Catalog-, AI- oder Release-Promotion. Die Karte nutzt einen privaten Grip-Trash-Choice fuer bis zu fuenf Karten und gewinnt 1 Credit pro getrashter Karte. Der erste Engine-Check war rot, weil die neue PublicPayload-Zaehlung fuer den Resolve-Choice-Event noch fehlte; nach PublicContext-Erweiterung fuer `trashedCount`, `gainedCredits` und `runnerCreditsAfter` sind `engine` 286, `catalog` 44, `ai` 86, `server` 72, `web` 79, `typecheck`, `test`, `lint` und `build` gruen. Build nur mit bekannter nicht-blockierender Turbopack-NFT-Warnung. V1.9.22 bleibt `blocked_open`, weil weitere Resolver, AI-Promotion-Artefakte, Webclient-Version und Final Review offen sind.
