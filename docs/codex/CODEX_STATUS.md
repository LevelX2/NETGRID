...
V1.9.22 Blocker-/State-Abgleich vom 2026-05-14 01:10 CEST: Nach Pruefung von Automation-State, Completion-Gate-Status, lokaler Kartenfaktenbasis, lokaler Resolver-Arbeitsgrundlage und den vier aktuellen Preflights bleibt V1.9.22 `implementing` und `blocked_open`. Es gibt lokale Fakten fuer die verbleibenden Zielkarten, aber keinen fachlich sicheren Runtime-Code-Schnitt ohne neuen engen LegalAction-/applyAction-Vertrag. Gesperrt bleiben `Security Purge`, `Data Fort Reclamation`, `Haunting Inquisition`, `Tutor`, `Viral 15`, `Virizz`, `Flak`, `Hammer`, `Japanese Water Torture`, `Reflector`, `Zetatech Software Installer` und die `Newsgroup Filter`-Ability. Removal Conditions stehen in `docs/derived/V1_9_22_SECURITY_PURGE_SLICE_PREFLIGHT.md`, `docs/derived/V1_9_22_REMAINING_CORP_LONGTAIL_PREFLIGHT.md`, `docs/derived/V1_9_22_REMAINING_CORP_ICE_PREFLIGHT.md` und `docs/derived/V1_9_22_REMAINING_RUNNER_PROGRAM_PREFLIGHT.md`. Keine Runtime-, Catalog-, AI- oder Release-Promotion wurde vorgenommen.
...
V1.9.22 Runner-Programm-Install-only-WIP vom 2026-05-14 00:45 CEST: `Newsgroup Filter` ist als enger install-only Runner-Programm-Schnitt umgesetzt, ohne Catalog-, AI- oder Release-Promotion. Die Karte hat jetzt eine Runtime-Definition mit Installkosten 5, MU 2 und finalem display-only Text; die Credit-Gain-Faehigkeit bleibt ohne LegalAction, bis Aktivierungskosten, Timingfenster und Turn-/Use-Limit lokal bestaetigt sind. Der Smoke deckt Install-LegalAction, Wrong-Side-/Stale-Revalidation, Memory-Kosten, Ability-Gate, PublicPayload/PlayerViews und Replay/StateHash ab. Verifikation: `engine` 297, `catalog` 44, `ai` 86, `server` 72, `web` 79, `typecheck`, `test`, `lint` und `build` gruen; Build nur mit bekannter nicht-blockierender Turbopack-NFT-Warnung. V1.9.22 bleibt `blocked_open`, weil weitere Runner-Programme, verbleibende Corp-Longtailkarten, finale AI-Promotion-Artefakte, Webclient-Version und Final Review offen sind.
...
V1.9.22 Newsgroup-Filter-Preflight vom 2026-05-14 00:32 CEST: `docs/derived/V1_9_22_NEWSGROUP_FILTER_SLICE_PREFLIGHT.md` dokumentiert den kleinsten verbleibenden Runner-Programm-Kandidaten. Nach dem Install-only-Schnitt bleibt nur die aktivierbare Credit-Gain-Faehigkeit gesperrt, bis Aktivierungskosten, Timingfenster und Turn-/Use-Limit lokal bestaetigt sind; eine automatische `[A]: Gain 1`-Auslegung wird nicht erfunden.
...
V1.9.22 Runner-Programm-Preflight vom 2026-05-14 00:27 CEST: `docs/derived/V1_9_22_RUNNER_PROGRAM_IMPLEMENTATION_PREFLIGHT.md` markiert nach dem Shield-Schnitt `Newsgroup Filter` als kleinsten verbleibenden Programm-Kandidaten; der Installationsvertrag ist inzwischen umgesetzt, die Ability bleibt offen. Nach den install-only-Schnitten bleiben fuenf Programmkarten durch No-Playable-/No-LegalAction-Guards geschuetzt.
