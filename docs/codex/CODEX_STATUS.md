...
V1.9.22 Runner-Event-Runtime-Resolver-WIP vom 2026-05-13 21:40 CEST: `Organ Donor` ist als weiterer enger Runner-Event-Resolver umgesetzt, ohne Catalog-, AI- oder Release-Promotion. Die Karte nutzt einen privaten Grip-Trash-Choice fuer bis zu fuenf Karten und gewinnt 1 Credit pro getrashter Karte. Der erste Engine-Check war rot, weil die neue PublicPayload-Zaehlung fuer den Resolve-Choice-Event noch fehlte; nach PublicContext-Erweiterung fuer `trashedCount`, `gainedCredits` und `runnerCreditsAfter` sind `engine` 286, `catalog` 44 und `typecheck` gruen. V1.9.22 bleibt `blocked_open`, weil weitere Resolver, AI-Promotion-Artefakte, Webclient-Version und Final Review offen sind.
