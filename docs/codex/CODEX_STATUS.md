...
V1.9.22-Hardware-LegalAction-WIP vom 2026-05-13 17:38 CEST: Die Runner-Hardware-Abdeckung des Per-card-Longtail-Slices wurde von Arasaka Portable Prototype auf alle neun V1.9.22-Hardware-Zielkarten verbreitert. Arasaka Portable Prototype, Artemis 2020, Bodyweight Data Creche, Corolla Speed Chip, Microtech Backup Drive, Pandora's Deck, Parraline 5750, PK-6089a und ZZ22 Speed Chip haben jetzt Install-LegalAction-Smokes mit Wrong-Side-/Stale-Revalidation, side-sicheren PublicPayload-/PlayerView-Assertions und Replay-/StateHash-Stabilitaet. Die zehn Runner-Event-Zielkarten haben zusaetzlich einen expliziten No-`play_event`-Promotion-Guard, bis konkrete Event-Resolver vorliegen. `docs/derived/V1_9_22_RUNNER_PROGRAM_READINESS_REVIEW.md` und ein Engine-Guard schuetzen die 14 Runner-Programme vor erfundenen Kosten-/MU-/Breakerwerten. Keine Karte wurde release- oder AI-promotet. Verifikation: JSON-Validation fuer 302 Dateien, `catalog` 36, `engine` 276 nach Program-Guard, `ai` 85, `server` 72, `web` 77, `typecheck`, `test`, `lint` und `build` gruen; Build nur mit bekannter nicht-blockierender Turbopack-NFT-Warnung. Cursor bleibt V1.9.22 `implementing`; naechster sinnvoller Schnitt ist ein bestaetigter Runner-Event-Resolver, eine lokal bestaetigte Runner-Programmkarte oder ein nicht-promotender Program-Guard.
