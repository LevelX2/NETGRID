...
Korp-Sicht Runner-Zonen-Layout ist nachgezogen: `apps/web/app/page.tsx` rendert gegnerische Runner-`Grip`-, `Stack`- und `Heap`-Informationen jetzt als kompakten `RunnerOpponentZonesStrip` direkt beim Runner-Rig statt in der unteren HQ-/Korp-Zeile. `Grip` und `Stack` zeigen dort nur Count/Handlimit-Count mit Accessible Labels; der gegnerische `Stack` nutzt keine Kartenrücken oder Slots mehr. `Heap` bleibt als öffentliche Zone über ein kompaktes Details-Element auffindbar. `apps/web/app/globals.css` ergänzt Desktop- und schmale Layoutregeln. Verifikation: `corepack pnpm --filter @netgrid/web typecheck`, `corepack pnpm --filter @netgrid/web test`, Browser-/Playwright-Prüfung Desktop 1280x900 und Narrow 390x900 grün.
