...
Current selected next scope: V1.6.3 Mechanikpaket C (ChoiceFlow-Gegnerentscheidung/Guessing, generische Upgrade-Fähigkeiten, Uninstall und InstalledCard-Destroy) ist als nächster Umsetzungsscope ausgewählt. V1.6.2 Mechanikpaket B ist abgeschlossen und lokal verifiziert (`docs/derived/V1_6_2_IMPLEMENTATION_REVIEW.md`, `docs/derived/V1_6_2_FINAL_REVIEW.md`); die Folgeplanung bleibt als verbindliche Sequenz V1.6.3 bis V1.9.0 in `docs/derived/NETGRID_CONSOLIDATED_RELEASE_ROADMAP.md` hinterlegt. Deck-Legal AI Approval Batch B-G bleibt weiterhin ein eigenständiger Gate-Strang.
