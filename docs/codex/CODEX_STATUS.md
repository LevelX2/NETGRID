...
V1.9.16-WIP-Lauf vom 2026-05-13 08:38 CEST: Der Program Subtypes, Hosting, Stealth, Worm und Installed-card-Destroy-Slice ist release-spezifisch detailgeplant und im Runtime-/Engine-/AI-Artefakt-WIP. Der Scope umfasst genau 16 Karten von `Baedeker's Net Map` bis `Fragmentation Storm`. Alle 16 Zielkarten haben WIP-Runtime-Definitionen, `ONR_V1_9_16_WIP_CARD_IDS` bleibt ein No-Promotion-Guard, und neue WIP-Datenartefakte liegen unter `data/scenarios/v1916-program-subtype-hosting-stealth-smoke.json`, `data/manifests/card-implementation-manifest-1.9.16.json`, `data/rules/mechanics-coverage-1.9.16.json`, `data/ai/ai-card-hints-deck-legal-v1916.json`, `data/scenarios/ai-deck-legal-v1916-smokes.json` und `data/manifests/deck-legal-ai-approval-v1916-manifest.json`. Neu abgedeckt sind installierter Link in side-sicheren Trace-Choices, Stealth-/Recurring-Refresh sowie Fragmentation Storm mit Trace-Erfolgsgate fuer Programm-Trash und Net Damage; Erfolg und Misserfolg replayen zum finalen StateHash. Die KI-Artefakte sind WIP mit `pending_engine_full_gate`, nicht `ai_supported`. Verifikation: JSON-Validation fuer 258 `data/**/*.json`, `engine` (231), `catalog` (30), `ai` (84), `server` (72), `web` (76) und `typecheck` sind grün. Keine Karte wurde promotet; der Cursor bleibt auf V1.9.16 in Phase `implementing`.
