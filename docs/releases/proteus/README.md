...
`docs/releases/proteus/` bündelt die Artefakte für den importierten Proteus-Kartenstand. Import, Coverage, Slicing und Mechanikverträge bleiben die führenden Handoffs; die abgeschlossenen Detail-Slices geben einzelne Karten beziehungsweise Foundations eng begrenzt für die Runtime frei. Nach PRO020 ist der finale Proteus-Abschluss-/Verify-Slice erledigt: 154 von 154 Proteus-Karten stehen als konkrete CardImplementation-Dateien bereit; es fehlt keine konkrete CardImplementation-Datei mehr. Nach dem Human-vs-Human-Decklegal-Gate sind alle 154 Proteus-Karten `deck_legal` und `format_legal`; `ai_supported` bleibt bis zu separaten AI-Hints, SzenarioRefs und AI-Smokes geschlossen.
...
Proteus ist für Human-vs-Human im expliziten Kartenpool `Originalset & Protheus` deck- und formatlegal. Abgeschlossen sind die Done-Activities für Phase 1a, 1b, 1d, 1g, 2a bis 2d, 3a bis 3c, 3e, 4a, 5b, 7a bis 7d, 8a bis 8f, 9d sowie PRO007, PRO008, PRO009, PRO009-1, PRO009-2, PRO010, PRO010-1, PRO011, PRO011-1, PRO012, PRO012-1, PRO013, PRO013-1, PRO014, PRO014-1, PRO014-2, PRO015, PRO015-1, PRO016, PRO016-1, PRO017, PRO017-1, PRO017-2, PRO018, PRO019, PRO019-1, PRO019-2 und PRO020. Die freigegebenen Karten sind `human_playable`, `deck_legal` und `format_legal`, aber weiterhin nicht `ai_supported`. PRO018 setzt `Hijack` und `Test Spin` mit runnerprivaten Hidden-Zone-Installationschoices, temporären Installationscredits, deterministischem Shuffle, Run-Followup und Return-or-Penalty-Cleanup um. PRO019 schließt die acht regelvertraglichen Baseline-/Utility-Karten mit bounded Emergency-Rig-X, Ice-and-Data-Play-Cost 3, Obfuscated-Fortress-Run-Spend-Ledger, Pavit-HQ-to-Fort-Replacement, Simon-Access-Queue-Reduktion, Herman-Fort-Reorder und Marcel-Top-R&D-Trash-Kosten; PRO019-1 härtet Simon-Access-Queues, Pavit-Rez-/HQ-Choice-Revalidierung, Obfuscated-Fortress-Run-Zahlungszählung und Ice-and-Data-Fortbindung nach; PRO019-2 ergänzt Pavit-Kombinationslegalität und bestätigt die zentrale Root-vor-stored-card-Access-Policy für HQ/R&D. PRO020 bestätigt den finalen Datei-/Registry-/Manifest- und Activity-Audit. Der Harness steht bei 154/154/0 ohne Drift. Alte Phase-Activities bleiben nur Statusreferenzen und zählen nicht als konkurrierender Implementierungsnachweis.

Phase 1c ist durch PRO019 als Statusreferenz aufgelöst: `Emergency Rig` nutzt die dokumentierte X-Grenze `1..max(1, Rez-Kosten des Ziel-ICE)`, `Rent-to-Own Contract` legt Term-Counter in Höhe der Ziel-ICE-Rez-Kosten. Die alte Activity bleibt nur als Referenz auf `docs/activities/done/act-2026-05-29-proteus-pro019-rule-contract-baseline-utilities.md`.

Phase 1e ist durch PRO019 als Statusreferenz aufgelöst und durch PRO019-1/PRO019-2 nachgehärtet: `Herman Revista`, `Marcel DeSoleil`, `Pavit Bharat` und `Simon Francisco` sind konkrete CardImplementation-Dateien. Private Fort-Reorder-/HQ-to-Fort-/Top-R&D-Pfade redigieren verdeckte Identitäten; Pavit nutzt gemeinsame HQ-Ersatzmengen-Legalität mit Root-Kapazitätsprüfung, und Simon reduziert eine spätere gespeicherte zentrale Access-Queue-Position, nachdem HQ-/R&D-Root-Upgrades separat vor gespeicherten Kartenzugriffen queued wurden.

Phase 1f ist durch PRO019 als Statusreferenz aufgelöst: `Obfuscated Fortress` zählt alle Runner-Credit-Zahlungen, die durch den runweiten Payment-Pfad `spendRunnerRunCredits` laufen, gegen die Ansage. Am Run-Ende verliert der Runner die nicht ausgegebene Differenz, gedeckelt durch aktuelle normale Credits.
...
Phase 2e/2f/PRO015 ist abgeschlossen; dokumentiert ist dies in `docs/activities/done/act-2026-05-28-proteus-pro015-bad-publicity-run-replacement-suite.md`. PRO015-1 ist als Nachhärtung in `docs/activities/done/act-2026-05-28-proteus-pro015-1-bad-publicity-hardening.md` dokumentiert. Die alten Umbrella-Activities für Phase 2e und 2f bleiben nur Referenzen ohne doppelte Zählung.
...
Phase 9b ist durch PRO017 erledigt und mit PRO017-1 sowie PRO017-2 nachgehärtet. Die alte Activity `docs/activities/in-progress/act-2026-05-24-proteus-phase-9b-action-economy-debt.md` bleibt nur als Referenz; die Zählung erfolgt über `docs/activities/done/act-2026-05-28-proteus-pro017-action-economy-debt-suite.md`, die Härtungen über `docs/activities/done/act-2026-05-28-proteus-pro017-1-action-economy-hardening.md` und `docs/activities/done/act-2026-05-28-proteus-pro017-2-pdca-damage-timing-hardening.md`.

Phase 9c ist durch PRO018 erledigt. `Hijack` installiert ein legales Programm oder eine Hardware aus der Runner-Grip mit genau drei temporären Installationscredits; `Test Spin` sucht ein Programm im Runner-Stack, installiert es kostenlos, shufflet deterministisch, startet den gewählten Run und löst danach Return-to-Stack oder Kosten-/Meat-Damage-Penalty aus. Die alte Phase-9c-Activity bleibt nur als Referenz; die Zählung erfolgt über `docs/activities/done/act-2026-05-28-proteus-pro018-hidden-zone-search-install-tutor-suite.md`.

Phase 9e ist durch PRO019 als Statusreferenz aufgelöst: `Ice and Data Special Report` wird mit Play-Cost 3 umgesetzt; die Klammerangabe `(0)` wird lokal als Quellen-/Druckartefakt behandelt und nicht als alternativer Kostenpfad. Der Effekt exposed bis zu fünf installierte Korp-Karten in oder auf einem einzelnen Data Fort über eine runnerprivate Choice.
