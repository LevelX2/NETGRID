...
| Konkrete Proteus-CardImplementation-Dateien | 80 | `packages/engine/src/card-implementations/proteus/*.ts` |
| Registry-paritätische Implementierungen | 80 | `coverage.test.ts`-Guard gegen Registry |
| Fehlende konkrete CardImplementation-Dateien | 74 | Gesamtbasis minus konkrete Dateien |
| Manifest-`implemented`-Einträge | 80 | Driftprüfung in `data/manifests/proteus-card-support.json` |
...
| --- | ---: | --- |
| `done + implemented/foundation` | 30 | Erledigte Runtime-, Foundation- oder gezielte Härtungsslices; Implementierungsfortschritt wird trotzdem nur über Dateien plus Registry gezählt. |
...
| Gesamt | 74 | Proteus-Activity-Dateien im Board. |
