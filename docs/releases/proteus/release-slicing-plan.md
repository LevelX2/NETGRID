...
- Proteus Phase 1f ist blockiert: `Obfuscated Fortress` braucht vor Promotion einen verbindlichen Run-Payment-Source-Vertrag für normale Credits, Bad-Publicity-Credits, temporäre Run-Credits, Hosted-/Recurring-Credits und Stealth-/zweckgebundene Bits. Der Blocker ist in `docs/activities/in-progress/act-2026-05-24-proteus-phase-1f-run-spend-cap.md` dokumentiert.
- Proteus Phase 1g ist umgesetzt: `Disintegrator` ist über eine eigene CardImplementation-Datei, einen generischen Runner-Post-Pass-Ability-Baustein für fully-broken ICE, Manifest-/Szenario-Spur und Replay-/StateHash-Tests `implemented`, `engine_supported`, `playable` und `human_playable`. Es wurden keine neuen Proteus-ID-Branches in Runtime, UI, Catalog oder KI ergänzt.
