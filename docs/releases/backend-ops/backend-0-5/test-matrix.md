...
| B05-T-SRV-005 | Deployment- und Auth-Gate | Maintenance ist im `private_internet`-Profil standardmäßig aus; bei expliziter Aktivierung sind HTTPS-Origin, vertrauenswürdiger Proxy, Adminsitzung, CSRF und Reauth-Gates nachgewiesen. | `apps/server/src/maintenance-http-auth.test.ts` und Backend-0.5-Deployment-Tests. |
