...
Der Lauf hat die Queue-Ordner `inbox`, `in_progress`, `done` und `blocked` sowie `docs/derived/ORIGINALSET_CARD_SPOTCHECK_REGISTER.md` und `data/reports/originalset-card-spotcheck-register.json` als Tabuquellen gelesen. Dabei wurden 170 tabuierte O:NR-v1-Card-IDs erkannt.
