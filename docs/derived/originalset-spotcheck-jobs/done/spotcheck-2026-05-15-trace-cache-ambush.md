...
## Umsetzungslauf 2026-05-15

Status: `blocked` mit grünem Teilfix.

Umgesetzt:

- `Ice Pick Willie` wurde vom falschen R&D-Top-Reveal auf den lokalen Subroutine-Vertrag `Trash a program` plus `End the run` korrigiert. Der Nachtest stellt sicher, dass keine R&D-Identität in Payload oder Runner-View leakt.
- `TRAP!` verursacht nun aus legalem Access-Fenster 3 Net Damage plus 1 Tag; Archives bleibt No-op, R&D-Reveal bleibt payloadfähig.
- `Cerberus` nutzt nun 3 Net Damage als erste Subroutine und entfernt den falschen Trace-Tag-Erfolg. Der vollständige Cerberus-Counter-/Run-Start-Damage-/Removal-Vertrag bleibt offen.

Blockierend offen:

- `Signpost` und `The Springboard`: post-bid Trace-Link-Fenster nach offengelegten Bids, Kosten, Einmal-pro-Trace-Grenze und AI-Bid-Policy.
- `Deal with Militech`: Research-Agenda-Turn-History plus Militech-Counter auf installierten Icebreakern.
- `Hunt Club BBS`: Runner-Choice für bis zu drei installierte Corp-Karten ohne Zielidentitäts-Leak vor Expose.
- `Sneak Preview`: temporärer Programminstall aus Heap oder Stack mit Shuffle, Memory-/Hosting-Revalidation und End-of-turn-Return.
- `Code Viral Cache`: HQ-Erfolgsbedingung, Purge-Replacement mit Runner-Counterchoice und Korp-Trash-Aktion.
- `Paris City Grid`: servergebundener 6-Bit-Trace-Pool während Runs auf diesem Fort, Payment-Priorität und Corp-Turnstart-Refresh.
