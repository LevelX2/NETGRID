...
Der naechste Code-Schnitt sollte genau eine dieser Karten freigeben, sobald ihr offener Vertrag geschlossen ist. Nach Errata 1.70 bleiben `Security Purge` und `Tutor` kleine Kandidaten: `Security Purge`, wenn Serverziel/Reihenfolge festgelegt sind; `Tutor`, wenn die spaetere Zusatz-ETR-Subroutine als breakbarer run-weiter Modifier modelliert wird.
