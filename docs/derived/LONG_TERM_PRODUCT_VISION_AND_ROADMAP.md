...
| KI-Gegner | V0.9 stärkere KI, side-sichere Inputs, KI-vs-KI Soaks vorhanden. | KI ist noch heuristisch und nicht vollständig regel-/kartenbreit. KI-Pacing fehlt. LLM darf keine Regelautorität sein. | AI LegalActions-only, hidden-invariance, soaks, difficulty regression, explanations leakfrei. |
...
| Audio/Animation/Komfort | S01 opt-in Audio; Action-Audio geplant. | Audio/Animation darf alte Events bei Reconnect nicht abspielen und darf nicht nerven. | Opt-in, volume, reduced motion, no replay after reconnect, no server/engine effect. |
