...
| `docs/derived/V1_0_*` bis `V1_1_*` | mehrere Dutzend | Markdown | Stabilisierung, UX, Storage, Hardening, Setup, Archives | Releasepläne, Requirements, Specs, Reviews, Smokes | `keep-evidence`, teilweise `condense` | Bestehende Folgepakete für V1.0-/Small-Release-Rollups nutzen; Final Reviews als führende Nachweise behalten. |
| `docs/derived/V1_2_*` bis `V1_8_*` | mehrere Dutzend | Markdown | Card Data Pipeline, KI, Belief, Simulation, Replay, Tutorial | Release- und Architekturartefakte | `keep-evidence`, teilweise `condense` | Releaseketten-Rollup nach Themenlinie: Card Data, AI, Replay/Lernhilfe, Tutorial. |
| `docs/derived/V1_9_*` | größter Releaseblock | Markdown | Originalset-Completion, Mechaniken, Karten, KI-Enablement | Pläne, Requirements, Reviews, Testmatrizen, Gate-Nachweise | `keep-evidence`; aktuelle WIP-/Gate-Dateien `keep-active` | Führende Final Reviews je Release behalten; nach Abschluss der Kette V1.9.10 bis V1.9.22 ein Originalset-Completion-Rollup schneiden. |
...
| `docs/derived/S01_*` | 7 | Markdown | Spielende, Ergebnisfenster, Matchserie, Audio | Sonderphasenartefakte | `condense`, danach `archive` | Bestehender Folgehinweis: `act-2026-05-17-docs-derived-s01-rollup-proposal`. |
...
| `docs/derived/originalset-spotcheck-jobs/done/` | 39 | Markdown | Originalset-Spotchecks | Einzeljob-Nachweise | `keep-evidence`, später `git-remove-after-condense` | Bestehender Bezug: `act-2026-05-17-docs-spotcheck-evidence-rollup`; nach vollständigem Rollup und Linkcheck können Einzeljobs Retention-Kandidaten werden. |
...
3. `docs/derived/` zuerst entlang bestehender Releaseketten verdichten: MVP-0.x, V1.0/V1.1, V1.2-V1.8, V1.9-Originalset, V2-Plattform, Proteus, AI.
