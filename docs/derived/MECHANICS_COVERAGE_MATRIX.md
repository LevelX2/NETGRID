# Mechanics Coverage Matrix 1.2.1

Status: V1.1.3 Normalisierung nach V1.1.2K, aktualisiert nach V1.2.0/V1.2.1
Stand: 2026-05-08
Maschinenlesbar: `data/rules/mechanics-coverage-1.2.1.json` ist der aktuelle versionierte JSON-Stand nach V1.2.0 Event Modification Foundation und V1.2.1 Replacement Effects. `data/rules/mechanics-coverage-0.99.json` bleibt nur historische Grundlage.
...
| `mechanic.damage.flatline` | Net/Meat/Core Damage und Flatline | `implemented_limited` | P1 | sehr hoch | erledigt/V1.1.1/V1.2.1 | Net-/Meat-Damage, Core Damage und Flatline-Grund sind spielbar; V1.2.0/V1.2.1 hängen test-only Damage Prevention und Damage Replacement an, ohne Runtime-Karten zu erweitern. |
...
| `mechanic.event_modification.prevent_avoid_interrupt` | Prevention, Avoid, Interrupts | `implemented_limited` | P0 | sehr hoch | erledigt/V1.2.0 | V1.2.0 setzt `ImminentEvent`, side-private `would`/`prevent`/`avoid`/eng geführte `interrupt`-Fenster und Damage Prevention als test-only Pilot um. Avoid und Interrupt bleiben Foundation-only ohne Runtime-Pilot. |
| `mechanic.event_modification.replacement` | Replacement Effects | `implemented_limited` | P0 | sehr hoch | erledigt/V1.2.1 | V1.2.1 setzt Replacement als getrennte Pipeline mit Originalevent, Replacementevent, einmal-pro-Fenster-Regeln, deterministischer Kandidatenordnung und sichtbaren Konfliktblockern um. Der Pilot bleibt test-only Damage Replacement. |
...
| `mechanic.special_cases` | Extra Cards, Set Aside, Remove from Game, Ownership/Control | `open` | P2 | hoch | nach V1.2.1 | Spezialfälle bleiben nach Event Modification und Replacement eigene Gates; nicht Teil von V1.1.3 bis V1.2.1. |
...
| DEV-008 Prevention/Replacement/Interrupt | partial | Aufgeteilt und als Foundation umgesetzt: Prevention/Avoid/Interrupt in V1.2.0 mit test-only Damage Prevention; Replacement in V1.2.1 mit test-only Damage Replacement. Breite Runtime-Karten, Avoid-/Interrupt-Piloten und weitere Eventfamilien bleiben spätere Gates. |
