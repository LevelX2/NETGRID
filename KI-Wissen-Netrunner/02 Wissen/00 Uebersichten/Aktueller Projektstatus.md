...
- Die detaillierte Planung für V0.94 bis V0.99 liegt vor. V0.94 ist als Damage-/Flatline-Gate mit engem Game-End-Grundvertrag geplant, V0.95 als Resource-/Tag-Interaktionsgate, V0.96 als Trace-/Link-/Bidding-Gate, V0.97 als Run-/Breach-/Multiaccess-Gate, V0.98 als Identity-/Hidden-Zone-Gate und V0.99 als Counter-/Hosting-/Virus-/Purge-Gate. Der Querreview `MVP_0.94_0.99_PLANNING_REVIEW.md` prüft Abdeckung, Integration und Testpflichten für M3 bis M10.
...
- Der private lokale O:NR-v1-Limited-Testzugang wurde nach V0.94 aktualisiert: 45 manuell verifizierte lokale Karten sind in Shared/Engine als testbare `playable_mvp`-Karten verfügbar und im ignorierten lokalen Snapshot/Index/Report als `implemented/playable/deck_legal` markiert. Die lokalen Asset-/Titel-Zuordnungskonflikte sind visuell nachgeprüft und beseitigt; `Data Wall`, `Data Wall 2.0`, `Rock Is Strong` und `Datapool by Zetatech` wurden zusätzlich aktiviert. `Data Darts` und `Data Raven` bleiben wegen fehlender Mechaniken katalog-only.
- Die Web-seitige Kartenpool-Runtime ist konsolidiert: Katalog-API und Deckvalidierung laden Basis-Snapshot plus privaten lokalen O:NR-v1-Overlay über `apps/web/app/api/card-pool-runtime.ts`. Damit müssen lokale testzugängliche Karten nicht mehr parallel in zwei Web-API-Pfaden gepflegt werden; Engine-/Manifest-/Test-Gates bleiben weiterhin Voraussetzung für Spielbarkeit.
...
- V0.92, V0.93 und V0.94 sind abgeschlossen. V0.95 bis V0.99 sind geplant, aber nicht implementiert; ihre Detailpläne dienen als Grundlage für spätere Requirements-Freeze- und Umsetzungsthreads.
- Die lokalen O:NR-v1-Scan-Zuordnungen für `Data Darts`, `Data Raven`, `Data Wall`, `Data Wall 2.0`, `Rock Is Strong`, `Datapool by Zetatech` sowie die direkt betroffenen Nachbarn `Data Naga` und `Rockerboy Promotion` wurden im ignorierten lokalen Importbereich repariert. `Data Wall`, `Data Wall 2.0`, `Rock Is Strong` und `Datapool by Zetatech` sind freigegeben; weitere Spielbarkeitsfreigaben brauchen weiterhin Resolver, Manifest, Engine-Test, Visibility-Test und Replay/StateHash-Abdeckung.
