import { execFileSync } from "node:child_process";
import fs from "node:fs";

const mdPath =
  "docs/reviews/ai/ai044-deck-doctrine-v2-diagnostic-schema-2026-06-04.md";
const jsonPath =
  "docs/reviews/ai/ai044-deck-doctrine-v2-diagnostic-schema-2026-06-04.json";
const codePath = "packages/ai/src/action-doctrine-goal-diagnostics.ts";
const testPath = "packages/ai/src/action-doctrine-goal-diagnostics.test.ts";

function read(path) {
  return fs.readFileSync(path, "utf8");
}

function fail(message) {
  console.error(`AI044 check failed: ${message}`);
  process.exit(1);
}

function changedFiles() {
  const names = new Set();
  for (const args of [
    ["diff", "--name-only"],
    ["diff", "--cached", "--name-only"],
  ]) {
    const output = execFileSync("git", args, { encoding: "utf8" }).trim();
    if (!output) continue;
    for (const line of output.split(/\r?\n/)) names.add(line.trim());
  }
  return [...names].filter(Boolean).sort();
}

const md = read(mdPath);
const report = JSON.parse(read(jsonPath));
const code = read(codePath);
const test = read(testPath);

if (report.step !== "AI044") fail("JSON step must be AI044");
if (report.runtimeConsumerStatus !== "none")
  fail("runtimeConsumerStatus must be none");
if (report.schemaVersion !== "deck-doctrine-v2-diagnostic-schema-v1")
  fail("schemaVersion mismatch");
if (report.exportsToProductiveAiIndex !== false)
  fail("must not export to productive AI index");
if (report.runtimeFilesTouched.length !== 0)
  fail("runtimeFilesTouched must be empty");

for (const [flag, value] of Object.entries(report.noEffectFlags ?? {})) {
  if (value !== false) fail(`No-effect flag must be false: ${flag}`);
}

for (const requiredStatus of ["ready", "partial", "blocked"]) {
  if (!report.readinessStatuses.includes(requiredStatus)) {
    fail(`readiness status missing: ${requiredStatus}`);
  }
}

for (const requiredCode of [
  "DECK_DOCTRINE_V2_DIAGNOSTIC_SCHEMA_VERSION",
  "buildDeckDoctrineV2DiagnosticReadinessReport",
  "deckDoctrineGaps",
  "hidden_info_blocked",
]) {
  if (!code.includes(requiredCode)) fail(`code missing: ${requiredCode}`);
}

for (const forbiddenCode of [
  "chooseRunnerPlanAction",
  "chooseCorpPlanAction",
  "chooseSemanticAiAction",
  "applyAction(",
  "getLegalActions(",
  "rankedAlternatives",
  "selectedActionId",
  "numericScore",
  "plannerWeight",
]) {
  if (code.includes(forbiddenCode)) fail(`forbidden code token: ${forbiddenCode}`);
}

for (const runtimeFile of [
  "packages/ai/src/index.ts",
  "packages/ai/src/input-dto.ts",
  "packages/ai/src/runner-plans.ts",
  "packages/ai/src/corp-plans.ts",
]) {
  if (read(runtimeFile).includes("action-doctrine-goal-diagnostics")) {
    fail(`runtime file imports diagnostics: ${runtimeFile}`);
  }
}

for (const requiredText of [
  "keine produktive Action-Auswahl",
  "keine numerischen Action-Scores",
  "keine Rangliste",
  "keine Runtime-Anbindung",
  "keine Hidden-Info-Projektion",
]) {
  if (!md.includes(requiredText)) fail(`Markdown report missing: ${requiredText}`);
}

for (const requiredTest of [
  "surfaces fully evidenced card fields",
  "surfaces unresolved card doctrine gaps",
  "keeps hidden-info-blocked candidates diagnostic-only",
]) {
  if (!test.includes(requiredTest)) fail(`test coverage missing: ${requiredTest}`);
}

const allowedChangedFiles = [
  mdPath,
  jsonPath,
  codePath,
  testPath,
  "scripts/check-ai044-deck-doctrine-v2-diagnostic-schema.mjs",
];
const unexpectedChanges = changedFiles().filter(
  (file) => !allowedChangedFiles.includes(file),
);
if (unexpectedChanges.length > 0) {
  fail(`Unexpected changed files for AI044: ${unexpectedChanges.join(", ")}`);
}

console.log("AI044_DECK_DOCTRINE_V2_DIAGNOSTIC_SCHEMA OK");
