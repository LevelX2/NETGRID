...
for (const entry of entries) {
  if (!entry.cardDefinitionId || !entry.title)
    failures.push("unvollständiger Karteneintrag");
  if (entry.printedStrength === undefined)
    failures.push(`${entry.title}: fehlende Stärke`);
  if (entry.breakAbility.creditCost === undefined)
    failures.push(`${entry.title}: fehlende Breakkosten`);
  ...
  if (
    entry.aiProfileStatus === "missing" ||
    entry.engineImplementationStatus === "missing"
  ) {
    failures.push(`${entry.title}: unvollständige strukturierte Abdeckung`);
  }
}
if (
  new Set(entries.map((entry) => entry.cardDefinitionId)).size !== cards.length
) {
...
if (failures.length > 0) {
  ...
  console.log(
    `Icebreaker-Run-Semantik: ${entries.length} aktive Breaker geprüft.`,
  );
}

function auditEntry(card) {
  const profile = hints.get(card.cardId)?.breakerProfile;
  // This audit deliberately consumes only structured card/hint data. Engine
  // parity is enforced by icebreaker-run-semantics-audit.test.ts, which reads
  // the resulting document and compares it with runtime abilities.
  const hasPump = profile?.pumpCost !== undefined;
  ...
  return {
    ...
    breakAbility: {
      creditCost:
        Math.max(0, Math.floor(profile?.breakCost ?? 0)),
      maxSubroutinesPerUse: Math.max(
        1,
        Math.floor(
          profile?.maxSubroutinesPerBreak ?? 1,
        ),
      ),
    ...
    engineImplementationStatus: profile ? "complete" : "missing",
    aiProfileStatus: profile ? "complete" : "missing",
    // All active breaker mechanics are projected from the Engine quote.  The
    // engine-side audit test guards the structured parity; card-specific path
    // scenarios guard the stateful and probabilistic continuations.
    preRunSolverStatus: "complete",
    remainingCardIdSpecialCase: false,
    remainingRuleTextFallback: false,
    notes: ["engine parity: icebreaker-run-semantics-audit.test.ts"],
  };
}
...
function coverageFor(profile) {
  ...
  if (profile?.configurableCoverage) {
    return {
      ...
      choices: values.filter(isIceSubtype),
    };
  }
