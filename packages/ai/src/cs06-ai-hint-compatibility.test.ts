...
import { deriveCs06AiHint } from "./cs06-ai-hint-compiler";
...
describe("CS06 effective AI hint compatibility", () => {
  ...
  it("keeps payoff use in planning while preserving mechanical damage facts", () => {
    ...
    const hint = deriveCs06AiHint(withoutTacticInterpretation as never);
  ...
  it("fails closed for an unknown Engine family or closed ontology value", () => {
    ...
    expect(() => deriveCs06AiHint(unknownFamily as never)).toThrow(
      "cs06_hint_unsupported_family: unownedFamily",
    );
    ...
    expect(() => deriveCs06AiHint(unknownExchange as never)).toThrow(
      "cs06_unknown_strategic_exchange: unowned_exchange",
    );
    ...
    expect(() => deriveCs06AiHint(unknownTactic as never)).toThrow(
      "cs06_unknown_tactic_signal: unowned_tactic",
    );
  });
});
