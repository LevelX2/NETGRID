...
function runnerHandDevelopmentOverflowBonus(
...
function cardsToDrawFromAction(action: LegalAction): number {
  const payload = action.payload ?? {};
  const values = [
    payload.amount,
    payload.drawAmount,
...
function runnerDiscardFodderCount(context: TacticalPlanBuildContext): number {
  ...
  return Math.max(evaluatedFodder, duplicateOwnHandFodderCount(context.input.playerView));
}
...
function runnerUsefulPlayableHandCount(context: TacticalPlanBuildContext): number {
...
function runnerDrawOverflowUrgencyOverride(
  context: TacticalPlanBuildContext,
  plan?: TacticalPlan,
): RunnerDrawOverflowUrgencyOverride {
  const targetServerId = plan?.target?.kind === "server" ? plan.target.id : undefined;
  if (
...
function runnerHasScoreThreatGoal(
  ...
  return (context.runnerTacticalGoals ?? []).some(
    (goal) =>
      ...
      (!targetServerId || !goal.targetServerId || goal.targetServerId === targetServerId),
  );
}
...
function runnerDrawOverflowPenalty(params: {
  currentOverflow: number;
  projectedOverflow: number;
  severity: RunnerDrawOverflowSeverity;
  discardFodderCount: number;
  ...
  const base =
    params.severity === "minor" ? 85 :
    params.severity === "moderate" ? 230 :
  ...
  const usefulPlayablePenalty =
    Math.min(210, params.usefulPlayableCardsInHand * 70);
  const blockedByCreditsPenalty =
    Math.min(120, params.usefulHandCardsBlockedByCredits * 60);
  const valuableRiskPenalty = params.valuableCardsAtRisk * 55;
  const urgencyReduction = params.urgencyOverride === "none"
    ? 0
    : params.severity === "high" ? 200 :
      params.severity === "moderate" ? 150 :
      90;
  return Math.max(
    0,
    Math.min(
...
function runnerDrawOverflowReasons(params: {
  currentOverflow: number;
  projectedOverflow: number;
  severity: RunnerDrawOverflowSeverity;
  discardFodderCount: number;
  ...
  if (params.discardFodderCount > 0) reasons.push("discard_fodder_reduces_penalty");
  if (params.valuableCardsAtRisk > 0) reasons.push("valuable_hand_cards_at_risk");
...
function runnerDrawOverflowRationale(
...
function runnerDrawOverflowEvidence(
  assessment: RunnerDrawOverflowAssessment,
): string[] {
  return [
...
function runnerDrawOverflowSupportsCreditPlan(
  assessment: RunnerDrawOverflowAssessment,
): boolean {
...
function runnerDrawOverflowCreditPriorityBoost(
  assessment: RunnerDrawOverflowAssessment,
): number {
  const severityBoost =
    assessment.severity === "high" ? 250 :
    assessment.severity === "moderate" ? 150 :
    assessment.severity === "minor" ? 60 :
    0;
  return Math.min(
    290,
    severityBoost + Math.min(90, assessment.usefulHandCardsBlockedByCredits * 45),
  );
}
