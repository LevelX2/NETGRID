...
describe("Runner turn planning coverage", () => {
  ...
  it("keeps an unbound installed card run ability with the existing run owner", () => {
    const actionId = "runner.activated_card_ability.shredder.archives.make_run";
    ...
    expect(dispositions).toEqual([
      expect.objectContaining({
        ...
        evidenceCode: "runner_card_run_ability_has_no_current_bound_route:hq",
      }),
    ]);
  });

  it("keeps an unbound strategic exchange with the economy owner", () => {
    const actionId = "runner.install.debt-resource";
    const dispositions = runnerCoverageDispositions({
      input: {
        side: "runner",
        playerView: { stateVersion: 74, own: { clicks: 4 } } as never,
        eventTail: [],
        legalActions: [
          {
            actionId,
        ...
        difficulty: "hard",
        seed: "runner-strategic-exchange-coverage",
        decisionId: "runner-strategic-exchange-coverage:runner:1",
        actionNumber: 1,
        profileId: "runner-strategic-exchange-coverage-test",
      },
      existing: [],
      candidates: [
        {
