...
const CS06_HINT_ENGINE_FIELDS = new Set([
  "schemaVersion",
  "characteristics",
  "abilities",
  "accessEffects",
...
export function deriveCs06AiHint(
  entry: ReturnType<typeof cs06PlanningCards>[number],
): AiCardHint {
  for (const field of Object.keys(entry.planning.engine))
    if (!CS06_HINT_ENGINE_FIELDS.has(field))
      throw new Error(`cs06_hint_unsupported_family: ${field}`);
  ...
  const costProfile = deriveCostProfile(annotations, entry.planning.engine);
...
function strategySupportRole(value: string): KnownHintStrategySupportPairRole {
  ...
  if (role === undefined)
    throw new Error(`cs06_unknown_strategy_support_role: ${value}`);
  return role;
}
...
function closedPlanningValue<const Values extends readonly string[]>(
  ...
  if (result === undefined) throw new Error(`cs06_unknown_${field}: ${value}`);
  return result;
}
...
function deriveAiSupportEvidence(
  ...
  if (
    ...
    throw new Error(
      `cs06_ai_support_scenario_evidence_missing: ${entry.definition.id}`,
    );
...
function deriveRoles(
  ...
  if (breaker?.kind === "break_subroutine") {
    ...
    for (const subtype of coveredSubtypes) {
      if (subtype === "code_gate") roles.add("breaker_decoder");
      if (subtype === "sentry") roles.add("breaker_killer");
      if (subtype === "wall") roles.add("breaker_fracter");
    }
  }
...
function hasHiddenInfoBarrier(
  engine: ReturnType<typeof cs06PlanningCards>[number]["planning"]["engine"],
): boolean {
  return (
...
function deriveBreakerProfile(
  ...
  if (breaker.matches.kind !== "ice_subtype")
    throw new Error(
      `cs06_unsupported_breaker_matcher: ${breaker.matches.kind}`,
    );
  return {
    coverage: [
      closedPlanningValue(
...
function hintDamageResource(
  ...
  throw new Error(`cs06_unknown_damage_type: ${String(damageType)}`);
}
...
function derivedFunctionSignals(
  ...
  if (breakAbility?.kind === "break_subroutine") {
    if (breakAbility.matches.kind === "ice_subtype")
      signals.add(`breaker.${breakAbility.matches.subtype}`);
...
function valueHintKey(axis: string): keyof AiRuntimeValueHints {
  ...
  throw new Error(`cs06_unknown_value_axis: ${axis}`);
}
...
function derivedStrategyEvidence(
  ...
  if (strategyId !== undefined && evidence.size === 0)
    throw new Error(`cs06_strategy_support_evidence_missing: ${strategyId}`);
