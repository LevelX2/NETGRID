...
describe("AI003 strategy goal taxonomy", () => {
  ...
  it("validates the AI017 controlled tactic signal catalog", () => {
    ...
    expect(report.taxonomy.tacticSignalCatalogCount).toBe(484);
  ...
  it("derives AI017/AI018 icebreaker signals without strategy anchors", () => {
    ...
    expect(recurringBreakerCredit.signals).not.toContain(
    ...
    expect(installedBreakerStrengthSupport.signals).not.toContain(
      "ice.strength_reduction",
    );
    ...
    expect(virusStrengthReduction.anchorStrategyIds).toEqual([]);
