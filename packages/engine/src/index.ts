...
const RUNNER_EVENT_RESOLVERS: Record<string, RunnerEventResolver> = {
  ...
  [ALL_NIGHTER_ID]: {
    ...
    resolve: (state, legalAction) => {
      ...
      startRun(state, serverId, undefined, 1, { grantAllNighterBonusRunOnFinish: true });
      legalAction.payload = {
        ...(legalAction.payload ?? {}),
        serverId,
  ...
  "onr_v1_083_desperate-competitor": {
    ...
    resolve: (state, legalAction) => {
      if (!runnerStoleAgendaSubtypeThisTurn(state, "gray_ops")) {
        throw new Error("Im aktuellen Runner-Zug wurde keine Gray Ops Agenda gestohlen.");
      }
      awardRunnerEventAgendaPoint(state, legalAction, "onr_v1_083_desperate-competitor");
    }
  },
  ...
  "onr_v1_090_hot-tip-for-wns": {
    ...
    resolve: (state, legalAction) => {
      if (!runnerStoleAgendaSubtypeThisTurn(state, "black_ops")) {
        throw new Error("Im aktuellen Runner-Zug wurde keine Black Ops Agenda gestohlen.");
      }
      awardRunnerEventAgendaPoint(state, legalAction, "onr_v1_090_hot-tip-for-wns");
    }
  },
  "onr_v1_094_inside-job": {
    ...
    resolve: (state, legalAction) => {
      const serverId = String(legalAction.payload?.serverId) as Exclude<ServerId, "new_remote">;
      startRun(state, serverId, undefined, 1, { bypassFirstIceRemaining: true });
      legalAction.payload = {
        ...(legalAction.payload ?? {}),
        serverId,
        bypassFirstIce: true
...
const CORP_OPERATION_RESOLVERS: Record<string, CorpOperationResolver> = {
  ...
  "onr_v1_283_audit-of-call-records": {
    ...
    resolve: (state, legalAction) => {
      if (runnerRunAttemptsLastTurn(state) < 2) throw new Error("Der Runner hat im letzten Zug nicht mindestens zwei Runs versucht.");
      startTraceFromOperation(state, "onr_v1_283_audit-of-call-records", 5, legalAction);
    }
  },
  "onr_v1_284_chance-observation": {
    ...
    resolve: (state, legalAction) => {
      ...
      startTraceFromOperation(state, "onr_v1_284_chance-observation", 5, legalAction);
    }
  },
...
export function validateGameState(state: GameState): ValidationResult {
  ...
  if (state.run?.remainderStrengthBonusByBreaker) {
    for (const [breakerId, amount] of Object.entries(state.run.remainderStrengthBonusByBreaker)) {
      if (amount === undefined || !Number.isInteger(amount) || amount < 0) {
        errors.push(`Run remainder strength bonus for ${breakerId} must be a non-negative integer.`);
      }
    }
  }
  ...
  if (state.run?.breach) {
    ...
    if (state.run.breach.serverId !== effectiveAccessServerId) errors.push("Breach server must match effective access server.");
    ...
    for (const entry of state.run.breach.queue) {
      ...
      if (entry.serverId !== effectiveAccessServerId) errors.push("Breach entry server must match effective access server.");
    }
...
function corpMainActions(state: GameState): LegalAction[] {
  ...
  for (const server of state.corp.servers) {
    for (const id of server.root) {
      ...
      if (definition.type === "agenda" && effectiveAgendaDifficulty(state, id) <= mustInstance(state.cardInstances, id).advancementCounters) {
    ...
    if (V1919_COUNTER_ASSET_IDS.has(definition.id)) {
      actions.push(
        action(
          ...
          assetId,
          [{ clicks: 1 }],
          { cardId: assetId, v1919AssetAbility: "add_power_counter", counterType: "power", addCounterAmount: 1 }
        )
      );
    }
    if (definition.id === V1919_INFORMATION_LAUNDERING_ID) {
      actions.push(
        action(
          ...
          assetId,
          [{ clicks: 1 }],
          { cardId: assetId, v1919AssetAbility: "gain_credits", gainCreditsAmount: 2 }
        )
      );
    }
  ...
  for (const agendaId of state.corp.scoreArea.slice().sort()) {
    ...
    if (definition.id === AI_CHIEF_FINANCIAL_OFFICER_ID) {
      actions.push(
        action(
          ...
          agendaId,
          [{ clicks: 1 }],
          { cardId: agendaId, agendaAbility: "ai_chief_financial_officer", drawCardsAmount: 5 }
        )
      );
      continue;
    }
    ...
    if (V1919_SCORED_REVEAL_AGENDA_IDS.has(definition.id) && state.corp.rd.length > 0) {
      actions.push(
        action(
          state,
          "corp",
          ...
          agendaId,
          [{ clicks: 1 }],
          { cardId: agendaId, agendaAbility: "v1919_scored_agenda_reveal_rd_top", hiddenZoneAction: "v1919_scored_agenda_reveal_rd_top" }
        )
      );
      continue;
    }
...
function runnerMainActions(state: GameState): LegalAction[] {
  ...
  for (const id of state.runner.grip) {
    ...
    if (hasClicks && definition.type === "program" && !uniqueBlocked && availableRunnerProgramInstallCredits(state) >= (definition.installCost ?? 0)) {
    ...
    if (hasClicks && definition.type === "hardware" && !uniqueBlocked && state.runner.credits >= (definition.installCost ?? 0)) {
      actions.push(action(state, "runner", "install_card", `${definition.title} installieren`, id, [{ clicks: 1, credits: definition.installCost ?? 0 }], { cardId: id }));
    }
    if (hasClicks && definition.type === "resource" && !uniqueBlocked && state.runner.credits >= (definition.installCost ?? 0)) {
    ...
    if (hasClicks && definition.type === "event" && state.runner.credits >= (definition.cost ?? 0)) {
  ...
  if (hasClicks) {
    ...
    for (const resourceId of state.runner.rig.resources.slice().sort()) {
      ...
      if (definition.id === "onr_v1_158_danshis-second-id" && state.runner.tags > 0) {
...
function iceStrengthFor(state: GameState, iceId: CardInstanceId): number {
  ...
  const runEncounterBonus = state.run?.encounteredIceId === iceId ? Math.max(0, Math.floor(state.run.futureEncounterIceStrengthBonus ?? 0)) : 0;
  const clownReduction = state.run?.encounteredIceId === iceId && runnerHasInstalledCardDefinition(state, "runner", "onr_v1_012_clown") ? 1 : 0;
  const pattelsReduction = cardCounter(state, iceId, "virus");
  const total = (definition.strength ?? 0) + instance.strengthModifier + iceStrengthBonusFor(state, iceId) + runEncounterBonus - clownReduction - pattelsReduction;
...
function runnerHasInstalledCardDefinition(state: GameState, side: Side, definitionId: CardDefinitionId): boolean {
  const installed = side === "runner" ? runnerInstalledCardIds(state) : corpInstalledCardIds(state);
  return installed.some((cardId) => definitionFor(state, cardId).id === definitionId);
}
...
function installedVirusCounterTotalForDefinition(state: GameState, definitionId: CardDefinitionId): number {
...
function isVisibleVirusCounterCardForRunner(state: GameState, cardId: CardInstanceId): boolean {
...
function restrictiveNetZoningInstallTax(state: GameState, serverId: Exclude<ServerId, "new_remote">): number {
...
function poxInstallTax(state: GameState, serverId: Exclude<ServerId, "new_remote">): number {
  return Math.floor(poxCountersForServer(state, serverId) / 2);
}

function corpIceInstallAdditionalCost(state: GameState, serverId: Exclude<ServerId, "new_remote">): number {
...
function runnerAccessActions(state: GameState): LegalAction[] {
  ...
  if (definition.type === "asset" || definition.type === "upgrade") {
    ...
    if (freeTrashEnabled) {
      actions.push(
        action(state, "runner", "trash_accessed_card", `${definition.title} kostenlos trashen`, run.accessedCardId, [], {
  ...
  if (freeTrashEnabled) {
    return [
      ...
      action(state, "runner", "decline_trash", run.breach ? "Weiter accessen" : "Access abschließen", "game_rule")
    ];
  }
...
function canFreeTrashCurrentAccessCard(state: GameState, run: ActiveRun, definition: CardDefinition): boolean {
  if (definition.type === "agenda") return false;
  const allowedZones = run.freeTrashAccessZones ?? [];
  if (allowedZones.length === 0) return false;
  const currentZone = run.breach?.queue[run.breach.currentIndex]?.zone ?? accessQueueZone(run.accessServerOverride ?? run.attackedServerId);
  if (currentZone !== "rd" && currentZone !== "hq") return false;
  return allowedZones.includes(currentZone);
}

function isCurrentAccessFromArchives(state: GameState, run: ActiveRun): boolean {
  const cardId = run.accessedCardId;
  if (!cardId) return false;
  const currentEntry = run.breach?.queue[run.breach.currentIndex];
  if (currentEntry?.cardInstanceId === cardId) return currentEntry.zone === "archives";
  const zone = mustInstance(state.cardInstances, cardId).zone;
  ...
  switch (legalAction.type) {
    ...
    case "gain_credit":
      ...
      if (legalAction.payload?.v1919AssetAbility === "add_power_counter") {
        if (legalAction.side !== "corp") throw new Error("Nur die Korp darf V1.9.19-Asset-Counter nutzen.");
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!rezzedCorpRootCardIds(state).includes(sourceCardId)) throw new Error("Die V1.9.19-Asset-Counter-Faehigkeit ist nicht rezzed installiert.");
        const definition = definitionFor(state, sourceCardId);
        if (!V1919_COUNTER_ASSET_IDS.has(definition.id)) throw new Error("Die V1.9.19-Asset-Counter-Faehigkeit passt nicht zur Karte.");
        const addAmount = Number(legalAction.payload?.addCounterAmount ?? 0);
        if (!Number.isInteger(addAmount) || addAmount !== 1) throw new Error("V1.9.19-Counter-Assets laden in diesem WIP genau 1 Power-Counter.");
        addCardCounter(state, sourceCardId, "power", addAmount);
        legalAction.payload = {
          ...(legalAction.payload ?? {}),
          addedCounterAmount: addAmount,
      ...
      if (legalAction.payload?.v1919AssetAbility === "gain_credits") {
        if (legalAction.side !== "corp") throw new Error("Nur die Korp darf V1.9.19-Asset-Economy nutzen.");
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!rezzedCorpRootCardIds(state).includes(sourceCardId)) throw new Error("Die V1.9.19-Asset-Economy-Faehigkeit ist nicht rezzed installiert.");
        const definition = definitionFor(state, sourceCardId);
        if (definition.id !== V1919_INFORMATION_LAUNDERING_ID) throw new Error("Die V1.9.19-Asset-Economy-Faehigkeit passt nicht zur Karte.");
        const gainAmount = Number(legalAction.payload?.gainCreditsAmount ?? 0);
        if (!Number.isInteger(gainAmount) || gainAmount !== 2) throw new Error("Information Laundering gewaehrt in diesem V1.9.19-WIP genau 2 Credits.");
        credits(state, "corp", gainAmount);
        legalAction.payload = {
          ...(legalAction.payload ?? {}),
          gainedCredits: gainAmount,
      ...
      if (legalAction.payload?.resourceAbility === "databroker") {
        ...
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.runner.rig.resources.includes(sourceCardId)) throw new Error("Databroker ist nicht installiert.");
        const agendaCost = Number(legalAction.payload?.agendaPointCost ?? 0);
        if (!Number.isInteger(agendaCost) || agendaCost !== 1) throw new Error("Der Databroker-Agenda-Kostenpfad ist ungueltig.");
        const forfeitAgendaCardId = String(legalAction.payload?.forfeitAgendaCardId ?? "");
        forfeitRunnerAgendaForPointCost(state, forfeitAgendaCardId);
        if (legalAction.payload?.trashOnUse === true) trashRunnerInstalledCardToHeap(state, sourceCardId);
        const gainAmount = Number(legalAction.payload?.gainCreditsAmount ?? 10);
        if (!Number.isInteger(gainAmount) || gainAmount <= 0) throw new Error("Der Databroker-Creditgewinn ist ungueltig.");
      ...
      if (legalAction.payload?.resourceAbility === "silicon_saloon_franchise") {
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.runner.rig.resources.includes(sourceCardId)) throw new Error("Silicon Saloon Franchise ist nicht installiert.");
      }
      if (legalAction.payload?.agendaAbility === "corporate_coup" || legalAction.payload?.agendaAbility === "political_coup") {
        if (legalAction.side !== "corp") throw new Error("Nur die Korp darf Coup-Agenda-Counter ausgeben.");
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.corp.scoreArea.includes(sourceCardId)) throw new Error("Die gewaehlte Coup-Agenda ist nicht gescort.");
        const definition = definitionFor(state, sourceCardId);
      ...
      if (legalAction.payload?.agendaAbility === "v1919_scored_agenda_reveal_rd_top") {
        if (legalAction.side !== "corp") throw new Error("Nur die Korp darf V1.9.19-Scored-Agenda-Faehigkeiten nutzen.");
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.corp.scoreArea.includes(sourceCardId)) throw new Error("Die V1.9.19-Scored-Agenda ist nicht gescort.");
        const definition = definitionFor(state, sourceCardId);
        if (!V1919_SCORED_REVEAL_AGENDA_IDS.has(definition.id)) throw new Error("Die V1.9.19-Scored-Agenda-Faehigkeit passt nicht zur Karte.");
        revealCorpRdTop(state, legalAction);
        legalAction.payload = { ...(legalAction.payload ?? {}), hiddenZoneAction: "v1919_scored_agenda_reveal_rd_top" };
        return;
      }
      if (legalAction.payload?.agendaAbility === "ai_chief_financial_officer") {
        if (legalAction.side !== "corp") throw new Error("Nur die Korp darf die AI Chief Financial Officer Agenda-Aktion nutzen.");
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.corp.scoreArea.includes(sourceCardId)) throw new Error("Die gewaehlte AI Chief Financial Officer Agenda ist nicht gescort.");
        const definition = definitionFor(state, sourceCardId);
      ...
      if (legalAction.payload?.drawCardAfter === true) {
        if (legalAction.side !== "runner") throw new Error("Nur der Runner darf mit diesem Effekt ziehen.");
        drawRunnerCard(state);
      }
      return;
    case "draw_card":
    ...
    case "pump_breaker":
      {
        ...
        executeEffectCommands(state, [{ type: "change_breaker_strength", breakerId: String(legalAction.payload?.breakerId), amount: 1 }]);
      }
      return;
    ...
    case "trash_accessed_card":
      trashAccessedCard(state, mustRun(state).accessedCardId ?? "", legalAction);
      return;
    case "trash_resource":
    ...
    case "remove_tag":
      ...
      if (legalAction.payload?.resourceAbility === "danshis_second_id") {
        const sourceCardId = String(legalAction.payload?.cardId ?? "");
        if (!state.runner.rig.resources.includes(sourceCardId)) throw new Error("Danshi's Second ID ist nicht installiert.");
        const requested = Number(legalAction.payload?.removeTagAmount ?? 0);
        if (!Number.isInteger(requested) || requested <= 0 || requested > 3) throw new Error("Die Tag-Entfernung ist ungueltig.");
...
function resolveAiChiefFinancialOfficer(state: GameState, agendaId: CardInstanceId, legalAction: LegalAction): void {
  ...
  state.corp.hq = [];
  state.corp.archives = [];
  state.corp.rd = shuffleStateIds(state, merge, `v192.shuffle.${AI_CHIEF_FINANCIAL_OFFICER_ID}.hq_archives_into_rd.${state.stateVersion + 1}`);
  for (const cardId of state.corp.rd) {
    state.cardInstances[cardId] = { ...mustInstance(state.cardInstances, cardId), faceup: false, rezzed: false, zone: { side: "corp", zone: "rd" } };
  }
  const drawAmount = Math.max(0, Math.floor(Number(legalAction.payload?.drawCardsAmount ?? 5)));
...
function installCard(state: GameState, legalAction: LegalAction): void {
  ...
  if (legalAction.side === "runner") {
    ...
    if (definition.id === "onr_v1_173_restrictive-net-zoning" && (!selectedServerId || selectedServerId === "new_remote")) {
      throw new Error("Restrictive Net Zoning benötigt einen gültigen Zielserver.");
    }
    const restrictiveTargetServerId =
      selectedServerId && selectedServerId !== "new_remote" ? (selectedServerId as Exclude<ServerId, "new_remote">) : undefined;
    if (definition.id === "onr_v1_156_corporate-ally") {
      const agendaCost = Number(legalAction.payload?.installAgendaPointCost ?? 0);
      if (!Number.isInteger(agendaCost) || agendaCost !== 1) throw new Error("Corporate Ally benötigt exakt 1 Agenda-Punkt als Zusatzkosten.");
      const forfeitAgendaCardId = String(legalAction.payload?.forfeitAgendaCardId ?? "");
      forfeitRunnerAgendaForPointCost(state, forfeitAgendaCardId);
      legalAction.payload = {
        ...(legalAction.payload ?? {}),
    ...
    state.cardInstances[cardId] = {
      ...
      ...(definition.id === "onr_v1_173_restrictive-net-zoning" && restrictiveTargetServerId
...
function startRun(
  ...
  state.run = {
    ...
    ...(options?.successfulRunAccessReplacement ? { successfulRunAccessReplacement: options.successfulRunAccessReplacement } : {}),
    ...(options?.successfulRunCreditLoss && options.successfulRunCreditLoss > 0 ? { successfulRunCreditLoss: options.successfulRunCreditLoss } : {}),
    ...(options?.successfulRunRunnerTagGain && options.successfulRunRunnerTagGain > 0 ? { successfulRunRunnerTagGain: options.successfulRunRunnerTagGain } : {}),
    ...(options?.successfulRunCorpDraw && options.successfulRunCorpDraw > 0 ? { successfulRunCorpDraw: options.successfulRunCorpDraw } : {}),
    ...(options?.bypassFirstIceRemaining ? { bypassFirstIceRemaining: true } : {}),
...
function beginEncounter(state: GameState, encounteredIceId: CardInstanceId): void {
  const run = mustRun(state);
  ...
  const queuedFatalDamage = Math.max(0, Math.floor(run.nextEncounterFatalDamage ?? 0));
  run.fatalDamageActiveForEncounter = queuedFatalDamage > 0;
  if (queuedFatalDamage > 0) run.fatalDamageAmountForEncounter = queuedFatalDamage;
  else delete run.fatalDamageAmountForEncounter;
  run.nextEncounterFatalDamage = 0;
  const encounterTax = Math.max(0, Math.floor(run.encounterTaxForFutureIce ?? 0));
  if (encounterTax > 0) {
...
function continueRun(state: GameState, legalAction?: LegalAction): void {
  ...
  for (let index = 0; index < subroutines.length; index += 1) {
    ...
    if (subroutine.type === "set_run_encounter_tax") {
      ...
      run.encounterTaxForFutureIce = Math.max(0, Math.floor(run.encounterTaxForFutureIce ?? 0)) + amount;
    }
    if (subroutine.type === "set_run_future_strength_bonus") {
      const amount = Math.max(0, Math.floor(subroutine.amount ?? 0));
      run.futureEncounterIceStrengthBonus = Math.max(0, Math.floor(run.futureEncounterIceStrengthBonus ?? 0)) + amount;
    }
    if (subroutine.type === "set_next_encounter_unless_fully_break_damage") {
      const amount = Math.max(0, Math.floor(subroutine.amount ?? 0));
      run.nextEncounterFatalDamage = Math.max(0, Math.floor(run.nextEncounterFatalDamage ?? 0)) + amount;
    }
  ...
  if (state.winner) return;
  const encounteredIceId = run.encounteredIceId;
  const encounterFullyBroken = encounteredIceId ? encounterWasFullyBrokenByRunner(run, subroutines) : false;
  if (encounteredIceId && encounterFullyBroken) recordRunFullyBrokenIce(run, encounteredIceId);
  if (run.fatalDamageActiveForEncounter) {
    const fatalDamageAmount = Math.max(0, Math.floor(run.fatalDamageAmountForEncounter ?? 0));
...
function encounterWasFullyBrokenByRunner(run: ActiveRun, subroutines: NonNullable<CardDefinition["subroutines"]>): boolean {
...
function startTraceFromOperation(state: GameState, sourceDefinitionId: string, baseTraceStrength: number, legalAction: LegalAction): void {
  if (state.trace || state.pendingChoice) throw new Error("Es ist bereits ein Trace oder eine Choice offen.");
  if (!Number.isInteger(baseTraceStrength) || baseTraceStrength < 0) throw new Error("Trace strength ist ungueltig.");
  const sourceCardInstanceId = String(legalAction.payload?.cardId ?? "");
  if (!sourceCardInstanceId || !state.cardInstances[sourceCardInstanceId]) throw new Error("Trace-Operation hat keine gueltige Quellenkarte.");
...
function revealArchivesAtBreachStart(state: GameState, run: ActiveRun, legalAction?: LegalAction): void {
  const accessServerId = run.accessServerOverride ?? run.attackedServerId;
  if (accessServerId !== "archives") return;
  const revealedIds = state.corp.archives.filter((cardId) => !mustInstance(state.cardInstances, cardId).faceup);
...
function autoAdvanceArchivesBreachPastNonDecisionCards(state: GameState, legalAction?: LegalAction): void {
  const run = state.run;
  const breach = run?.breach;
  if (!run || !breach || breach.serverId !== "archives" || breach.completed) return;
  ...
  while (true) {
    ...
    if (archivesAccessRequiresDecisionOrEffect(state, current.cardInstanceId)) break;

    queue = queue.map((entry, index) => (index === currentIndex ? { ...entry, status: "accessed" as const } : entry));
    accessedSummaries = [
      ...accessedSummaries,
      {
    ...
    const nextIndex = queue.findIndex((entry, index) => index > currentIndex && entry.status === "pending");
    if (nextIndex === -1) {
      state.run = {
        ...run,
        breach: {
...
function archivesAccessRequiresDecisionOrEffect(state: GameState, cardId: CardInstanceId): boolean {
  const definition = definitionFor(state, cardId);
  if (definition.type === "agenda") return true;
  if (state.ambushHarness?.enabled && (!state.ambushHarness.triggerDefinitionId || state.ambushHarness.triggerDefinitionId === definition.id)) return true;
  if (
  ...
  return (definition.mechanics ?? []).some((mechanic) => mechanic === "access_ambush" || mechanic === "access_trace" || mechanic === "access_replacement");
}

function recordArchivesAutoAccess(legalAction: LegalAction | undefined, count: number): void {
  if (!legalAction || count <= 0) return;
  const previousCount = typeof legalAction.payload?.archivesAutoAccessedCount === "number" ? legalAction.payload.archivesAutoAccessedCount : 0;
  legalAction.payload = {
    ...(legalAction.payload ?? {}),
...
function applySuccessfulRunAccessReplacement(state: GameState, run: ActiveRun): void {
  ...
  const runnerTagGain = Math.max(0, Math.floor(run.successfulRunRunnerTagGain ?? 0));
...
function resolveV1919AssetOnAccess(state: GameState, cardId: CardInstanceId, legalAction: LegalAction): void {
  const definition = definitionFor(state, cardId);
  if (
    definition.id !== V1919_CORPRUNNERS_SHATTERED_REMAINS_ID &&
    definition.id !== V1919_EXPERIMENTAL_AI_ID &&
  ...
  if (legalAction.side !== "runner" || legalAction.type !== "access_card" || state.run?.accessedCardId !== cardId) {
    throw new Error("V1.9.19-Asset-Ambush darf nur aus einem legalen Access-Fenster ausloesen.");
  }

  if (definition.id === V1919_CORPRUNNERS_SHATTERED_REMAINS_ID || definition.id === V1919_EXPERIMENTAL_AI_ID) {
    const candidates = definition.id === V1919_CORPRUNNERS_SHATTERED_REMAINS_ID ? state.runner.rig.hardware : state.runner.rig.programs;
    const targetCardId = candidates
      .slice()
      .sort((left, right) => {
        const leftDefinition = definitionFor(state, left);
        const rightDefinition = definitionFor(state, right);
  ...
  const damageType: DamageType = definition.id === V1919_VACANT_SOULKILLER_ID ? "core" : "net";
...
function stealAgenda(state: GameState, cardId: string, legalAction?: LegalAction): void {
  ...
  const definition = definitionFor(state, cardId);
  if (cardHasSubtype(definition, "gray_ops")) flags.stoleGrayOpsAgendaThisTurn = true;
  if (cardHasSubtype(definition, "black_ops")) flags.stoleBlackOpsAgendaThisTurn = true;
  removeFromAllZones(state, cardId);
...
function delayBizarreEncryptionSchemeAgendaScore(state: GameState, cardId: CardInstanceId, legalAction?: LegalAction): void {
  const run = mustRun(state);
  const definition = definitionFor(state, cardId);
...
function trashAccessedCard(state: GameState, cardId: string, legalAction?: LegalAction): void {
  const definition = definitionFor(state, cardId);
  const rawOverride = legalAction?.payload?.accessTrashCostOverride;
  const overrideCost = typeof rawOverride === "number" ? Math.max(0, Math.floor(rawOverride)) : undefined;
...
function completeCurrentBreachAccess(state: GameState, status: BreachEntryStatus, legalAction?: LegalAction): void {
  const run = mustRun(state);
  const breach = run.breach;
  if (!breach) {
    finishRun(state, true);
...
function applyV181SuccessfulRunCounterTriggers(state: GameState, run: ActiveRun, legalAction?: LegalAction): void {
  ...
  if (pattelsInstalled > 0) {
    ...
    if (targetIceId && state.cardInstances[targetIceId]) addCardCounter(state, targetIceId, "virus", 1);
  }
  const poxInstalled = runnerInstalledCardCountByDefinition(state, "onr_v1_049_pox");
  if (poxInstalled > 0) {
    const serverId = run.attackedServerId;
    const current = poxCountersForServer(state, serverId);
    state.poxCountersByServer = { ...(state.poxCountersByServer ?? {}), [serverId]: current + 1 };
  }
...
function queueIncubatorStartOfTurnTransforms(state: GameState): boolean {
  ...
  if (flags.incubatorPendingTransforms === undefined) {
    ...
    for (let index = 0; index < counterTotal; index += 1) {
      const die = rollDeterministicDie(state, `v191.die.${INCUBATOR_ID}.start_of_turn.roll.${state.stateVersion}.${index}`);
      if (die === 6) pending += 1;
    }
...
function startIncubatorTransformChoice(state: GameState): boolean {
  const flags = ensureRunnerTurnFlags(state);
  const pending = Math.max(0, Math.floor(flags.incubatorPendingTransforms ?? 0));
  ...
  const poxTargets = state.corp.servers
    .map((server) => ({ serverId: server.id, amount: poxCountersForServer(state, server.id) }))
...
function runnerHasInstalledCorporateAlly(state: GameState): boolean {
  return state.runner.rig.resources.some((cardId) => definitionFor(state, cardId).id === "onr_v1_156_corporate-ally");
}

function corpHasScoredExecutiveExtraction(state: GameState): boolean {
  return scoredCorpAgendaIds(state).some((cardId) => definitionFor(state, cardId).id === "onr_v1_201_executive-extraction");
}

function effectiveAgendaDifficulty(state: GameState, agendaId: CardInstanceId): number {
  const definition = definitionFor(state, agendaId);
  if (definition.type !== "agenda") throw new Error("Difficulty kann nur fuer Agenda-Karten berechnet werden.");
  let difficulty = definition.advancementRequirement ?? 0;
  if (runnerHasInstalledCorporateAlly(state)) difficulty += 1;
  if (corpHasScoredExecutiveExtraction(state) && cardHasSubtype(definition, "gray_ops")) difficulty -= 1;
  difficulty -= v1919ServerDifficultyReduction(state, agendaId);
  return Math.max(0, difficulty);
}

function v1919ServerDifficultyReduction(state: GameState, agendaId: CardInstanceId): number {
  const zone = mustInstance(state.cardInstances, agendaId).zone;
  if (zone.side !== "corp" || zone.zone !== "serverRoot" || !zone.serverId) return 0;
  ...
  return server.root.reduce((sum, rootCardId) => {
    ...
    if (!instance.rezzed) return sum;
    return V1919_SERVER_DIFFICULTY_UPGRADE_IDS.has(definitionFor(state, rootCardId).id) ? sum + 1 : sum;
  }, 0);
}

function agendaPointsForScoredCard(state: GameState, cardId: CardInstanceId): number {
  const definition = definitionFor(state, cardId);
...
function pickRunnerAgendaForAgendaPointCost(state: GameState): CardInstanceId | undefined {
...
function forfeitRunnerAgendaForPointCost(state: GameState, cardId: CardInstanceId): void {
  if (!cardId || !state.runner.scoreArea.includes(cardId)) throw new Error("Der Runner kann diese Agenda nicht fuer Kosten forfeiten.");
  if (agendaPointsForScoredCard(state, cardId) < 1) throw new Error("Die gewaehlte Runner-Agenda liefert keinen Agenda-Punkt fuer Kosten.");
  const instance = mustInstance(state.cardInstances, cardId);
  removeFromAllZones(state, cardId);
  const specialZones = ensureSpecialZones(state);
  specialZones.removedFromGame.push(cardId);
...
function scoreAgenda(state: GameState, cardId: string, legalAction?: LegalAction): void {
  ...
  if (instanceBefore.advancementCounters < requiredDifficulty) throw new Error("Agenda hat nicht genug Advancements.");
  removeFromAllZones(state, cardId);
...
function resolveDiscardChoice(state: GameState, legalAction: LegalAction, playerAction: PlayerAction): void {
  ...
  if (expectedCount !== choice.minSelections) throw new Error("Die Discard-Anzahl ist nicht mehr gueltig.");
  const cockroachRandomized = side === "corp" && cockroachRandomHqDiscardActive(state);
  let selectedCards: CardInstanceId[] = [];
  if (cockroachRandomized) {
    selectedCards = discardRandomCorpHqCards(state, expectedCount, `v191.random.${COCKROACH_ID}.hq_discard_phase`);
  } else {
    ...
    if (selectedCards.length !== expectedCount) throw new Error("Die Discard-Anzahl ist nicht mehr gueltig.");
    const hand = handForSide(state, side);
...
function resolveIncubatorTransformChoice(state: GameState, legalAction: LegalAction, playerAction: PlayerAction): void {
  const choice = state.pendingChoice;
  if (!choice || !choice.source.startsWith("v191.incubator_transform")) throw new Error("Es ist keine Incubator-Choice offen.");
  const selectedId = selectedChoiceIds(playerAction.selectedChoices)[0] ?? "";
  const selectedOption = choice.options.find((option) => option.id === selectedId);
  if (!selectedOption || typeof selectedOption.value !== "string") throw new Error("Die Incubator-Auswahl ist ungültig.");

  const value = selectedOption.value;
  if (value.startsWith("card:")) {
    const cardId = value.slice("card:".length);
    if (!cardId || !state.cardInstances[cardId]) throw new Error("Der gewählte Karten-Counter ist ungültig.");
    const available = cardCounter(state, cardId, "virus");
    if (available <= 0) throw new Error("Der gewählte Karten-Counter ist nicht mehr verfügbar.");
    ...
    const serverId = value.slice("pox:".length) as Exclude<ServerId, "new_remote">;
    const available = poxCountersForServer(state, serverId);
    if (available <= 0) throw new Error("Der gewählte Pox-Counter ist nicht mehr verfügbar.");
    state.poxCountersByServer = { ...(state.poxCountersByServer ?? {}), [serverId]: available + 1 };
    legalAction.payload = {
      ...(legalAction.payload ?? {}),
      hiddenZoneBarrier: true,
      hiddenZoneAction: "incubator_transform",
  ...
  const flags = ensureRunnerTurnFlags(state);
  const remaining = Math.max(0, Math.floor((flags.incubatorPendingTransforms ?? 0) - 1));
...
function publicContextForAction(state: GameState, legalAction: LegalAction): Record<string, unknown> {
  ...
  if (typeof legalAction.payload?.archivesAutoAccessedCount === "number") context.archivesAutoAccessedCount = legalAction.payload.archivesAutoAccessedCount;
...
function totalCounters(state: GameState, counterType: CounterType): number {
  const cardCounterTotal = Object.keys(state.cardInstances).reduce((sum, cardId) => sum + cardCounter(state, cardId, counterType), 0);
