...
function makeHost(legalAction: LegalAction) {
  ...
  const host: AccessEffectHandlerHost = {
    ...
    definitions: {
      ...
      dedicatedResponseTeam: definitions.dedicated!.id,
      dieterEsslin: definitions.dieter!.id,
      turbeauDelacroix: definitions.turbeau!.id,
      corprunnersShatteredRemains: definitions.remains!.id,
