...
export function createTurnCorpRuntime(
  ...
  function parseAdvancementDistributionValue(
    ...
    return value.split("|").map((entry) => {
      ...
      if (!targetId || !Number.isInteger(amount) || amount <= 0)
        throw new Error(
          "Advancement-Choice enthaelt ungueltige Placement-Daten.",
        );
      return [targetId as CardInstanceId, amount];
    });
  }

  function sourcePartsForP334Choice(source: string): {
    sourceDefinitionId: CardDefinitionId;
    sourceCardId: CardInstanceId;
    amount: number;
    mode: AdvancementDistributionMode;
  } {
    const [, sourceDefinitionId, sourceCardId, rawAmount, mode] =
      source.split(":");
    const amount = Number(rawAmount);
    if (
      !sourceDefinitionId ||
  ...
  function validateAdvancementDistribution(
    ...
    for (const [targetId, placed] of placements) {
      if (!eligibleTargets.has(targetId))
        throw new Error(
          "Advancement-Counter duerfen nur auf advancebare Ziele.",
        );
      total += placed;
    ...
    if (total !== amount)
      throw new Error(
        "Die Advancement-Verteilung hat die falsche Counterzahl.",
      );
  }
  ...
  function resolveCardImplementationMoveAdvancementChoice(
    ...
    if (sourceMode === "source_card" && fromId !== sourceCardId)
      throw new Error(
        "Diese Karte darf nur eigene Advancement-Counter bewegen.",
      );
    if (fromId === toId)
      throw new Error(
        "Advancement-Counter muessen auf eine andere Karte wechseln.",
      );
    const fromInstance = state.cardInstances[fromId];
