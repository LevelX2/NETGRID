...
describe.each(["real-flow-11", "real-flow-29", "real-flow-47"])(
  "real Engine game flow (%s)",
  (seed) => {
    it("keeps live LegalActions, applyAction, hidden views and replay aligned", () => {
      ...
      const firstCreditAction = playerActionFor(
        state,
        actionOfType(corpMainActions, "gain_credit"),
      );
      const creditsBefore = state.corp.credits;
      state = applyPlayerAction(state, firstCreditAction);
      expect(state.corp).toMatchObject({
      ...
      const resolveDiscard = actionOfType(
      ...
      expect(
        runnerMainActions.some((action) => action.type === "gain_credit"),
      ).toBe(true);
      expect(
        runnerMainActions.some((action) => action.type !== "gain_credit"),
      ).toBe(true);
      ...
      for (const hiddenCorpCardId of state.corp.hq)
        expect(runnerSurface).not.toContain(hiddenCorpCardId);
      const corpSurface = JSON.stringify(getPlayerView(state, "corp"));
      for (const hiddenRunnerCardId of state.runner.grip)
        expect(corpSurface).not.toContain(hiddenRunnerCardId);
...
function actionOfType(
  ...
  if (!action)
    throw new Error(
      `Expected legal action ${type}; received ${actions.map((candidate) => candidate.type).join(", ")}.`,
    );
  return action;
}
...
function applyType(
  ...
  return applyPlayerAction(
    state,
    playerActionFor(state, actionOfType(getLegalActions(state, side), type)),
  );
}
