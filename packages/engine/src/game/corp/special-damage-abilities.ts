...
type AccessAmbushDefinitionIds = {
...
export type CorpSpecialDamageAbilityHost = {
  ...
  definitions?: AccessAmbushDefinitionIds;
  cards: {
    ...
    uniqueDirectLongtailKindForCard: (cardId: CardInstanceId) => string | undefined;
...
export type AccessAmbushDamageAbilityResult = {
  handled: boolean;
  sourceCardId?: CardInstanceId;
  sourceDefinitionId?: CardDefinitionId;
  damageType?: DamageType;
...
export function handleCorpAccessAmbushDamageEffects(
  host: CorpSpecialDamageAbilityHost,
  cardId: CardInstanceId,
): AccessAmbushDamageAbilityResult {
  ...
  const result: AccessAmbushDamageAbilityResult = {
    handled: payload !== beforePayload,
    sourceCardId: cardId,
    sourceDefinitionId: definition.id,
    accessResolved: true,
    ...(payload ? { resolvedPayload: payload as SpecialDamagePayload } : {}),
    ...(host.legalAction?.resolvedEffects
      ? { resolvedEffects: host.legalAction.resolvedEffects }
      : {}),
  };
  if (payload?.damageType === "net" || payload?.damageType === "meat" || payload?.damageType === "core")
...
export function resolveCorpAccessAmbushPaymentChoice(
  host: CorpSpecialDamageAbilityHost,
  ...
  if (
    ...
    !accessEffectApplies(host, sourceCardId as CardInstanceId, effect, accessZone)
  )
...
function resolveAccessAmbushAssetEffect(
  ...
  const legalAction = requireLegalAction(host);
  const definition = host.cards.definitionFor(cardId);
  const ids = requireDefinitions(host);
...
function requireDefinitions(host: CorpSpecialDamageAbilityHost): AccessAmbushDefinitionIds {
  if (!host.definitions)
    throw new Error("Access-Ambush-Definitionen fehlen.");
