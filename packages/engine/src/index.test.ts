...
describe("V1.1.2 Full Archives Access", () => {
  it("turns existing facedown Archives cards faceup at breach start and skips cards without decisions", () => {
    ...
    expect(state.run?.breach?.queue.map((entry) => entry.hiddenInfo)).toEqual([false, false, false]);
    ...
    expect(state.run?.breach?.accessedSummaries).toEqual([
      ...
      { entryId: `${state.run?.runId}.breach.1`, status: "accessed", cardDefinitionId: "simple_economy_asset" }
    ]);
    expect(state.eventLog.at(-1)?.publicPayload).toMatchObject({
      actionType: "start_run",
      hiddenZoneBarrier: true,
    ...
    expect(JSON.stringify(getPlayerView(state, "runner"))).toContain("Simple Economy Asset");
    expect(JSON.stringify(getPlayerView(state, "runner"))).toContain("Simple Agenda");
  });
  ...
  it("preserves Archives queue progress, forbids Archives trash, and replays deterministically", () => {
    ...
    state = apply(state, "runner", (action) => action.type === "start_run" && action.payload?.serverId === "archives");
    expect(getLegalActions(state, "runner").some((action) => action.type === "trash_accessed_card")).toBe(false);
    expect(state.run?.breach?.currentIndex).toBe(2);
    expect(state.run?.breach?.accessedSummaries).toEqual([
      { entryId: `${state.run?.runId}.breach.0`, status: "accessed", cardDefinitionId: "simple_economy_operation" },
      { entryId: `${state.run?.runId}.breach.1`, status: "accessed", cardDefinitionId: "simple_economy_asset" }
