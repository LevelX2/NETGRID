export const CORP_HQ_AGENDA_REVEAL_CARD_ID =
  "onr_v1_314_corporate-negotiating-center";

export const HIDDEN_ZONE_REVEAL_ASSET_CARD_IDS = new Set<string>([]);

export const CORP_HQ_SHUFFLE_DRAW_CARD_ID = "onr_v1_336_rescheduler";

export const HIDDEN_ZONE_REORDER_ASSET_CARD_IDS = new Set<string>([]);

export const SERVER_ICE_SWAP_UPGRADE_CARD_ID =
  "onr_v1_369_singapore-city-grid";

export const RUNNER_STACK_TOP5_EVENT_CARD_ID =
  "onr_v1_093_if-you-want-it-done-right";

export const AUJOURD_OUI_RESOURCE_CARD_ID = "onr_v1_151_aujourdoui";

export const RUNNER_GRIP_TRASH_EVENT_CARD_ID = "onr_v1_103_organ-donor";

export const CORP_ARCHIVES_TO_HQ_OPERATION_CARD_ID =
  "onr_v1_296_off-site-backups";

export const CORP_RD_TOP5_REORDER_OPERATION_CARD_ID =
  "onr_v1_298_planning-consultants";

export const SERVER_EXPOSE_PROGRAM_CARD_IDS = new Set([
  "onr_v1_042_mouse",
  "onr_v1_058_seeya",
]);

export const STACK_SEARCH_PROGRAM_CARD_IDS = new Set([
  "onr_v1_059_self-modifying-code",
  AUJOURD_OUI_RESOURCE_CARD_ID,
  "onr_v1_169_n-e-t-o",
  "onr_v1_177_the-short-circuit",
]);

export const STACK_TOP_REVEAL_PROGRAM_CARD_IDS = new Set<string>([]);

export const COUNTER_STACK_TOP_REVEAL_PROGRAM_CARD_ID = "onr_v1_032_i-spy";

export const STACK_TOP_REORDER_RESOURCE_CARD_ID = "onr_v1_175_ronin-around";

export const SHORT_CIRCUIT_RESOURCE_CARD_ID =
  "onr_v1_177_the-short-circuit";
