...
export function runHiddenContextActionHint(
  view: PlayerView,
  ...
  const runnerRig =
    view.side === "runner" ? (view.own.rig ?? []) : (view.opponent.rig ?? []);
  const breakerIds = new Set(
    breakerActions
      ...
      .filter(
        (id): id is string =>
          typeof id === "string" && id !== "basic_action" && id !== "game_rule",
      ),
  );
  ...
  if (uniqueNames.length === 1)
    return `Eisbrecher verfügbar: Wähle ${uniqueNames[0]} im Rig für Aktionen gegen ${target}.`;
  if (uniqueNames.length > 1)
    return `Eisbrecher verfügbar: Wähle ${uniqueNames.slice(0, 2).join(" oder ")} im Rig für Aktionen gegen ${target}.`;
