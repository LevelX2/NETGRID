...
describe("account statistics client", () => {
  it("loads private filters with credential cookies and cursor pagination", async () => {
    ...
    await loadAccountMatchHistory(
      ...
      { cursor: "result_1", limit: 10 },
      fetcher,
    );

    const [statisticsUrl, statisticsInit] = (
      fetcher as unknown as ReturnType<typeof vi.fn>
    ).mock.calls[0] as [string, RequestInit];
    const [historyUrl, historyInit] = (
      fetcher as unknown as ReturnType<typeof vi.fn>
    ).mock.calls[1] as [string, RequestInit];
    expect(statisticsUrl).toContain(
      "/api/account/statistics?period=30d&side=runner&opponentKind=ai&matchMode=human_runner_vs_corp_ai",
    );
    expect(historyUrl).toContain(
      "/api/account/match-history?period=30d&side=runner&opponentKind=ai&matchMode=human_runner_vs_corp_ai&limit=10&cursor=result_1",
    );
