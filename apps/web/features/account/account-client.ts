...
export function loginAccount(
  input: { loginName: string; password: string; deviceLabel?: string },
  fetcher: AccountFetch = fetch,
): Promise<AccountSessionPayload> {
  return accountRequest(fetcher, "/api/account/login", {
    method: "POST",
    body: JSON.stringify(input),
  });
}

export function acceptAccountInvite(
  input: { inviteToken: string; password: string; deviceLabel?: string },
  fetcher: AccountFetch = fetch,
): Promise<AccountSessionPayload> {
  return accountRequest(
    fetcher,
    `/api/account/invites/${encodeURIComponent(input.inviteToken)}/accept`,
    {
      method: "POST",
      body: JSON.stringify({
...
export function acceptAccountReset(
  input: { resetToken: string; newPassword: string },
  fetcher: AccountFetch = fetch,
): Promise<{ ok: true; sessionsRevoked: true }> {
  return accountRequest(
    fetcher,
    `/api/account/resets/${encodeURIComponent(input.resetToken)}/accept`,
    {
      method: "POST",
...
export function createAccountInvite(
  input: { loginName: string; displayName: string; csrfToken: string },
  fetcher: AccountFetch = fetch,
): Promise<{
  inviteToken: string;
...
export function createAccountReset(
  input: { loginName: string; csrfToken: string },
  fetcher: AccountFetch = fetch,
...
export async function accountRequest<T>(
  ...
  if (!response.ok) {
    throw new AccountClientError(
      ...
      payload.error?.message ?? "Die Account-Anfrage ist fehlgeschlagen.",
      response.status,
    );
  }
  return payload as T;
