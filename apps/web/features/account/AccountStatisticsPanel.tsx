...
export function AccountStatisticsPanel({ accountId }: { accountId: string }) {
  ...
  const [opponentKind, setOpponentKind] =
    useState<FilterValue<ApiPlayerIdentityKind>>("all");
  const [matchMode, setMatchMode] = useState<FilterValue<ApiMatchMode>>("all");
  const [statistics, setStatistics] = useState<ApiAccountStatistics | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    setError("");
    void Promise.all([
      loadAccountStatistics(filters),
      loadAccountMatchHistory(filters),
    ])
  ...
  const loadMore = async () => {
    ...
    setError("");
    try {
      const next = await loadAccountMatchHistory(filters, {
        cursor: nextCursor,
      });
  ...
  return (
    <section
      className="accountStatisticsPanel"
      ...
      <div className="accountStatisticsHeader">
        <div>
          ...
          <p className="muted">
            {statistics
              ? t("recordedSince", {
                  date: formatDate(statistics.statisticsSince, locale),
                })
              : t("secureLoading")}
          </p>
        </div>
        {totals && totals.selfPlay > 0 ? (
          <span className="accountStatisticsSelfPlay">
            {t("selfPlayExcluded", { count: totals.selfPlay })}
          </span>
        ) : null}
      </div>

      <div
        className="accountStatisticsFilters"
      ...
      >
        <label>
          ...
          <select
            value={period}
            onChange={(event) =>
              setPeriod(event.target.value as ApiAccountStatisticsPeriod)
            }
          >
        ...
        <label>
          ...
          <select
            value={side}
            onChange={(event) =>
              setSide(event.target.value as FilterValue<Side>)
            }
          >
            <option value="all">{t("bothSides")}</option>
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              setOpponentKind(
                event.target.value as FilterValue<ApiPlayerIdentityKind>,
              )
            }
          >
        ...
        <label>
          ...
          <select
            value={matchMode}
            onChange={(event) =>
              setMatchMode(event.target.value as FilterValue<ApiMatchMode>)
            }
          >
            <option value="all">{t("allModes")}</option>
            <option value="human_vs_human">{t("humanVsHuman")}</option>
            <option value="human_runner_vs_corp_ai">
              {t("runnerVsCorpAi")}
            </option>
            <option value="human_corp_vs_runner_ai">
              {t("corpVsRunnerAi")}
            </option>
          </select>
        </label>
        ...
        <>
          <div className="accountStatisticsKpis">
            ...
            <StatCard
              ...
              hint={t("ratedGames", { count: totals.gamesPlayed })}
            />
          ...
          <div className="accountStatisticsBreakdowns">
            <Breakdown
              title={t("asRunner")}
              bucket={statistics.bySide.runner}
            />
            ...
            <div className="accountStatisticsBreakdown">
              ...
              <span>
                {t("seriesRecord", {
                  won: statistics.series.seriesWon,
                  lost: statistics.series.seriesLost,
                  drawn: statistics.series.seriesDrawn,
                })}
              </span>
              {side !== "all" || matchMode !== "all" ? (
                <small>{t("seriesFilterHelp")}</small>
              ) : null}
            </div>
          </div>
          ...
          <div className="accountStatisticsSection">
            ...
            <div className="accountStatisticsBreakdowns">
              <Breakdown
                title={t("againstAccount")}
                bucket={statistics.byOpponentKind.account}
              />
              <Breakdown
                title={t("againstGuest")}
                bucket={statistics.byOpponentKind.guest}
              />
              <Breakdown
                title={t("againstAi")}
                bucket={statistics.byOpponentKind.ai}
              />
            </div>
          </div>
          ...
          <div className="accountStatisticsHistory">
            <div>
              <h4>{t("history")}</h4>
              <p className="muted">{t("historyPrivacy")}</p>
            </div>
            {history.length === 0 ? (
            ...
            {nextCursor ? (
              <button
                ...
                type="button"
              >
                {loadingMore ? t("loadingMore") : t("moreResults")}
              </button>
            ) : null}
          </div>
        </>
...
function StatCard({
  ...
  return (
    <div className={`accountStatisticsKpi${tone ? ` ${tone}` : ""}`}>
      <span>{label}</span>
      <strong>{value}</strong>
...
function Breakdown({
  ...
  return (
    <div className="accountStatisticsBreakdown">
      <h4>{title}</h4>
      <strong>{bucket.gamesPlayed}</strong>
      <span>
        {t("record", {
          wins: bucket.wins,
      ...
      <small>
        {t("agendaPoints", {
          for: bucket.agendaPointsFor,
          against: bucket.agendaPointsAgainst,
        })}
      </small>
    </div>
  );
...
function HistoryEntry({
  ...
  return (
    <article
      className={`accountStatisticsHistoryEntry${excluded ? " excluded" : ""}`}
    >
      <div>
        <strong>{t(`outcome.${entry.outcome}`)}</strong>
        <span>{formatDateTime(entry.completedAt, locale)}</span>
      </div>
      <div>
        <span>
          {t("versus", {
            side: entry.side === "runner" ? "Runner" : t("corp"),
            opponent: t(`opponentKind.${entry.opponentKind}`),
          })}
        </span>
        <span>
          {t("agenda", {
            for: entry.agendaPointsFor,
            against: entry.agendaPointsAgainst,
          })}
        </span>
      </div>
      <div>
        <span>{t(`matchMode.${entry.matchMode}`)}</span>
        {entry.series ? (
          <span>{t("seriesGame", { number: entry.series.gameNumber })}</span>
        ) : null}
      </div>
      {entry.finishKind === "forfeit" ? (
        <small>{t("decidedByForfeit")}</small>
      ) : null}
      {entry.outcome === "abandoned" ? (
        <small>{t("abandonedHelp")}</small>
      ) : null}
      {entry.exclusionReason === "self_play" ? (
        <small>{t("selfPlayHelp")}</small>
      ) : null}
    </article>
  );
}
