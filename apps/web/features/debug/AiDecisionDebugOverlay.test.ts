...
describe("AI plan-first decision export", () => {
  it("exports the structured authority contract without legacy score semantics", () => {
    ...
    const parsed = JSON.parse(output) as { display: Record<string, unknown> };

    expect(parsed.display).toMatchObject({
      contractStatus: "plan_first",
      selectedLegalAction: { actionId: "corp.draw", actionType: "draw_card" },
      planFirstDecision: {
        ...
        executionOrigin: { commitmentId: "commitment:corp:turn:2" },
        selectedStep: { stepId: "draw_score_material" },
  ...
  return {
    ...
    detail: {
      ...
      planFirstDecision: {
        ...
        engineQuoteEvidence: { status: "not_reported", evidenceCodes: [] },
