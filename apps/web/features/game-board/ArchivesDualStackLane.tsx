...
export function ArchivesDualStackLane({
  ...
  const { faceupCards, facedownCards, facedownCount } =
    splitArchiveCardsForDisplay(viewerSide, visibleCards, totalArchivesCount);
  const [corpArchivesFacedownView, setCorpArchivesFacedownView] = useState<
    "details" | "backs"
  >("details");
  const { archivePercent } = useCardScaleSettings();
  const archiveCardScale = Math.max(
    CARD_SCALE_PERCENT_MIN / 100,
    archivePercent / 100,
  );
  const shownFaceupCards = faceupCards.slice(0, ARCHIVES_STACK_PREVIEW_LIMIT);
  const shownFacedownCards = facedownCards.slice(
    0,
    ARCHIVES_STACK_PREVIEW_LIMIT,
  );
  const shownFacedownCount =
    viewerSide === "corp"
      ? shownFacedownCards.length
      : Math.min(ARCHIVES_STACK_PREVIEW_LIMIT, facedownCount);
  const faceupOverflow = Math.max(
    0,
    faceupCards.length - shownFaceupCards.length,
  );
  ...
  const faceupRowStyle = {
    "--archive-visible-steps": String(Math.max(0, faceupRowItems - 1)),
  } as CSSProperties;
  const facedownRowStyle = {
    "--archive-visible-steps": String(Math.max(0, facedownRowItems - 1)),
  } as CSSProperties;
  const hasSeparateFacedownToggle =
    viewerSide === "corp" && faceupCards.length > 0 && facedownCards.length > 0;
  const [archiveStep, setArchiveStep] = useState<number | null>(null);
  const showCorpFacedownBacks =
    viewerSide === "corp" && corpArchivesFacedownView === "backs";
  const archiveCardsStyle = useMemo(
    () =>
      ({
        "--archive-card-scale": String(archiveCardScale),
        ...(archiveStep !== null
          ? { "--archive-card-step": `${archiveStep}px` }
          : {}),
      }) as CSSProperties,
  ...
  useEffect(() => {
    ...
    const syncArchiveStep = () => {
      ...
      const horizontalPadding =
        ...
        (Number.parseFloat(laneStyle.paddingRight) || 0);
    ...
    const observer =
      typeof ResizeObserver === "undefined"
        ? null
        : new ResizeObserver(syncArchiveStep);
  ...
  if (collapsed) {
    return (
      <span
        className="laneCollapsedPlaceholder archiveCollapsedPlaceholder"
        style={archiveCardsStyle}
  ...
  if (faceupCards.length === 0 && facedownCount === 0) {
    return (
      <span
        className="laneEmptyPlaceholder archiveEmptyPlaceholder"
        style={archiveCardsStyle}
      >
        {resolvedEmptyLabel}
      </span>
  ...
  return (
    <div
      ref={stackRef}
      className="archivesDualStack"
      style={archiveCardsStyle}
      data-testid="archives-dual-stack"
    >
        ...
        <div className="archivesPile">
          <div className="archivesPileBody">
            <div className="archivesOverlapRow" style={faceupRowStyle}>
              ...
              {faceupOverflow > 0 ? (
                <span className="archivesOverflowBadge">+{faceupOverflow}</span>
              ) : null}
            </div>
          </div>
        </div>
      ...
      {hasSeparateFacedownToggle ? (
        <div className="archivesToggleColumn">
          <button
            ...
            onClick={() =>
              setCorpArchivesFacedownView((current) =>
                current === "details" ? "backs" : "details",
              )
            }
            title={t(showCorpFacedownBacks ? "showDetails" : "showBacks")}
          >
            {showCorpFacedownBacks ? (
              <Eye size={12} strokeWidth={2.4} />
            ) : (
              <Image size={12} strokeWidth={2.4} />
            )}
          </button>
        </div>
      ) : null}
        ...
        <div className="archivesPile archivesFacedownPile">
          ...
          facedownCards.length > 0 ? (
            <div className="archivesInlineToggle">
              <button
                ...
                type="button"
                aria-label={t(
                  showCorpFacedownBacks ? "showDetails" : "showBacks",
                )}
                aria-pressed={showCorpFacedownBacks}
                onClick={() =>
                  setCorpArchivesFacedownView((current) =>
          ...
          <div className="archivesPileBody">
            <div className="archivesOverlapRow" style={facedownRowStyle}>
              {viewerSide === "corp"
                ? shownFacedownCards.map((card) => {
                    ...
                    return (
                      <CardView
                        ...
                        archiveFacedown
                        {...(!showCorpFacedownBacks
                          ? { inactiveZone: "archives" as const }
                          : {})}
                        {...(showCorpFacedownBacks
                          ? { forceCardBack: "corp" as Side }
                          : {})}
                        selected={
                          selectedContext?.kind === "card" &&
                          selectedContext.id === card.instanceId
                ...
                : Array.from({ length: shownFacedownCount }, (_, index) => (
                    <CardView
                      ...
                      card={{
                        instanceId: `archives-facedown-${index}`,
                        known: false,
                        rezzed: false,
                      }}
                      compact
              ...
              {facedownOverflow > 0 ? (
                <span className="archivesOverflowBadge">
                  +{facedownOverflow}
                </span>
              ) : null}
            </div>
