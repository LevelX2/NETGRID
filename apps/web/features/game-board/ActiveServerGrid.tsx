...
import { ServerCounterStrip, ServerStatusStrip } from "./CounterStrips";
import {
  HandCardsRow,
  ZoneCollapseButton,
  ZoneIdentityIcon,
...
export function ActiveServerGrid({
  ...
  exposedCardHighlightIds: ReadonlySet<string>;
        ...
        row.servers.length > 0 ? (
          ...
          >
            {row.servers.map((server) => {
              ...
              const isOpponentCorpHq =
                view.side === "runner" && server.id === "hq";
              const isCorpHqComposite = isOwnCorpHq || isOpponentCorpHq;
              const opponentCorpHqCount = isOpponentCorpHq
                ? Math.max(0, Math.floor(view.opponent.handCount))
                : 0;
              const opponentCorpHqPreviewCount = Math.min(
                opponentCorpHqCount,
                CORP_OPPONENT_HQ_PREVIEW_LIMIT,
              );
              const opponentCorpHqPreviewCards = Array.from(
                { length: opponentCorpHqPreviewCount },
                (_, index): DisplayVisibleCard => ({
              ...
              const serverCollapsed = boardZoneCollapsedFor(
                `corp:${server.id}`,
              );
              const renderLaneCards = (lane: {
                kind: "ice" | "root";
                label: "ICE" | "Root";
                cards: VisibleCard[];
              }) => {
                if (server.id === "archives" && lane.kind === "root") {
                  return (
                    <ArchivesDualStackLane
                      ...
                      totalArchivesCount={
                        view.side === "runner"
                          ? (view.opponent.discardCount ?? lane.cards.length)
                          : lane.cards.length
                      }
                ...
                return lane.cards.map((card, index) => {
                  ...
                  return (
                    <CardView
                      ...
                      displayMode={cardDisplayMode}
                      hiddenSide="corp"
                      installedCorpCard={showInstalledCorpState(
                        server.id,
                        lane.kind,
                      )}
                      selected={
                      ...
                      slotClassName={[
                        ...(lane.kind === "ice"
                          ? [iceStackSlotClass(card)]
                          : []),
                        ...(exposedCardHighlightIds.has(card.instanceId)
                          ? ["exposedCardHighlightSlot"]
                          : []),
                      ]
                        .filter(Boolean)
                        .join(" ")}
                      ...
                      viewMarkerActive={
                        (lane.kind === "ice" &&
                          viewedApproachIceId === card.instanceId) ||
                  ...
                  <div className="serverLayout">
                    <div className="serverLead">
                      <div className="serverLeadTop">
                        ...
                        {countLabel !== null ? (
                          <span
                            className={`serverCount serverCountSideLabel ${zoneSideClass("corp")}`}
                          >
                            {countLabel}
                          </span>
                        ) : null}
                      ...
                      <div className="serverLeadBottom">
                        <ZoneIdentityIcon
                          side="corp"
                          kind={serverZoneIdentityIconKind(server.id)}
                          label={serverDisplayLabel(server.id)}
                        />
                        <ZoneCollapseButton
                          side="corp"
                          ...
                          onToggle={() =>
                            toggleBoardZoneCollapsed(`corp:${server.id}`)
                          }
                        />
                      </div>
                    </div>
                          ...
                          {isOwnCorpHq ? (
                            <>
                                      ...
                                      <CardView
                                        ...
                                        {...(discardOption
                                          ? {
                                              discardShortcut: {
                                                selected:
                                                  selectedDiscardOptionIdSet.has(
                                                    discardOption.id,
                                                  ),
                                                disabled: actionDisabled,
                                                onToggle: () =>
                              ...
                              <ServerLanes
                                ...
                                renderLaneCards={renderLaneCards}
                                wrapForCorpHq
                              />
                            </>
                          ) : isOpponentCorpHq ? (
                            <>
                              <div
                                className={`corpHqHandPanel corpOpponentHqHandPanel ${zoneHighlighted(activeHighlight, view.side, "hq") ? "cueHighlightSoft" : ""}`}
                              >
                                {opponentCorpHqPreviewCards.length > 0 ? (
                                  <div
                                    ...
                                    style={
                                      {
                                        ...zoneCardsStyle,
                                        "--corp-hq-visible-steps": String(
                                          Math.max(
                                            0,
                                            opponentCorpHqPreviewCards.length -
                                              1,
                                          ),
                                        ),
                                      } as CSSProperties
...
function ServerLanes({
  ...
  return (
    <div className="pairedServerLanes corpHqServerLanes">{laneGroups}</div>
  );
}
