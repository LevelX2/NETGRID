...
export type CatalogRarityFilterKey =
  ...
  | "rare"
  | "vital";
export type CatalogAiHintFilterKey =
  | "all"
  | "new_facts"
  | "warnings"
  | "missing";
...
export const PRIMARY_CATALOG_STATUS_KEYS: CatalogStatusKey[] = [
  "human_playable",
  "deck_legal",
  "format_legal",
  "ai_supported",
...
export const TECHNICAL_CATALOG_STATUS_KEYS: CatalogStatusKey[] = [
  "imported",
  "validated",
  "catalog_ready",
  "implemented",
...
export const CATALOG_STATUS_FILTER_KEYS: CatalogStatusKey[] = [
  ...PRIMARY_CATALOG_STATUS_KEYS,
  ...TECHNICAL_CATALOG_STATUS_KEYS,
];

export const CATALOG_RARITY_FILTERS: Array<{
  key: CatalogRarityFilterKey;
  label: string;
}> = [
...
export const CATALOG_AI_HINT_FILTERS: Array<{
  key: CatalogAiHintFilterKey;
  label: string;
}> = [
...
export const CATALOG_BLOCK_STATUS_FILTERS: Array<{
  key: CatalogBlockStatusFilterKey;
  label: string;
}> = [
...
const CATALOG_RARITY_LABELS_DE: Record<
  Exclude<CatalogRarityFilterKey, "all">,
  string
> = {
  common: "Häufig",
...
export function catalogTypeKeysForCard(
  card: Pick<CatalogCardForTypeFilter, "type" | "subtypes">,
): CatalogTypeFilterKey[] {
...
export function catalogCardMatchesTypeFilters(
  card: CatalogCardForTypeFilter,
  filters: CatalogTypeFilterState,
): boolean {
...
export function filterCatalogCardsByType<T extends CatalogCardForTypeFilter>(
  cards: T[],
  filters: CatalogTypeFilterState,
): T[] {
...
export function nextCatalogSelection<T extends CatalogCardForTypeFilter>(
  current: string | null,
  cards: T[],
  filters: CatalogTypeFilterState,
): string | null {
  const filteredCards = filterCatalogCardsByType(cards, filters);
  if (current && filteredCards.some((card) => card.catalogCardId === current))
    return current;
  return filteredCards[0]?.catalogCardId ?? null;
}

export function summarizeCatalogTypeFilters(
  cards: CatalogCardForTypeFilter[],
): Partial<Record<CatalogTypeFilterKey, number>> {
...
export function isCatalogVisibleCard(
  card: Pick<CatalogCardForStatusSummary, "type">,
): boolean {
  return card.type !== "identity";
}

export function summarizeCatalogStatuses(
  cards: CatalogCardForStatusSummary[],
...
export function catalogSetKeyForCard(
  card: CatalogCardForSetFilter,
): Exclude<CatalogSetFilterKey, "all"> {
  const setId = card.setId.toLowerCase();
  if (ORIGINAL_SET_PREFIXES.some((prefix) => setId.startsWith(prefix)))
    return "original";
  if (TEST_SET_PREFIXES.some((prefix) => setId.startsWith(prefix)))
    return "test";
  return "other";
}

export function catalogCardMatchesSetFilter(
  card: CatalogCardForSetFilter,
  filter: CatalogSetFilterKey,
): boolean {
  return filter === "all" || catalogSetKeyForCard(card) === filter;
}

export function filterCatalogCardsBySet<T extends CatalogCardForSetFilter>(
  cards: T[],
  filter: CatalogSetFilterKey,
): T[] {
...
export function summarizeCatalogSetFilters(
  cards: CatalogCardForSetFilter[],
): Record<CatalogSetFilterKey, number> {
  const counts: Record<CatalogSetFilterKey, number> = {
    all: cards.length,
    original: 0,
    test: 0,
    other: 0,
...
export function catalogSetLabelForSetId(setId: string): string {
  ...
  if (CATALOG_SET_LABELS[normalizedSetId])
    return CATALOG_SET_LABELS[normalizedSetId];
  if (
    normalizedSetId.startsWith("originalset-") ||
    normalizedSetId.startsWith("onr-v1")
  )
  ...
  if (
    normalizedSetId.startsWith("testset-") ||
    normalizedSetId.startsWith("mvp-")
  )
    return "Testkarten";
  if (
    normalizedSetId.includes("android") ||
    normalizedSetId.includes("netrunner")
  )
    return "Android: Netrunner";
  return setId
    .split(/[-_]+/)
    .filter(Boolean)
...
export function catalogSetShortLabelForSetId(
...
export function catalogSetDetailLabel(
  card: CatalogCardForSetDisplay,
): string | null {
...
export function catalogSetFilterOptions(
  ...
  const options = [...counts.entries()]
    .map(([key, count]) => ({
      key,
      label: catalogSetLabelForSetId(key),
      count,
    }))
    .sort(
      (left, right) =>
...
export function filterCatalogCardsBySetId<T extends CatalogCardForSetFilter>(
  cards: T[],
  setId: string,
): T[] {
...
export function catalogRarityLabel(
  card: CatalogCardForRarityFilter,
): string | null {
...
export function catalogCardMatchesRarityFilter(
  card: CatalogCardForRarityFilter,
  filter: CatalogRarityFilterKey,
): boolean {
  return filter === "all" || card.rarity?.code === filter;
}

export function filterCatalogCardsByRarity<
  T extends CatalogCardForRarityFilter,
>(cards: T[], filter: CatalogRarityFilterKey): T[] {
  return cards.filter((card) => catalogCardMatchesRarityFilter(card, filter));
}

export function summarizeCatalogRarityFilters(
  cards: CatalogCardForRarityFilter[],
): Record<CatalogRarityFilterKey, number> {
  const counts: Record<CatalogRarityFilterKey, number> = {
    all: cards.length,
    common: 0,
    uncommon: 0,
    rare: 0,
...
export function catalogCardMatchesAiHintFilter(
  card: CatalogCardForAiHintFilter,
  filter: CatalogAiHintFilterKey,
): boolean {
...
export function filterCatalogCardsByAiHint<
  T extends CatalogCardForAiHintFilter,
>(cards: T[], filter: CatalogAiHintFilterKey): T[] {
  return cards.filter((card) => catalogCardMatchesAiHintFilter(card, filter));
}

export function summarizeCatalogAiHintFilters(
  cards: CatalogCardForAiHintFilter[],
): Record<CatalogAiHintFilterKey, number> {
  const counts: Record<CatalogAiHintFilterKey, number> = {
    all: cards.length,
    new_facts: 0,
    warnings: 0,
    missing: 0,
  ...
  for (const card of cards) {
    if (catalogCardMatchesAiHintFilter(card, "new_facts"))
      counts.new_facts += 1;
...
export function catalogCardMatchesBlockStatusFilter(
  card: CatalogCardForBlockStatusFilter,
  filter: CatalogBlockStatusFilterKey,
): boolean {
  switch (filter) {
    case "all":
      return true;
...
export function filterCatalogCardsByBlockStatus<
  T extends CatalogCardForBlockStatusFilter,
>(cards: T[], filter: CatalogBlockStatusFilterKey): T[] {
  return cards.filter((card) =>
    catalogCardMatchesBlockStatusFilter(card, filter),
  );
}

export function summarizeCatalogBlockStatusFilters(
  cards: CatalogCardForBlockStatusFilter[],
): Record<CatalogBlockStatusFilterKey, number> {
  const counts: Record<CatalogBlockStatusFilterKey, number> = {
    all: cards.length,
    not_blocked: 0,
    blocked: 0,
  };
...
function isCatalogRarityCode(
  value: string | undefined,
): value is Exclude<CatalogRarityFilterKey, "all"> {
  return (
    value === "common" ||
    value === "uncommon" ||
    value === "rare" ||
