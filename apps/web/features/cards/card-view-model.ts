...
export function enrichVisibleCard(
  card: VisibleCard,
  detailsById: Record<string, CardViewCatalogDetail>,
): DisplayVisibleCard {
  ...
  addNumeric(
    ...
    detail.numeric.advancementRequirement,
  );
  addNumeric(
    enriched,
    "agendaPoints",
    card.agendaPoints,
    detail.numeric.agendaPoints,
  );
  if (
    typeof card.strength === "number" &&
    typeof detail.numeric.strength === "number" &&
    card.strength !== detail.numeric.strength
  ) {
...
export function visibleCardFromCatalogDetail(
  card: CardViewCatalogDetail,
): DisplayVisibleCard {
  ...
  addNumeric(
    ...
    "advancementRequirement",
    undefined,
    card.numeric.advancementRequirement,
  );
  addNumeric(visible, "agendaPoints", undefined, card.numeric.agendaPoints);
  return visible;
}

export function visibleCardFromPublicEvent(
...
function addCatalogSetDisplay(
  target: DisplayVisibleCard,
  detail: Pick<CardViewCatalogDetail, "setId" | "setName" | "collectorNumber">,
): void {
...
function addPrintedStrength(
  target: DisplayVisibleCard,
  printedStrength: number | null | undefined,
): void {
...
function addNumeric(
  ...
  if (current !== undefined || fallback === null || fallback === undefined)
    return;
  target[key] = fallback;
}
