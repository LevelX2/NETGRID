...
export function DeckTableBoard({
  ...
  const totalCards = layout.piles.reduce(
    (sum, pile) =>
      sum +
      pile.entries.reduce((entrySum, entry) => entrySum + entry.quantity, 0),
    0,
  );
  return (
    <section className="deckTableBoardPanel">
      <div className="deckBuilderPanelHeader deckTableBoardHeader">
        ...
        <div className="deckTableBoardTools">
          <button
            className={`button deckTableSelectionButton ${selectionMode ? "active" : ""}`}
            type="button"
          ...
          {selectionMode ? (
            <>
              <span className="deckTableSelectionCount">
                {t("selected", { count: selectedCards.length })}
              </span>
              {selectedCards.length > 0 ? (
                <button
                  className="button deckTableSelectionClear"
                  type="button"
          ...
          <span className={`deckTableSaveState ${dirty ? "dirty" : "ok"}`}>
            {dirty ? t("unsaved") : t("saved")}
          </span>
          <button
            className="button primary deckTableSaveButton"
            type="button"
            onClick={onSave}
            disabled={!dirty}
          >
          ...
          <button
            className={`button deckTableViewButton ${controlsOpen ? "active" : ""}`}
            type="button"
      ...
      {controlsOpen ? (
        <div className="deckTableControls" aria-label={t("tableView")}>
          <label>
            ...
            <input
              min={DECK_TABLE_LIBRARY_WIDTH_MIN}
              max={DECK_TABLE_LIBRARY_WIDTH_MAX}
              step={DECK_TABLE_LIBRARY_WIDTH_STEP}
              type="range"
              value={tableWidth}
              onChange={(event) => onSetTableWidth(Number(event.target.value))}
            />
          </label>
          <label>
            <span>{t("cardSize")}</span>
            <input
              min={DECK_TABLE_CARD_WIDTH_MIN}
              max={DECK_TABLE_CARD_WIDTH_MAX}
              step={DECK_TABLE_CARD_WIDTH_STEP}
              type="range"
              value={cardWidth}
              onChange={(event) => onSetCardWidth(Number(event.target.value))}
            />
          </label>
          <label>
            <span>{t("overlap")}</span>
            <input
              min={DECK_TABLE_OVERLAP_MIN}
              max={DECK_TABLE_OVERLAP_MAX}
              step={DECK_TABLE_OVERLAP_STEP}
              type="range"
              value={overlapPercent}
              onChange={(event) =>
                onSetOverlapPercent(Number(event.target.value))
              }
            />
          </label>
          <label>
            <span>{t("piles")}</span>
            <input
              min={MIN_DECK_TABLE_PILE_COUNT}
              max={MAX_DECK_TABLE_PILE_COUNT}
              step={1}
              type="range"
              value={layout.piles.length}
              onChange={(event) => onSetPileCount(Number(event.target.value))}
            />
            <b>{layout.piles.length}</b>
          </label>
          <label
            className={`deckBuilderToggle deckTableNameToggle ${layout.showPileNames ? "checked" : ""}`}
          >
            <input
              checked={layout.showPileNames}
              onChange={(event) => onShowPileNames(event.target.checked)}
              type="checkbox"
            />
            {t("pileNames")}
          </label>
          <label>
            ...
            >
              ...
              {deckTableSortKeysForSide(deckSide).map((sortBy) => (
                <option
                  disabled={
                    deckTableSortRequiresDetails(sortBy) && !numericDetailsReady
                  }
                  key={sortBy}
                  value={sortBy}
                >
            ...
            {!numericDetailsReady ? (
              <small className="deckTableSortStatus" role="status">
                {t("numericSortHelp")}
              </small>
            ) : null}
          </label>
...
function DeckTablePileView({
  ...
  const t = useTranslations("Decks.table");
  const cardCount = pile.entries.reduce(
    (sum, entry) => sum + entry.quantity,
    0,
  );
  const pileSelectionKeys = pile.entries.map((entry) =>
    deckTableSelectionKey(pile.id, entry.cardId, entry.order),
  );
  const pileSelected =
    pileSelectionKeys.length > 0 &&
    pileSelectionKeys.every((key) => selectedCardKeys.has(key));
  const numericDetailsReady = pile.entries.every((entry) =>
    Boolean(cardDetailsById[entry.cardId]),
  );
  const numericSortWaiting =
    deckTableSortRequiresDetails(pile.sortMode) && !numericDetailsReady;
  ...
  return (
    <section
      ...
      aria-label={t("pileAria", {
        name: pile.name || t("pile"),
        count: cardCount,
      })}
    >
      <div className="deckTablePileHeader">
        <div className="deckTablePileTitleRow">
          <button
            aria-label={t("movePile", {
              name: pile.name || t("pileNumber", { number: pile.order + 1 }),
            })}
          ...
          {showName ? (
            <input
              value={pile.name ?? ""}
              onChange={(event) => onRenamePile(pile.id, event.target.value)}
              aria-label={t("pileName")}
            />
          ) : (
          ...
          {selectionMode ? (
            <button
              className={`deckTableSelectPileButton ${pileSelected ? "active" : ""}`}
      ...
      <div className="deckTablePileCards">
        {pile.entries.map((entry, index) => {
          ...
          const numericSortValue = deckTableNumericSortValue(
            pile.sortMode,
            detail,
          );
          const menuKey = `${pile.id}:${entry.cardId}:${entry.order}`;
          const selectionKey = deckTableSelectionKey(
            pile.id,
            entry.cardId,
            entry.order,
          );
          ...
          const dragCards =
            selected && selectedCards.length > 0
              ? selectedCards
              : [{ pileId: pile.id, cardId: entry.cardId, order: entry.order }];
          return (
            ...
            >
              ...
              >
                ...
                <span className="deckTableCardCaption">
                  {card?.title ?? entry.cardId}
                </span>
                {numericSortValue !== null ? (
                  <span
                    aria-label={`${t(`sort.${pile.sortMode}`)}: ${numericSortValue}`}
                    className="deckTableSortValue"
                  >
                    {numericSortValue}
                  </span>
                ) : null}
                {selectedIndex ? (
                  <span className="deckTableSelectionBadge">
                    {selectedIndex}
                  </span>
                ) : null}
                {activeMenuKey === menuKey ? (
                  <span
                    className="deckTableCardMenu"
                    onClick={(event) => event.stopPropagation()}
                  >
                    <button
                      type="button"
                      onClick={() =>
                        onDuplicateCard(pile.id, entry.cardId, entry.order, 1)
                      }
                    >
                      {t("copy")}
                    </button>
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        onRemoveCard(pile.id, entry.cardId, entry.order)
                      }
                    >
                      {t("remove")}
                    </button>
        ...
        {pile.entries.length === 0 ? (
          <p className="meta deckTablePileEmpty">{t("dropCard")}</p>
        ) : null}
      </div>
    </section>
  );
