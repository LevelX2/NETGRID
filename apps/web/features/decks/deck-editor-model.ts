...
export function deckAgendaStatusForEditor(
  ...
  const rule =
    DECK_AGENDA_RULES_BY_PROFILE[deck.formatProfileId] ??
    DEFAULT_DECK_AGENDA_RULE;
  const totalCards = deck.cards.reduce(
    (sum, entry) => sum + safeQuantity(entry.quantity),
    0,
  );
  ...
  const agendaPoints = deck.cards.reduce((sum, entry) => {
    ...
    return (
      sum + (detail.numeric.agendaPoints ?? 0) * safeQuantity(entry.quantity)
    );
  }, 0);
  const effectiveCardsForMinimum = Math.max(totalCards, rule.minimumDeckCards);
  const additionalAgendaPoints = Math.floor(
    Math.max(0, effectiveCardsForMinimum - rule.minimumDeckCards) /
      rule.additionalAgendaPointEveryCards,
  );
  const minimumAgendaPoints = rule.minimumAgendaPoints + additionalAgendaPoints;
  return {
    agendaPoints: detailsComplete ? agendaPoints : null,
    minimumAgendaPoints,
    missingAgendaPoints: detailsComplete
      ? Math.max(0, minimumAgendaPoints - agendaPoints)
      : null,
    totalCards,
