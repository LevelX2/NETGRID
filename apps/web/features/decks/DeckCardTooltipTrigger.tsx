...
export function DeckCardTooltipTrigger({
  ...
  const tooltipImageUrl = tooltipImageUnavailable
    ? undefined
    : tooltipImageSource.src;
  ...
  const hasTooltipTextContent = Boolean(
    card && (card.title || detailLines.length > 0 || hasRulesLines),
  );
  const tooltipEnabled =
    Boolean(card) && (showImageTooltip || hasTooltipTextContent);
  const tooltipId =
    tooltipEnabled && card
      ? `deck-card-tooltip-${card.catalogCardId.replace(/[^A-Za-z0-9_-]/g, "-")}`
      : undefined;
  ...
  const tooltipStats = detail
    ? [
        detail.numeric.cost !== null && detail.numeric.cost !== undefined
          ? { icon: "¢", label: t("cost"), value: String(detail.numeric.cost) }
          : null,
        ...
        detail.numeric.installCost !== undefined
          ? {
              icon: "↓",
              label: t("install"),
              value: String(detail.numeric.installCost),
            }
          : null,
        detail.numeric.rezCost !== null && detail.numeric.rezCost !== undefined
          ? {
              icon: "R",
              label: t("rez"),
              value: String(detail.numeric.rezCost),
            }
          : null,
        detail.numeric.trashCost !== null &&
        detail.numeric.trashCost !== undefined
          ? {
              icon: "🗑",
              label: t("trash"),
              value: String(detail.numeric.trashCost),
            }
          : null,
        detail.numeric.strength !== null &&
        detail.numeric.strength !== undefined
          ? {
              icon: "⚔",
              label: t("strength"),
              value: String(detail.numeric.strength),
            }
          : null,
        detail.numeric.memoryCost !== null &&
        detail.numeric.memoryCost !== undefined
          ? {
              icon: "MU",
              label: t("memory"),
              value: String(detail.numeric.memoryCost),
            }
          : null,
        detail.numeric.advancementRequirement !== null &&
        detail.numeric.advancementRequirement !== undefined
          ? {
              icon: "⟐",
              label: t("required"),
              value: String(detail.numeric.advancementRequirement),
            }
          : null,
      ].filter(
  ...
  const showHardwareOverlay =
    Boolean(tooltipImageUrl) &&
    card?.type === "hardware" &&
    Boolean(overlayImageId) &&
    hasGeneratedCardArt(overlayImageId);
            ...
            <>
              ...
              {effectiveTooltipMode === "enhanced" ? (
                <span className="cardTooltipStats">
                  {tooltipStats.map((stat) => (
                    <span
                      key={`${card.catalogCardId}-tooltip-stat-${stat.label}`}
              ...
              {effectiveTooltipMode === "enhanced"
                ? detailLines.map((line) => (
                    <span key={`${card.catalogCardId}-tooltip-detail-${line}`}>
                      {line}
                    </span>
                  ))
                : null}
              <span className="cardTooltipText">
                {rulesTextLines(rulesText).map((line, index) => (
                  <span
                    key={`${card.catalogCardId}-tooltip-rules-${index}`}
                    className={
                  ...
                  >
                    ...
                    {renderRuleTextSegments(
                      line,
                      `${card.catalogCardId}-tooltip-rules-${index}`,
                    )}
                  </span>
                ))}
              </span>
...
function catalogDetailLines(card: DeckTooltipDetail): string[] {
  const typeLine = [card.side, formatCardTypeLine(card)]
    .filter(Boolean)
    .join(" · ");
...
function deckBuilderCardTooltip(
  card: DeckTooltipCard,
  detail: DeckTooltipDetail | undefined,
): string {
  return [
    card.title,
    formatCardTypeLine(card),
    detail ? catalogSetDetailLabel(detail) : "",
    cardMetricLine(detail),
    detail?.text ?? "",
  ]
    .filter(Boolean)
    .join("\n");
