...
function deckBuilderCardGroup(card: CatalogCardSummary | null): string {
  ...
  if (card.side === "runner") {
    if (
      card.type === "program" &&
      card.subtypes.some((subtype) => subtype.toLowerCase() === "icebreaker")
    )
      return "Icebrecher";
...
export function normalizeDeckTableViewSettings(
  value: unknown,
): DeckTableViewSettings {
  const candidate =
  ...
  return {
    ...
    libraryOverlapPercent: normalizeSteppedNumber(
      candidate.libraryOverlapPercent,
...
export function parseDeckTableViewSettings(
  raw: string | null,
): DeckTableViewSettings {
...
export function deckFingerprint(deck: EditableDeck): string {
  return JSON.stringify({
    ...
    cards: [...deck.cards].sort((left, right) =>
...
export function defaultDeckTablePileName(side: Side, index: number): string {
  ...
  return (
    (side === "runner" ? runnerNames : corpNames)[index] ??
    `Stapel ${index + 1}`
  );
}

function defaultDeckTablePileIndexForCard(
  card: CatalogCardSummary | undefined,
): number {
  if (!card) return 0;
...
export function normalizeDeckTablePileSortMode(
  value: unknown,
): DeckTablePileSortMode {
  return value === "name" ||
    value === "type" ||
    value === "install" ||
    value === "rez" ||
...
export function deckTableSortRequiresDetails(
  sortMode: DeckTablePileSortMode,
): boolean {
  return sortMode !== "free" && deckTableNumericKey(sortMode) !== null;
}

export function normalizeDeckTableLayout(
  deck: EditableDeck,
  ...
  const pileCount = Math.min(
    ...
    Math.max(
      ...
      deck.tableLayout?.piles.length ?? DEFAULT_DECK_TABLE_PILE_COUNT,
    ),
  );
  const normalizedPiles: DeckTablePile[] = Array.from(
    { length: pileCount },
    (_, index) => {
      ...
      return {
        ...
        name:
          typeof source?.name === "string"
            ...
            : defaultDeckTablePileName(deck.side, index),
        order: index,
        sortMode: normalizeDeckTableSortModeForSide(
          source?.sortMode,
          deck.side,
        ),
        entries: [],
  ...
  for (let pileIndex = 0; pileIndex < normalizedPiles.length; pileIndex += 1) {
    ...
    for (const entry of entries) {
      ...
      for (let copy = 0; copy < quantity; copy += 1) {
        normalizedPiles[pileIndex]!.entries.push({
          cardId: entry.cardId,
          quantity: 1,
          order: normalizedPiles[pileIndex]!.entries.length,
        });
      }
  ...
  for (const [cardId, quantity] of remaining) {
    if (quantity <= 0) continue;
    const pileIndex = Math.min(
      normalizedPiles.length - 1,
      Math.max(0, defaultDeckTablePileIndexForCard(cardLookup?.get(cardId))),
    );
    const fallbackPile = normalizedPiles[pileIndex]!;
    for (let copy = 0; copy < quantity; copy += 1) {
      fallbackPile.entries.push({
        cardId,
        quantity: 1,
        order: fallbackPile.entries.length,
      });
    }
  }
  return {
    ...
    piles: normalizedPiles.map((pile) =>
      applyDeckTablePileSort(
        pile,
        cardLookup ?? new Map<string, CatalogCardSummary>(),
        detailsById,
      ),
    ),
  };
...
export function deckCardsFromTableLayout(
...
export function deckTableCardTotal(
  layout: DeckTableLayout,
  cardId: string,
): number {
  return layout.piles.reduce(
    (sum, pile) =>
      sum +
      pile.entries
        .filter((entry) => entry.cardId === cardId)
        .reduce((pileSum, entry) => pileSum + entry.quantity, 0),
    0,
  );
}

export function reorderDeckTableEntries(
  entries: DeckTableLayoutEntry[],
): DeckTableLayoutEntry[] {
  return entries.map((entry, order) => ({ ...entry, order }));
}

export function insertDeckTableEntry(
  entries: DeckTableLayoutEntry[],
  ...
  if (targetOrder === undefined)
    return reorderDeckTableEntries([
      ...orderedEntries,
      { ...entry, order: orderedEntries.length },
    ]);
  const targetIndex = orderedEntries.findIndex(
    (candidate) => candidate.order === targetOrder,
  );
  if (targetIndex < 0)
    return reorderDeckTableEntries([
      ...orderedEntries,
  ...
  return reorderDeckTableEntries([
    ...orderedEntries.slice(0, targetIndex),
    { ...entry, order: targetIndex },
    ...orderedEntries.slice(targetIndex),
  ]);
}

export function insertDeckTableEntries(
  entries: DeckTableLayoutEntry[],
  insertedEntries: DeckTableLayoutEntry[],
  targetOrder?: number,
): DeckTableLayoutEntry[] {
  const orderedEntries = [...entries].sort(
    (left, right) => left.order - right.order,
  );
  const normalizedInserted = insertedEntries.map((entry, offset) => ({
    ...entry,
    order: offset,
  }));
  if (targetOrder === undefined)
    return reorderDeckTableEntries([...orderedEntries, ...normalizedInserted]);
  const targetIndex = orderedEntries.findIndex(
    (candidate) => candidate.order === targetOrder,
  );
  if (targetIndex < 0)
...
export function deckTableSelectionKey(
  pileId: string,
  cardId: string,
  order: number,
): string {
...
export function deckTableNumericSortValue(
  sortMode: DeckTablePileSortMode,
...
function sortDeckTableEntries(
  entries: DeckTableLayoutEntry[],
  cardLookup: Map<string, CatalogCardSummary>,
  detailsById: Record<string, CatalogCardDetail>,
  sortBy: DeckTableSortKey,
): DeckTableLayoutEntry[] {
  ...
  const titleValue = (entry: DeckTableLayoutEntry): string =>
    cardLookup.get(entry.cardId)?.title ?? entry.cardId;
  const typeValue = (entry: DeckTableLayoutEntry): string =>
    deckBuilderCardGroup(cardLookup.get(entry.cardId) ?? null);
  const numericKey = deckTableNumericKey(sortBy);
  return reorderDeckTableEntries(
    [...entries].sort((left, right) => {
      if (numericKey)
        return (
          numericValue(left, numericKey) - numericValue(right, numericKey) ||
          typeValue(left).localeCompare(typeValue(right)) ||
...
export function deckTablePileSortModeLabel(
  sortMode: DeckTablePileSortMode,
): string {
  return sortMode === "free" ? "Frei" : deckTableSortLabel(sortMode);
}

export function applyDeckTablePileSort(
  pile: DeckTablePile,
  cardLookup: Map<string, CatalogCardSummary>,
  detailsById: Record<string, CatalogCardDetail>,
): DeckTablePile {
  if (pile.sortMode === "free")
    return { ...pile, entries: reorderDeckTableEntries(pile.entries) };
  return {
    ...pile,
    entries: sortDeckTableEntries(
      pile.entries,
      cardLookup,
      detailsById,
      pile.sortMode,
...
function normalizeDeckTableSortModeForSide(
  value: unknown,
  side: Side,
): DeckTablePileSortMode {
...
function deckTableEntryNumericValue(
  entry: DeckTableLayoutEntry,
  detailsById: Record<string, CatalogCardDetail>,
  key: string,
): number | null {
...
function deckTableEntryBuildCost(
  entry: DeckTableLayoutEntry,
  detailsById: Record<string, CatalogCardDetail>,
): number | null {
  const detail = detailsById[entry.cardId];
  if (!detail) return null;
  if (detail.type === "agenda")
    return detail.numeric.advancementRequirement ?? null;
  return (
    detail.numeric.installCost ??
    detail.numeric.rezCost ??
    detail.numeric.cost ??
    null
  );
}

function deckTableBuildCostGroupName(
  key: string,
  entries: DeckTableLayoutEntry[],
  cardLookup: Map<string, CatalogCardSummary>,
): string {
  ...
  const hasAgenda = entries.some(
    (entry) => cardLookup.get(entry.cardId)?.type === "agenda",
  );
  const hasNonAgenda = entries.some(
    (entry) => cardLookup.get(entry.cardId)?.type !== "agenda",
  );
...
function deckTableTypeGroupOrder(side: Side, label: string): number {
  const labels =
    side === "corp"
      ? ["ICE", "Agenda", "Asset", "Operation", "Upgrade"]
      : ["Prep", "Hardware", "Ressource", "Programm", "Icebrecher"];
  const index = labels.indexOf(label);
  return index >= 0 ? index : labels.length;
}

export function distributeDeckTableByType(
  layout: DeckTableLayout,
  side: Side,
  cardLookup: Map<string, CatalogCardSummary>,
  detailsById: Record<string, CatalogCardDetail>,
): DeckTableLayout {
  const allEntries = sortDeckTableEntries(
    layout.piles.flatMap((pile) => pile.entries),
    cardLookup,
    detailsById,
    "name",
  );
  ...
  const groups = [...grouped.entries()].sort(
    (left, right) =>
      deckTableTypeGroupOrder(side, left[0]) -
        deckTableTypeGroupOrder(side, right[0]) ||
      left[0].localeCompare(right[0]),
  );
  const groupCount = Math.min(
    MAX_DECK_TABLE_PILE_COUNT,
    Math.max(MIN_DECK_TABLE_PILE_COUNT, groups.length || 1),
  );
  return {
    ...layout,
    piles: Array.from({ length: groupCount }, (_, index) => {
      const [label, entries] = groups[index] ?? [
        defaultDeckTablePileName(side, index),
        [],
      ];
...
export function distributeDeckTableByInstallCost(
  layout: DeckTableLayout,
  side: Side,
  cardLookup: Map<string, CatalogCardSummary>,
  detailsById: Record<string, CatalogCardDetail>,
  ...
  const allEntries = sortDeckTableEntries(
    ...
    const rightType = deckBuilderCardGroup(
      cardLookup.get(right.cardId) ?? null,
    );
    return (
      normalizedLeft - normalizedRight ||
      leftType.localeCompare(rightType) ||
      (cardLookup.get(left.cardId)?.title ?? left.cardId).localeCompare(
        cardLookup.get(right.cardId)?.title ?? right.cardId,
      )
    );
  });
  const grouped = new Map<string, DeckTableLayoutEntry[]>();
  ...
  const groups = [...grouped.entries()].sort((left, right) => {
    const leftValue =
      left[0] === "none" ? Number.POSITIVE_INFINITY : Number(left[0]);
    const rightValue =
      right[0] === "none" ? Number.POSITIVE_INFINITY : Number(right[0]);
    return leftValue - rightValue;
  });
  const visibleGroups =
    groups.length > MAX_DECK_TABLE_PILE_COUNT
      ? [
          ...groups.slice(0, MAX_DECK_TABLE_PILE_COUNT - 1),
          [
            "overflow",
            groups
              .slice(MAX_DECK_TABLE_PILE_COUNT - 1)
              .flatMap(([, entries]) => entries),
          ] as [string, DeckTableLayoutEntry[]],
        ]
      : groups;
  const groupCount = Math.min(
    MAX_DECK_TABLE_PILE_COUNT,
    Math.max(MIN_DECK_TABLE_PILE_COUNT, visibleGroups.length || 1),
  );
  return {
    ...layout,
    piles: Array.from({ length: groupCount }, (_, index) => {
...
export function deckMetadataFromEditable(
  deck: EditableDeck | null,
): DeckPublicMetadata | undefined {
