...
export function DeckAgendaStatusBadge({ status }: { status: DeckAgendaStatus | null }) {
  ...
  const missingLabel = loading ? t("loading") : complete ? t("complete") : t("missing", {count: status.missingAgendaPoints ?? 0});
  return (
    <p
    ...
    >
      <AgendaIcon size={14} />
      <span>{loading ? t("minimum", {minimum: status.minimumAgendaPoints}) : t("points", {points: status.agendaPoints!, minimum: status.minimumAgendaPoints})}</span>
