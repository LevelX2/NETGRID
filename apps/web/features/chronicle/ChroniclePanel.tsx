...
export function chronicleContextByEventId(
  ...
  return Object.fromEntries(
    events.map((event) => {
      ...
      const cardTitle = options.preferGermanCardImages
        ? (localizedDeCardTitle(cardId) ?? card?.title ?? null)
        : (card?.title ?? null);
      const serverTurnNumber = payloadPositiveInteger(
        event.publicPayload,
        "chronicleTurnNumber",
      );
      return [
        event.eventId,
        {
          ...
          turnNumber:
            serverTurnNumber ??
            (tailLikelyTruncated ? null : turnNumberByEventId[event.eventId]) ??
            null,
          turnSide:
...
export function ChroniclePanel({
  ...
  return (
    <section
      className={`section chroniclePanel ${collapsed ? "collapsed" : ""}`}
        ...
        <div className="chronicleList">
          ...
          {groupedEntries.map((group) => {
            const groupKey = chronicleGroupCollapseKey(
              group.label,
              group.kind,
              group.turnGroupLabel,
              group.firstItemId,
            );
            const groupRenderKey = `${groupKey}:${group.firstItemId}`;
            const turnKey = group.turnGroupLabel
              ? chronicleTurnGroupCollapseKey(group.turnGroupLabel)
              : null;
            const isTurnGroup = Boolean(
              group.turnGroupLabel && group.label === group.turnGroupLabel,
            );
            const turnCollapsed = Boolean(
              turnKey && collapsedGroups.has(turnKey),
            );
            const groupCollapsed = collapsedGroups.has(groupKey);
            const hiddenByParent = group.kind === "run" && turnCollapsed;
            const entriesCollapsed = isTurnGroup
              ? groupCollapsed
              : groupCollapsed || turnCollapsed;
            if (hiddenByParent) return null;
            const shouldRenderGroup = chronicleGroupShouldRender(
              group.label,
              shownChronicleGroupLabels,
            );
            return (
              <div
                className={`chronicleGroupBlock group-${group.kind} ${entriesCollapsed ? "entriesCollapsed" : ""}`}
                key={groupRenderKey}
              >
                {shouldRenderGroup ? (
                  <button
...
function chronicleGroupCollapseKey(
  label: string,
  kind: ChronicleGroupKind,
  turnGroupLabel: string | null,
  firstItemId: string,
): string {
  if (turnGroupLabel && label === turnGroupLabel)
    return chronicleTurnGroupCollapseKey(label);
  return `${kind}:${turnGroupLabel ?? "none"}:${label}:${firstItemId}`;
}

function chronicleGroupShouldRender(
  label: string,
...
function chronicleEntriesWithRunGroups(
  ...
  for (const event of events) {
    ...
    if (
      runEndPending &&
      !chronicleActionContinuesCompletedRun(event, actionType)
    ) {
...
function chronicleEventBelongsToActiveRun(
  ...
  if (actionType === "end_turn" || actionType === "mandatory_draw")
    return false;
  if (
  ...
  if (
    ...
    return (
      ...
      items.some(
        (item) =>
          chronicleGroupLabel(item).startsWith("Run") ||
          item.category === "run",
      )
    );
  }
...
function chronicleActionContinuesCompletedRun(
  ...
  if (
    actionType === "trigger_ability" &&
    isISpySuccessfulRunFollowupPayload(event.publicPayload)
  )
    return true;
  return (
...
function chronicleActionCompletesRun(
  event: PublicGameEvent,
  actionType: string,
): boolean {
  if (
  ...
  if (actionType === "continue_run")
    return (
      ...
      event.publicPayload.rioRunEnded === true
    );
  return false;
}
