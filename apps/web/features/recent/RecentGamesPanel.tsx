  ...
  return (
    <article className="recentGameCard">
      <div className="recentGamePrimary">
        <div>
          ...
          <p className="recentGameMeta">
            ...
            {result.series
              ? ` · Spiel ${result.series.gameNumber}/${result.series.gamesPlanned}`
              : ""}
          </p>
        </div>
        <div
          className="recentGameScore"
          aria-label={`Endstand Runner ${result.runner.agendaPoints} zu Korp ${result.corp.agendaPoints}`}
        >
          <span>{scoreText}</span>
        ...
        <div
          ...
          aria-label={`Matchpunkte Runner ${runnerMatchPoints} zu Korp ${corpMatchPoints}`}
        >
          <span>
      ...
      <div className="recentGameDetails">
        ...
        <span title={resultReasonLabel(result.reason, result.winner)}>
          {shortResultReasonLabel(result.reason)}
        </span>
        <span>{result.actionCount} Aktionen</span>
        <span>{result.runCount} Runs</span>
        <span title={result.finalStateHash}>
          Hash {result.finalStateHash.slice(0, 8)}
        </span>
      </div>
      <div className="recentGameDecks">
        <span>
          {result.runner.deckName
            ? `Runner-Deck: ${result.runner.deckName}`
            : "Runner-Deck"}
        </span>
        <span>
          {result.corp.deckName
            ? `Korp-Deck: ${result.corp.deckName}`
            : "Korp-Deck"}
        </span>
        {result.isPublic ? (
          <>
  ...
  return (
    <article className="recentGameCard recentSeriesCard">
      <div className="recentGamePrimary">
        ...
        <div
          className="recentGameScore matchPoints"
          aria-label={`Serien-Matchpunkte Spieler A ${result.players.player_a.matchPoints} zu Spieler B ${result.players.player_b.matchPoints}`}
        >
          <span>
        ...
        <div
          className="recentGameScore"
          aria-label={`Serien-Agenda-Punkte Spieler A ${result.players.player_a.agendaPoints} zu Spieler B ${result.players.player_b.agendaPoints}`}
        >
          <span>
      ...
      <div className="recentGameDetails">
        ...
        <span>
          {result.players.player_a.wins} : {result.players.player_b.wins} Siege
        </span>
      ...
      <ol className="recentSeriesGames">
        {result.games.map((game) => (
          <li key={game.matchId}>
            ...
            <span>
              {game.runnerDisplayName} als Runner {game.runnerAgendaPoints} AP /{" "}
              {game.runnerMatchPoints} MP
            </span>
            <span>
              {game.corpDisplayName} als Korp {game.corpAgendaPoints} AP /{" "}
              {game.corpMatchPoints} MP
            </span>
...
function resultReasonLabel(
  ...
  if (reason === "agenda_points" && winner === "runner")
    return "Der Runner hat die Pläne der Korp vereitelt.";
  if (reason === "agenda_points" && winner === "corp")
    return "Die Korp hat ihre Agendas durchgesetzt.";
  if (reason === "agenda_points")
    return "Die entscheidenden Agenda-Punkte wurden erreicht.";
  if (reason === "bad_publicity_7")
    return "Die Korp hat 7 Bad Publicity erreicht.";
  if (reason === "corp_deck_empty")
    return "Die Korp konnte keine Karte mehr ziehen.";
  ...
  if (reason === "time_expired")
    return "Das Spiel wurde durch abgelaufene Spielerzeit beendet.";
