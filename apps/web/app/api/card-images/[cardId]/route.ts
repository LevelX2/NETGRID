...
import { ManagedCardImageRuntimeError } from "@netgrid/card-images/runtime";
import { NextResponse } from "next/server";
import {
  lookupCardImage,
  type CardImageLookupResult,
...
export async function GET(
  ...
  try {
    ...
    if (error instanceof ManagedCardImageRuntimeError) {
      if (error.code === "invalid_variant")
        return safeImageError("card_image_variant_invalid", error.message, 400);
      return safeImageError("personal_card_image_invalid", error.message, 404);
    }
    throw error;
  }
  if (!image)
    return safeImageError(
      "card_image_not_found",
      "Kartenbild wurde nicht gefunden.",
      404,
    );
  ...
  if (clientHasFreshImage(request, validators))
...
function imageValidators(
  ...
  const hash = createHash("sha256")
    .update(
      image.contentHash ??
        `${image.kind}:${image.printingId}:${Number(fileStat.size)}:${Math.floor(Number(fileStat.mtimeMs))}`,
    )
    .digest("base64url")
    .slice(0, 24);
  return {
...
export function clientHasFreshImage(
  ...
  if (ifNoneMatch)
    return ifNoneMatch
      ...
      .some((value) => value === "*" || value === validators.etag);
  ...
  return (
    ...
    lastModifiedMs <= sinceMs
  );
}

function safeImageError(
  code: string,
  message: string,
  status: 400 | 403 | 404,
): NextResponse {
