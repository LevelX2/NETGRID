...
describe("Backend 0.5 maintenance UI helpers", () => {
  it("builds match-list queries for the supported filters", () => {
    ...
    expect(
      ...
      "?status=active&terminal=false&mode=human_vs_human&olderThanDays=14&largerThanBytes=3145728&limit=50",
    );
    expect(
      buildMaintenanceMatchQuery({ ...EMPTY_MAINTENANCE_FILTERS, limit: "" }),
    ).toBe("?limit=all");
  });
  ...
  it("detects forbidden markers in maintenance payloads", () => {
    expect(
      findForbiddenMaintenanceMarkers({
        matchId: "match_1",
        stateHash: "fnv1a:1234",
      }),
    ).toEqual([]);
    expect(
      findForbiddenMaintenanceMarkers({
        sessionToken: "secret",
        privatePayload: { value: true },
      }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("builds bounded cleanup requests with terminal abandoned defaults", () => {
    expect(
      buildMaintenanceCleanupRequest(DEFAULT_MAINTENANCE_CLEANUP_FILTERS),
    ).toEqual({
      statuses: ["abandoned"],
      olderThanMinutes: 60,
      limit: 100,
  ...
  it("builds a root reconnect link from local recovery access", () => {
    expect(
      ...
      "http://127.0.0.1:3000/?matchId=match_abc&side=runner&reconnectToken=recovery_access_secret&recovery=1",
    );
  });

  it("uses the loopback backend for locally opened maintenance pages", () => {
    expect(
      resolveMaintenanceServerHttp("http://192.168.178.141:8787", "127.0.0.1"),
    ...
    ).toBe("http://192.168.178.141:8787");
  });
  ...
  it("formats AI decision trace navigation labels and meta rows", () => {
    ...
    expect(aiTraceTitle(trace)).toBe(
      "#2 Korp · install_card · build_scoring_remote",
    );
    expect(aiTraceMetaRows(trace)).toContainEqual([
      "Ausgeführt",
      "install_card",
    ]);
    expect(aiTraceMetaRows(trace)).toContainEqual(["Vertrauen", "73%"]);
    expect(aiTracePlanLabel("access_trash_steal")).toBe(
      "Zugriff / Trash / Steal",
    );
    expect(aiTracePlanLabel("runner.obtain_breaker_coverage")).toBe(
      "Breaker-Abdeckung vorbereiten",
    );
    expect(aiTracePlanLabel("runner.contest_remote")).toBe("Remote-Run prüfen");
    expect(aiTracePlanLabel("runner.clear_tags_or_survive")).toBe(
      "Tags entfernen / überleben",
    );
    expect(aiTracePlanLabel("runner.convert_success_window")).toBe(
      "Run-Erfolg nutzen",
    );
    expect(aiTracePlanLabel("runner.play_best_hand_card")).toBe(
      "Beste Handkarte spielen",
    );
    expect(aiTracePlanLabel("runner.build_credit_base")).toBe(
      "Credit-Basis aufbauen",
    );
    expect(aiTracePlanLabel("runner.shell_traders_pipeline")).toBe(
  ...
  it("formats AI trace action-level rows for Broker versus basic credit", () => {
    ...
    expect(
      findForbiddenMaintenanceMarkers({
        detail: { actionAlternatives: [{ meta: { decisionDebug: true } }] },
      }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("keeps applied and diagnostic AI trace selections separate", () => {
    ...
    expect(aiTraceMetaRows(trace)).toEqual(
  ...
  it("formats in-game AI trace score rows without inventing missing values", () => {
    ...
    expect(aiTraceDebugGapNotes(detail)).toEqual([
    ...
    expect(JSON.stringify({ detail })).not.toMatch(
      /AIInput|DecisionDebug|cardInstances|privatePayload|decklist|C:\\Users/i,
    );
  });
  ...
  it("builds cursor paths and merges AI decision trace live-follow updates", () => {
    ...
    expect(buildMaintenanceAiTraceIndexPath("match/1", 1)).toBe(
      "/api/storage/maintenance/ai-decision-traces/matches/match%2F1?afterDecisionIndex=1",
    );
    expect(buildMaintenanceAiTraceEnablePath("match/1")).toBe(
      "/api/storage/maintenance/ai-decision-traces/matches/match%2F1/enable",
    );
    expect(
      mergeMaintenanceAiTraceIndex([second], [first, second]).map(
        (trace) => trace.traceId,
      ),
  ...
  it("keeps just-enabled AI trace matches visible while the match index refreshes", () => {
    ...
    expect(
      mergeMaintenanceAiTraceMatches([stale], [activated]).map(
        (match) => match.matchId,
      ),
    ).toEqual(["match_new", "match_old"]);
    expect(
      mergeMaintenanceAiTraceMatches(
        [activated],
        [
          {
            ...activated,
            traceCount: 1,
            lastTraceAt: "2026-05-22T10:03:00.000Z",
          },
        ],
      )[0]?.traceCount,
    ).toBe(1);
  ...
  it("exports only redacted AI trace index projections", () => {
    ...
    expect(() =>
      buildMaintenanceAiTraceNdjsonExport({
        ...
        generatedAt: "2026-05-22T10:01:00.000Z",
        traces: [
          { ...trace, meta: { decisionDebug: { privatePayload: true } } },
        ],
      }),
