...
const TUTORIAL_GLOSSARY: TutorialGlossaryEntry[] = [
  {
    term: "Korp",
    definition:
      "Die verteidigende Seite, die Server aufbaut und Agenden erzielt.",
  },
  {
    term: "Runner",
    definition: "Die angreifende Seite, die Runs startet und Agenden stiehlt.",
  },
  {
    term: "Klick",
    definition: "Aktionspunkt pro Zug, mit dem Du legale Aktionen ausführst.",
  },
  {
    term: "Credit",
    definition: "Ressource zum Bezahlen von Aktionen, Rezzing und Fähigkeiten.",
  },
  ...
  { term: "Archive", definition: "Ablagebereich der Korp." },
  {
    term: "Remote",
    definition:
      "Von der Korp erstellter Server für Assets, Upgrades oder Agenden.",
  },
  { term: "Run", definition: "Angriff des Runners auf einen Server." },
  {
    term: "Begegnung",
    definition: "Phase im Run gegen ICE inklusive Rezzen und Subroutinen.",
  },
  { term: "Zugriff", definition: "Access auf Karten nach erfolgreichem Run." },
  {
    term: "Agenda",
    definition: "Siegpunktkarte für Korp (Scoring) oder Runner (Steal).",
  },
  {
    term: "Tag",
    definition: "Runner-Marker, der bestimmte Korp-Maßnahmen erlaubt.",
  },
  {
    term: "Schaden",
    definition:
      "Kartenschaden gegen den Runner; bei leerer Hand droht Flatline.",
  },
  {
    term: "LegalAction",
    definition: "Von der Engine freigegebene, regelkonforme Aktion.",
  },
];

const HINT_LIBRARY: Record<string, string> = {
  keep_or_mulligan:
    "Wähle nur die aktuell angebotene Setup-Entscheidung aus den LegalActions.",
  clicks_credits_draw:
    "Nutze Klicks für Credits oder Kartenziehen nur über aktuelle LegalActions.",
  run_basics:
    "Ein Run startet immer über eine legale Start-Run-Aktion auf einen sichtbaren Server.",
  encounter_breaker:
    "Im Encounter-Fenster bleiben nur die aktuell legalen Rez/Continue/Breaker-Aktionen gültig.",
  access_basics:
    "Beim Zugriff sind nur die aktuell legalen Access-/Steal-/Trash-Optionen erlaubt.",
  score_basics:
    "Eine Agenda kann nur gescort werden, wenn `score_agenda` als LegalAction vorliegt.",
  steal_game_end:
    "Wenn `steal_agenda` legal ist, kann der Steal direkt den Spielausgang bestimmen.",
  damage_flatline:
    "Damage-/Flatline-Erklärungen bleiben beschreibend und nutzen nur aktuelle LegalActions.",
};
...
const TUTORIAL_SCENARIO_DEFINITIONS: TutorialScenarioDefinition[] = [
    ...
    steps: [
      {
        ...
        successCondition:
          "Runner beantwortet die Setup-Entscheidung über LegalActions.",
      },
    ],
    learnerSide: "runner",
    build(seed) {
      const initial = createGame({
        ...
        corpDeck: CORP_TUTORIAL_ICE_DECK,
      });
  ...
  },
  {
    ...
    build(seed) {
      ...
      const state = applyScriptAction(initial, {
        side: "corp",
    ...
    steps: [
      {
        ...
        hintRefs: ["run_basics"],
        successCondition:
          "Ein Run wird über eine legale Start-Run-Aktion gestartet.",
      },
    ],
    learnerSide: "runner",
    build(seed) {
      ...
      let state = applyScriptAction(initial, {
        side: "corp",
        actionType: "mandatory_draw",
      });
      state = applyScriptAction(state, {
        side: "corp",
        actionType: "end_turn",
      });
  ...
  },
  {
    ...
    rulesBaseline: "0.94.0",
    allowedMechanics: [
      "mechanic.run.encounter",
      "mechanic.breaker.pump_and_break",
      "mechanic.ice.rez",
    ],
    deckSnapshotRefs: [
      "demo_runner_008_snapshot_v0_8",
    ...
    steps: [
      {
        ...
        successCondition:
          "Im Encounter werden nur aktuell legale Optionen genutzt.",
      },
    ],
    learnerSide: "runner",
    build(seed) {
    ...
    steps: [
      {
        ...
        successCondition:
          "Access-Entscheidungen erfolgen über die aktuellen LegalActions.",
      },
    ],
    learnerSide: "runner",
    build(seed) {
    ...
    steps: [
      {
        ...
        successCondition:
          "Korp scored nur über `score_agenda` als LegalAction.",
      },
    ],
    learnerSide: "corp",
    build(seed) {
      const initial = createGameAfterSetup({
        seed,
        runnerDeck: RUNNER_TUTORIAL_DECK,
        corpDeck: CORP_TUTORIAL_SCORE_DECK,
      });
      let state = applyScriptAction(initial, {
        side: "corp",
        actionType: "mandatory_draw",
  ...
  },
  {
    ...
    rulesBaseline: "0.94.0",
    allowedMechanics: [
      "mechanic.agenda.steal",
      "mechanic.game_end.agenda_points",
    ],
    deckSnapshotRefs: [
      "demo_runner_008_snapshot_v0_8",
      "demo_corp_008_snapshot_v0_8",
    ...
    steps: [
      {
        ...
        successCondition:
          "Damage/Flatline wird nur mit bestehenden Mechaniken erklärt.",
      },
    ],
    learnerSide: "runner",
    build(seed) {
      const initial = createGameAfterSetup({
        seed,
        runnerDeck: RUNNER_TUTORIAL_DECK,
        corpDeck: CORP_TUTORIAL_DAMAGE_DECK,
      });
      let state = applyScriptAction(initial, {
        side: "corp",
        actionType: "mandatory_draw",
      ...
      state = applyIfAvailable(state, {
        side: "corp",
        actionType: "rez_ice",
        labelIncludes: "Iceberg",
      });
      return { initial, state };
    },
  },
...
export function listTutorialScenarios(): TutorialScenario[] {
  return TUTORIAL_SCENARIO_DEFINITIONS.map((entry) => ({
    ...
    steps: entry.steps.map((step) => ({
      ...step,
      legalActionRefs: [...step.legalActionRefs],
      hintRefs: [...step.hintRefs],
    })),
  }));
}
...
export function createTutorialSession(
  scenarioId: string,
  seed: string = `tutorial:${scenarioId}`,
): TutorialSession {
  const definition = TUTORIAL_SCENARIO_DEFINITIONS.find(
    (entry) => entry.scenarioId === scenarioId,
  );
  if (!definition)
    throw new Error(`Tutorial-Szenario nicht gefunden: ${scenarioId}`);
...
export function applyTutorialAction(
  ...
  const action = session.legalActions.find(
    (candidate) => candidate.actionId === actionId,
  );
  if (!action)
    throw new Error("Die Aktion ist in diesem Tutorialschritt nicht legal.");
  ...
  const stepAdvanced = currentStep
    ? currentStep.legalActionRefs.includes(action.type)
    : false;
  const nextStep = stepAdvanced
    ? Math.min(
        session.stepIndex + 1,
        Math.max(0, session.scenario.steps.length - 1),
      )
    : session.stepIndex;
  return sessionForState({
    ...
    state: result.state,
    stepIndex: nextStep,
    initialReplayState: replayInitialStateForScenario(
      session.scenario.scenarioId,
    ),
  });
}

export function tutorialAiSparringSuggestion(
  session: TutorialSession,
  side: Side = session.state.activeSide,
): TutorialAiSparringSuggestion | undefined {
  ...
  return {
    ...
    explanation:
      "Sparring nutzt eine lokale LegalAction-Heuristik ohne Hidden-Info-Vorteil.",
  };
}

function pickTutorialSparringAction(
  legalActions: LegalAction[],
  side: Side,
  ...
  return legalActions
    ...
    .sort(
      (left, right) =>
        tutorialActionPriority(right, side) -
          tutorialActionPriority(left, side) ||
...
function toPublicScenario(
  definition: TutorialScenarioDefinition,
): TutorialScenario {
  return {
...
function buildHint(
  ...
  const preferred = legalActions.filter((action) =>
    step.legalActionRefs.includes(action.type),
  );
  const usable =
    preferred.length > 0
      ? preferred
      : legalActions.slice(0, Math.min(2, legalActions.length));
  const text = step.hintRefs
    .map((ref) => HINT_LIBRARY[ref])
  ...
  return {
    ...
    text:
      text ||
      "Nutze die aktuell sichtbaren LegalActions; die Rules Engine bleibt Regelautorität.",
    legalActionIds: usable.map((action) => action.actionId),
    legalActionTypes: usable.map((action) => action.type),
  };
}

function choicePayloadFor(
  state: GameState,
  action: LegalAction,
): PlayerAction["selectedChoices"] | undefined {
  if (action.type !== "resolve_choice" || !state.pendingChoice)
    return undefined;
...
function applyScriptAction(
  state: GameState,
  instruction: ScriptAction,
): GameState {
  ...
  const selected = legalActions.find((action) => {
    ...
    if (
      instruction.serverId &&
      action.payload?.serverId !== instruction.serverId
    )
      return false;
    if (
      instruction.labelIncludes &&
      !action.label.includes(instruction.labelIncludes)
    )
      return false;
    return true;
  });
  if (!selected)
    throw new Error(
      `Tutorial script action not found: ${instruction.side} ${instruction.actionType}`,
    );
