...
export default function MaintenancePage() {
  ...
  const loadSummary = async () => {
    ...
    try {
      const response = await auth.request("/api/storage/maintenance/summary", {
      ...
      if (controller.signal.aborted) throw new Error(t("summaryTimeout"));
      throw loadError;
    } finally {
  ...
  const enableAiTracingForSelectedMatch = async () => {
    ...
    try {
      ...
      const message = t("tracingEnabled", { matchId: matchId });
  ...
  const refresh = async (
    ...
    try {
      ...
      if (firstLoadError) {
        setError(t("m023"));
        setOperationNotice({ tone: "error", message: t("loadFailedHint") });
        return;
      }
      setOperationNotice({
        tone: "success",
  ...
  const runLoadStep = async (
    ...
    try {
      ...
      markLoadStep(
        id,
        "error",
        loadError instanceof Error ? loadError.message : t("m027"),
      );
      onError?.(loadError);
    }
  };
      ...
      <div style={loadStepGrid}>
        {steps.map((step) => (
          <span key={step.id} style={loadStep}>
            ...
            {step.label}
            {step.error ? <span role="alert">: {step.error}</span> : null}
          </span>
        ))}
      </div>
    </section>
...
const workingNotice: CSSProperties = {
  color: "var(--primary-text)",
  fontSize: "0.86rem",
};
