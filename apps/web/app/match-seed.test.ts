...
describe("match seed helpers", () => {
  it("creates a readable fresh match seed", () => {
    expect(
      createMatchSeed({ now: 1_700_000_000_000, randomPart: "abc123" }),
    ).toBe("match-loyw3v28-abc123");
  });
  ...
  it("replaces blank and legacy demo seeds with fresh seeds", () => {
    ...
    expect(normalizeMatchSeed(LEGACY_DEFAULT_MATCH_SEED)).toMatch(
      /^match-loyw3v28-/,
    );
  });
});
