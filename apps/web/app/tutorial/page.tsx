...
import {
  ...
  type TutorialAiSparringSuggestion,
...
export default function TutorialPage() {
  ...
  const [session, setSession] = useState<TutorialSession | null>(() =>
    SCENARIOS[0] ? createTutorialSession(SCENARIOS[0].scenarioId) : null,
  );
  const [aiSuggestion, setAiSuggestion] =
    useState<TutorialAiSparringSuggestion | null>(null);
  const [error, setError] = useState<string>("");

  const scenario = useMemo(
    () => SCENARIOS.find((entry) => entry.scenarioId === scenarioId),
    [scenarioId],
  );
  const currentStep =
    session && scenario
      ? (scenario.steps[session.stepIndex] ??
    ...
    >
      <header style={{ display: "grid", gap: "0.35rem" }}>
        ...
        <p style={{ margin: 0, opacity: 0.82 }}>
          Separater Tutorialmodus ({session?.mode ?? "tutorial_local"}) mit
          LegalAction-basierten Hinweisen und replaybarer StateHash-Prüfung.
        </p>
      </header>
      ...
      {session && scenario && currentStep ? (
        <section style={{ display: "grid", gap: "0.75rem" }}>
          <div style={{ display: "grid", gap: "0.35rem" }}>
            ...
            <p style={{ margin: 0 }}>
              Schritt {session.stepIndex + 1} von {scenario.steps.length} ·
              Timing: <code>{session.state.timingPoint}</code>
            </p>
            <p style={{ margin: 0 }}>{session.hint.text}</p>
            <p style={{ margin: 0 }}>
              Replay/StateHash:{" "}
              <code>{session.replayCheck.ok ? "ok" : "prüfen"}</code> ·{" "}
              <code>{session.replayCheck.finalStateHash}</code>
            </p>
          </div>
          ...
          <div style={{ display: "grid", gap: "0.35rem" }}>
            ...
            {aiSuggestion ? (
              <p style={{ margin: 0 }}>
                {aiSuggestion.side === "runner" ? "Runner" : "Korp"}:{" "}
                <code>{aiSuggestion.label}</code> ({aiSuggestion.explanation})
              </p>
            ) : null}
          </div>
          ...
          <details>
            ...
            <pre style={pre}>
              {JSON.stringify(
                {
                  currentStep,
                  allowedMechanics: scenario.allowedMechanics,
                  deckSnapshotRefs: scenario.deckSnapshotRefs,
                },
                null,
                2,
              )}
