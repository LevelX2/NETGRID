...
export function matchTimerDecisionKey(input: MatchTimerSnapshotInput): string {
  ...
  const choiceKey = choice
    ? `${choice.choiceId}:${choice.side}:${choice.stateVersion}`
    : "none";
  const legalActionKey = input.legalActions
...
export function matchTimerScopeLabel(
  view: PlayerView,
  legalActions: LegalAction[],
): string {
  if (view.winner) return "Spiel beendet";
  if (view.pendingChoice)
    return `${sideLabel(view.pendingChoice.side)} entscheidet`;
  if (
    legalActions.length > 0 &&
    legalActions.every((action) => action.side === view.side)
  )
    return `${sideLabel(view.side)} entscheidet`;
...
export function playerClockLiveRemaining(
...
export function playerClockLiveConsumed(
  snapshot: ApiPlayerClockSnapshot,
  side: Side,
  nowMs: number,
): number {
