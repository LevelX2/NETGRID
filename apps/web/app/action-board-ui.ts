...
const BASE_ACTION_TYPES = new Set<LegalAction["type"]>([
  "mandatory_draw",
  "gain_credit",
  "draw_card",
  "start_run",
...
const PURGEABLE_RUNNER_VIRUS_HELP =
  "Purgefähig: Die Korp kann alle Runner-Virus-Counter entfernen; danach muss sie ihre nächsten 3 Aktionen aussetzen.";
...
function serverHasRezzedTesseractFortConstruction(
  server: PlayerView["servers"][number],
): boolean {
...
export function storedCreditAmount(
  card: Pick<VisibleCard, "counterDisplays">,
): number {
  const display = counterDisplayById(card, "stored_credits");
...
export function armoredFridgeAblativeCounterBadge(
  card: Pick<VisibleCard, "counterDisplays">,
): CardCounterBadgeView | null {
  const display = counterDisplayById(card, "ablative");
  return display
    ? counterDisplayBadgeView(display, "ablative-counter-badge")
    : null;
}

export function counterDisplayById(
  card: Pick<VisibleCard, "counterDisplays">,
  id: string,
): NonNullable<VisibleCard["counterDisplays"]>[number] | null {
  return (
    card.counterDisplays?.find(
      (display) =>
        display.id === id && safeCounterDisplayAmount(display.amount) > 0,
    ) ?? null
  );
}
  ...
  switch (display.counterType) {
    ...
    case "mark":
      ...
      if (display.id === "project_zurich_credits_per_turn")
        return `Project Zurich: ${amount} ${amount === 1 ? "zusätzlicher Credit" : "zusätzliche Credits"} zu Beginn jedes Corp-Zugs. Der Wert stammt aus den Overadvance-Countern beim Scoren.`;
...
export function isConcealedRunnerResourceCard(
  card: Pick<
    VisibleCard,
    "concealed" | "hiddenRunnerResource" | "type" | "subtypes"
  >,
): boolean {
...
export function selectedSubtypeDetailLabel(
  card: Pick<VisibleCard, "selectedSubtypeLabel">,
): string | null {
...
export function hostedOnDetailLabel(
  card: Pick<VisibleCard, "hostedOnLabel">,
): string | null {
...
function serverCounterChipLabel(
  display: NonNullable<VisibleCard["counterDisplays"]>[number],
): string {
  if (
...
export function advancementCounterDisplay(
  card: Pick<VisibleCard, "known" | "advancementCounters">,
): AdvancementCounterDisplay | null {
...
export function cardCreditCounterVisual(
  amount: number,
...
export function counterDisplayUsesCreditBadge(
  display: NonNullable<VisibleCard["counterDisplays"]>[number],
): boolean {
  return (
...
export function counterDisplayUsesRefreshingCreditBadge(
  display: NonNullable<VisibleCard["counterDisplays"]>[number],
): boolean {
  return (
    display.creditPool?.refresh !== undefined ||
    display.displayKind === "recurring_credit"
  );
}

export function splitLegalActions(actions: LegalAction[]): {
...
function hasSelectableActionContext(action: LegalAction): boolean {
  return (
    ...
    serverRefsForAction(action).length > 0
  );
}
...
function warnContextlessLegalAction(action: LegalAction): void {
  ...
  console.warn(
    "NETGRID legal action was classified as contextual without a selectable context; keeping it visible as a primary action.",
    {
...
export function automaticEndTurnAction(
  view: PlayerView,
  actions: LegalAction[],
  side: Side,
  ...
  return ownActions.every((action) => action.type === "end_turn")
    ? endTurn
    : undefined;
}

export function automaticCorpMandatoryDrawAction(
  view: PlayerView,
  actions: LegalAction[],
  side: Side,
): LegalAction | undefined {
  if (
    ...
    return undefined;
  const ownActions = actions.filter((action) => action.side === side);
  const mandatoryDraw = ownActions.find(
    (action) => action.type === "mandatory_draw",
  );
  if (!mandatoryDraw) return undefined;
  return ownActions.every((action) => action.type === "mandatory_draw")
    ? mandatoryDraw
    : undefined;
}
...
export function actionMatchesContext(
  ...
  if (context.kind === "card")
    return cardRefsForAction(action).includes(context.id);
...
export function actionButtonLabel(action: LegalAction): string {
  switch (action.type) {
    ...
    case "gain_credit":
      return action.source === "basic_action"
        ? "Credit nehmen"
...
export function contextualCardActionLabel(action: LegalAction): string {
  switch (action.type) {
    case "gain_credit":
      return (
        scoredAgendaAbilityContextLabel(action) ??
        installedCardAbilityContextLabel(action) ??
    ...
    case "break_subroutine":
      return stripTrailingActionSourceParenthetical(
        breakSubroutineActionLabel(action),
      );
...
export function orderedCardContextActions(
  actions: LegalAction[],
): LegalAction[] {
  return actions
...
function installedCardAbilityContextLabel(action: LegalAction): string | null {
  ...
  if (typeof action.payload?.v1917AssetAbility === "string") {
    switch (action.payload.v1917AssetAbility) {
      case "gain_credits": {
        const amount = Number(
          action.payload.gainCreditsAmount ?? action.payload.gainedCredits ?? 0,
        );
        return amount > 0
  ...
  if (
    ...
    const amount = Number(
      action.payload.gainCreditsAmount ?? action.payload.gainedCredits ?? 6,
    );
    return amount > 0
      ? `${amount} ${amount === 1 ? "Credit" : "Credits"} nehmen`
      : "Credits nehmen";
  }
  return null;
}
...
function scoredAgendaAbilityContextLabel(action: LegalAction): string | null {
  if (
    ...
    return null;
  const amount = Number(
    action.payload.gainCreditsAmount ?? action.payload.gainedCredits ?? 3,
  );
  return amount > 0
    ? `${amount} ${amount === 1 ? "Credit" : "Credits"} nehmen`
    : "Credits nehmen";
...
function resourceAbilityContextLabel(action: LegalAction): string | null {
  ...
  switch (action.payload?.resourceAbility) {
    ...
    case "short_term_contract_take_credits": {
      const amount = Number(
        action.payload.gainCreditsAmount ?? action.payload.gainedCredits ?? 2,
      );
      return amount > 0
        ? `${amount} ${amount === 1 ? "Credit" : "Credits"} nehmen`
        : "Credits nehmen";
...
function breakerNameFromActionLabel(
  ...
  actionSuffixPattern: string,
): string | null {
  if (!label) return null;
  const trimmed = label.trim();
  const prefixed = new RegExp(`^(.+?):\\s*${actionSuffixPattern}$`, "i").exec(
    trimmed,
  );
  if (prefixed?.[1]) return prefixed[1].trim();
  const trailing = new RegExp(`^(.+)\\s+${actionSuffixPattern}$`, "i").exec(
    trimmed,
  );
...
function isNewRemoteInstallAction(
  action: Pick<LegalAction, "type" | "payload">,
): boolean {
  return (
    action.type === "install_card" && action.payload?.serverId === "new_remote"
  );
}
...
function playEventContextLabel(action: LegalAction): string {
  const serverId =
    typeof action.payload?.serverId === "string"
      ...
      : null;
  if (!serverId)
    return action.payload?.xValue !== undefined
      ? cardContextFallbackLabel(action)
      : "Spielen";
  const serverLabel = serverDisplayLabel(serverId);
  ...
  if (
    ...
    /\bRun\b/i.test(fullLabel) ||
    /\bDeep Dive\b/i.test(fullLabel)
  )
...
function scoreAgendaContextLabel(action: LegalAction): string {
  ...
  return selectedServerLabel
    ? `Scoren: ${selectedServerLabel} wählen`
    : "Scoren";
}
...
export function actionSlotDisplay(
  ...
  displayCapacity: number | undefined,
  active: boolean,
): ActionSlotDisplay {
  ...
  const capacity = shouldShowSlots
    ? Math.max(
        baseCapacity,
        Math.floor(displayCapacity ?? baseCapacity),
        available,
      )
    : 0;
...
export function actionSlotCapacityForTurn(
  side: Side,
  currentClicks: number,
  ...
  const available = Math.max(0, Math.floor(currentClicks));
  return Math.max(
    baseActionSlotCapacity(side),
    available + spentActionClicksThisTurn(side, events),
  );
}

function spentActionClicksThisTurn(
  side: Side,
  events: PublicGameEvent[],
): number {
  let spent = 0;
...
export function stealCostPaymentLabel(
  payload: LegalAction["payload"] | undefined,
): string | null {
  const additionalCost =
    positiveInteger(payload?.stealAdditionalCost) ||
    positiveInteger(payload?.stealCost);
...
function accessedAgendaTitleFromStealActionLabel(label: string): string | null {
  const title = normalizeVisibleTerms(label)
    .match(/^(.+?)\s+stehlen$/i)?.[1]
    ?.trim();
...
export function actionCostChips(
  action: Pick<LegalAction, "costs"> &
    Partial<Pick<LegalAction, "type" | "payload">>,
): CostChipView[] {
...
export function aiPacingDelayMs(
...
export function serverBoardRows<T extends { id: string }>(
  servers: T[],
  viewerSide: Side,
): Array<{ kind: "remotes" | "centrals"; servers: T[] }> {
  const centralOrder = new Map([
    ["hq", 0],
    ["rd", 1],
    ["archives", 2],
  ]);
  const central = servers
    .filter((server) => centralOrder.has(server.id))
    .sort(
      (left, right) => centralOrder.get(left.id)! - centralOrder.get(right.id)!,
    );
  const remotes = servers
    .filter((server) => /^remote_\d+$/.test(server.id))
    .sort((left, right) => remoteNumber(left.id) - remoteNumber(right.id));
  const other = servers.filter(
    (server) => !centralOrder.has(server.id) && !/^remote_\d+$/.test(server.id),
  );
  const centralRow = { kind: "centrals" as const, servers: central };
  const remoteRow = {
    kind: "remotes" as const,
    servers: [...remotes, ...other],
  };
  return viewerSide === "corp"
    ? [remoteRow, centralRow]
    : [centralRow, remoteRow];
}
...
export function accessRevealStatusLabel(
  ...
  const withServer = (status: string) =>
    `${accessServerStatusPrefix(serverLabel)}: ${status}`;
  if (actorSide !== viewerSide)
    return withServer(observedAccessStatusLabel(card, actorSide, fromArchives));
  const stealCostStatus = accessRevealStealCostStatus(actions);
  if (stealCostStatus) return withServer(stealCostStatus);
  if (actions.some((action) => action.type === "steal_agenda"))
    return withServer("Diese Agenda kann jetzt gestohlen werden.");
  if (fromArchives && (card.type === "asset" || card.type === "upgrade")) {
    return withServer(
      "Du hast diese Karte im Archiv gesehen. Du kannst weiter zugreifen oder den Zugriff abschließen.",
    );
  }
  const freeTrashSource = accessFreeTrashSourceLabel(actions);
  if (freeTrashSource)
    return withServer(
      `${freeTrashSource}: Du kannst diese Karte kostenlos trashen, auch wenn sie normalerweise keine Trash-Kosten hat.`,
    );
  if (actions.some((action) => action.type === "trash_accessed_card"))
    return withServer(
      "Du kannst diese Karte jetzt trashen oder den Zugriff abschließen.",
    );
  if (actions.some((action) => action.type === "access_card"))
    return withServer(
      "Der Zugriff auf diese Karte ist abgeschlossen. Du kannst direkt zur nächsten Karte weitergehen.",
    );
  if (actions.length === 0) {
    if (serverDisplayLabel(serverLabel) === "R&D")
      return withServer("Angezeigte Karte aus Research and Development.");
    return withServer("Angezeigte Karte. Du kannst das Fenster schließen.");
  }
  if (card.type === "asset" || card.type === "upgrade")
    return withServer(
      "Du hast aktuell nicht genug Credits, um die Trash-Kosten zu bezahlen. Du kannst den Zugriff abschließen.",
    );
  if (actions.some((action) => action.type === "decline_trash"))
    return withServer(
      "Diese Karte hat keine Trash-Kosten. Du kannst den Zugriff abschließen.",
    );
  return withServer(
    "Diese Karte hat keine Trash-Kosten. Der Zugriff ist abgeschlossen.",
  );
}
...
function accessFreeTrashSourceLabel(actions: LegalAction[]): string | null {
  ...
  const counterType =
    freeTrashAction?.payload?.proteusRunnerVirusFreeTrashCounterType;
...
function accessRevealStealCostStatus(actions: LegalAction[]): string | null {
  ...
  const blockedAction = actions.find(
    (action) =>
      action.type === "decline_trash" &&
      action.payload?.stealBlockedByCost === true,
  );
  const sourceAction = stealAction ?? blockedAction;
  if (!sourceAction) return null;
  const additionalCost =
    positiveInteger(sourceAction.payload?.stealAdditionalCost) ||
    positiveInteger(sourceAction.payload?.stealCost);
  if (additionalCost <= 0) return null;
  const sourceLabel = stealCostSourceLabel(
    sourceAction.payload?.stealCostSourceTitles,
  );
...
export function retainedAccessRevealEvent(
  events: PublicGameEvent[],
  ...
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    return newerEvents.every((newerEvent) =>
      accessRevealCanBeRetainedPast(newerEvent, event),
    )
      ? event
      : null;
  }
...
export function latestRetainableAccessRevealEvent(
  events: PublicGameEvent[],
): PublicGameEvent | null {
  return retainedAccessRevealEvent(events, null);
}

export function retainedExposeReviewEvent(
  events: PublicGameEvent[],
  dismissedEventId: string | null,
): PublicGameEvent | null {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index];
    if (!event || event.eventId === dismissedEventId) return null;
    if (event.publicPayload.approachIceExposeViewDecision === "finish")
      return null;
    if (event.publicPayload.hiddenZoneAction === "approach_ice_expose_finish")
      return null;
    if (event.publicPayload.hiddenZoneAction === "expose_installed_card_review")
      return null;
    if (event.publicPayload.hiddenZoneAction === "expose_installed_card_finish")
      return null;
    if (
    ...
    if (event.publicPayload.hiddenZoneAction === "approach_ice_expose")
      return null;
...
export function approachIceExposeViewingIceId(
  ...
  return typeof action?.payload?.iceId === "string"
...
function accessRevealCanBeRetainedPast(
  newerEvent: PublicGameEvent,
  accessEvent: PublicGameEvent,
): boolean {
  if (newerEvent.stateVersionAfter === accessEvent.stateVersionAfter)
    return true;
  ...
  if (
    ...
    (typeof newerEvent.publicPayload.ambushDefinitionId === "string" ||
      typeof newerEvent.publicPayload.accessEffectSourceDefinitionId ===
        "string")
  )
    return true;
  return [
...
function eventHasPublicRevealDefinition(event: PublicGameEvent): boolean {
  if (typeof event.publicPayload.publicRevealDefinitionId === "string")
    return true;
  return (
...
function observedAccessStatusLabel(
  card: Pick<VisibleCard, "type" | "trashCost">,
  actorSide: Side,
  ...
  if (card.type === "agenda")
    return `${subject} kann diese Agenda jetzt stehlen.`;
  if (fromArchives && (card.type === "asset" || card.type === "upgrade"))
    return `${subject} hat diese Karte im Archiv gesehen.`;
  if (
    (card.type === "asset" || card.type === "upgrade") &&
    typeof card.trashCost === "number"
  )
    return `${subject} entscheidet jetzt, ob diese Karte getrasht oder liegen gelassen wird.`;
  return `${subject} hat diese Karte gesehen; der Zugriff ist abgeschlossen.`;
}

export function currentRunTimelineStep(
  view: PlayerView,
  actions: LegalAction[],
): RunTimelineStepId | null {
...
export function runTargetServerIds(view: PlayerView): string[] {
  if (!view.run) return [];
  return view.servers.some((server) => server.id === view.run?.attackedServerId)
    ? [view.run.attackedServerId]
    : [];
}
...
export function shouldUseCardChoicePanel(
  choice: NonNullable<PlayerView["pendingChoice"]>,
): boolean {
...
export function cardChoiceIsReadonlyPrivateLook(
  choice: NonNullable<PlayerView["pendingChoice"]>,
): boolean {
  if (
    choice.kind !== "select_cards" ||
    ...
    return false;
  const doneOptions = choice.options.filter(
    (option) => option.id === "done" && option.selectable !== false,
  );
  ...
  return (
    ...
    cardOptions.every(
      (option) => option.selectable === false && Boolean(option.card),
    )
  );
}

export function cardChoiceReadonlyConfirmationOptionId(
  choice: NonNullable<PlayerView["pendingChoice"]>,
): string | null {
...
export function cardChoiceUsesOrderedSelection(
  choice: NonNullable<PlayerView["pendingChoice"]>,
): boolean {
  const minSelections = Math.max(0, Math.floor(choice.minSelections));
  const maxSelections = Math.max(
...
export function newBloodReorderTargetLabel(
  ...
  if (
    choice.kind !== "select_cards" ||
    !choice.source.startsWith("p3_58.new_blood_reorder")
  )
    return null;
  ...
  const publicLabel =
    typeof targetOption.publicLabel === "string"
      ? targetOption.publicLabel.trim()
      : "";
...
export function fieldCardChoiceOptionsForCard(
  ...
  return choice.options.filter(
    (option) =>
      ...
      fieldCardChoiceOptionTargetsCard(option, card),
  );
}
...
function fieldCardChoiceOptionTargetsCard(
  ...
  return (
    option.value === card.instanceId || option.id === `card_${card.instanceId}`
  );
}
...
export function runnerProgramInstallTrashChoiceInfo(
  ...
  const sourceCard = view.own.gripOrHq.find(
  ...
  return {
    ...
    canSubmit:
      !memoryRequired || enoughMemoryFreed || selectedOptionIds.length === 0,
    requiredMemoryToFree,
    selectedMemoryFreed,
  };
}
...
export function breachHighlighterAccessHint(view: PlayerView): string | null {
  ...
  const knownTotal = Math.max(
    current,
    breach.completed ? current : breach.currentIndex + breach.remainingCount,
  );
...
function corpHighlighterCounterAmount(view: PlayerView): number {
  const corpIdentity =
    view.side === "corp" ? view.own.identity : view.opponent.identity;
...
export function runAwareActionButtonLabel(
  ...
  if (action.type === "jack_out") {
    return iceLabel
      ? `Jack-out: an ${iceLabel} abbrechen`
      : "Jack-out: vor Zugriff abbrechen";
  }
  if (action.type === "continue_run") {
    ...
    if (view.run.phase === "movement")
      return iceLabel
        ? `Weiterlaufen: zu ${iceLabel}`
        : "Weiterlaufen: zum Zugriff";
    if (view.run.phase === "approach_ice" && iceLabel)
      return `Annäherung an ${iceLabel} fortsetzen`;
  }
  if (
    (action.type === "pump_breaker" || action.type === "break_subroutine") &&
    iceLabel &&
...
export function actionButtonTone(
  view: PlayerView,
  action: LegalAction,
): ActionButtonTone {
...
function isUnnecessaryBreakerPumpAction(
  ...
  const breaker = runnerRigForView(view).find(
    (card) => card.instanceId === breakerId,
  );
  if (!breaker) return false;
  const breakerStrength = visibleCardStrength(breaker);
  const iceStrength = visibleCardStrength(run.encounteredIce);
  return (
    breakerStrength !== null &&
    iceStrength !== null &&
    breakerStrength >= iceStrength
  );
}

function visibleCardStrength(
  card: Pick<VisibleCard, "strength"> | null | undefined,
): number | null {
  return typeof card?.strength === "number" && Number.isFinite(card.strength)
    ? card.strength
    : null;
}
...
export function runBreakerActionHint(
  ...
  if (view.side !== "runner") {
    ...
    if (visibleBreakers.length === 1)
      return `Runner-Rig zeigt passenden Eisbrecher: ${visibleBreakers[0]!.title}.`;
    if (visibleBreakers.length > 1)
      return `Runner-Rig zeigt passende Eisbrecher: ${visibleBreakers
        .slice(0, 2)
        .map((card) => card.title)
        .join(", ")}${visibleBreakers.length > 2 ? " ..." : ""}.`;
    return null;
  }
  return "Kein passender Eisbrecher für dieses ICE verfügbar.";
}

function compactRunWindowBreakerLabel(
  view: PlayerView,
  action: LegalAction,
): string {
...
export function encounterBreakerActions(
  ...
  const rigById = new Map(
    runnerRigForView(view).map((card) => [card.instanceId, card]),
  );
  return actions.filter((action) => {
    if (action.type !== "pump_breaker" && action.type !== "break_subroutine")
      return false;
    ...
    return breaker
      ? breakerMatchesEncounteredIce(breaker, encounteredIce)
      : false;
  });
}
...
function breakerMatchesEncounteredIce(
  breaker: VisibleCard,
  encounteredIce: VisibleCard | null,
): boolean {
  if (!encounteredIce?.known) return true;
  const iceSubtypes = new Set(
    (encounteredIce.subtypes ?? []).map(normalizeBreakerSubtype),
  );
  ...
  if (
    iceSubtypes.has("sentry") &&
    /\bbreak\s+(?:1\s+)?sentry\s+subroutine\b/.test(rulesText)
  )
    return true;
  if (
    iceSubtypes.has("wall") &&
    /\bbreak\s+(?:1\s+)?wall\s+subroutine\b/.test(rulesText)
  )
    return true;
  if (
    iceSubtypes.has("code gate") &&
    /\bbreak\s+(?:1\s+)?code gate\s+subroutine\b/.test(rulesText)
  )
    return true;
  return false;
}
...
function isVisibleIcebreaker(card: VisibleCard): boolean {
  ...
  return /\bbreak\s+(?:1\s+)?(?:ice|sentry|wall|code gate)\s+subroutine\b/.test(
    normalizeBreakerRulesText(card.rulesText ?? ""),
  );
}
...
export function runnerRigMemorySummary(
  view: PlayerView,
  owner: "own" | "opponent",
): { used: number; limit: number; text: string; ariaLabel: string } | null {
  const source = owner === "own" ? view.own : view.opponent;
  if (
    typeof source.memoryUsed !== "number" ||
    typeof source.memoryLimit !== "number"
  )
    return null;
  return {
...
export function corpInstalledCardState(
  card: VisibleCard,
): "hidden" | "unrezzed" | "rezzed" | "known" {
...
export function showInstalledCorpState(
...
export function splitArchiveCardsForDisplay(
  ...
  faceupCards: VisibleCard[];
  facedownCards: VisibleCard[];
  ...
  return {
    faceupCards,
    facedownCards,
    facedownCount: Math.max(
      facedownCards.length,
      totalArchivesCount - faceupCards.length,
    ),
  };
}
...
function objectBoundAction(action: LegalAction): boolean {
  ...
  return (
    serverRefsForAction(action).length > 0 ||
    ["advance_card", "score_agenda", "trash_resource"].includes(action.type)
  );
}

function isSelfModifyingCodeAction(
  action: Partial<Pick<LegalAction, "type" | "payload">>,
): boolean {
  return (
    action.type === "trigger_ability" &&
    actionHasAbility(action, "self_modifying_code_install_program")
  );
}

function isApproachIceExposeAction(
  action: Partial<Pick<LegalAction, "type" | "payload">>,
): boolean {
  return (
    action.type === "trigger_ability" &&
...
function isRemoveSpyCounterAction(
  action: Partial<Pick<LegalAction, "type" | "payload">>,
): boolean {
  return (
    action.type === "trigger_ability" &&
...
function sourceCardTitleForAction(
  view: PlayerView,
  action: LegalAction,
): string | null {
  const sourceId =
  ...
  const sourceCard = visibleActionCards(view).find(
...
function cardRefsForAction(action: LegalAction): string[] {
  ...
  if (action.abilityRef?.sourceCardInstanceId)
    refs.add(action.abilityRef.sourceCardInstanceId);
  if (action.type !== "pump_breaker" && action.type !== "break_subroutine") {
...
function serverRefsForAction(action: LegalAction): string[] {
  ...
  for (const requirement of action.targetRequirements) {
    for (const allowedServer of requirement.allowedServers ?? [])
      refs.add(allowedServer);
  }
  return Array.from(refs);
}
...
function remoteNumber(serverId: string): number {
  return Number(
    /^remote_(\d+)$/.exec(serverId)?.[1] ?? Number.MAX_SAFE_INTEGER,
  );
}
