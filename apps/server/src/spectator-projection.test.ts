...
describe("SpectatorProjectionV1 leakscan harness", () => {
  it("builds a named versioned spectator projection without player, replay, token or hidden-card payload leaks", () => {
    ...
    expect(
      projection.board.servers.find((server) => server.id === "remote_1")
        ?.rootSlots,
    ).toEqual([{ slot: 0, visibility: "hidden" }]);
  ...
  it("keeps delayed cursor projections free of board and event data from cursor N+1", () => {
    ...
    expect(
      current.board.servers.some((server) => server.id === "remote_2"),
    ).toBe(true);
    expect(
      current.events.some((event) => event.type === "future_public_marker"),
    ).toBe(true);
  ...
  it("rejects delayed cursor projections when no safe historical snapshot exists", () => {
    ...
    const withoutHistoricalSnapshot = {
      ...record,
      stateSnapshots: record.stateSnapshots.filter(
        (snapshot) => snapshot.stateVersion > 2,
      ),
    };
  ...
  it("rebuilds the same spectator projection for the same reconnect cursor without PlayerView fallback", () => {
    ...
    const firstReconnect = buildSpectatorProjectionV1(record, {
      eventCursor: 2,
    });
    const secondReconnect = buildSpectatorProjectionV1(record, {
      eventCursor: 2,
    });

    expect(secondReconnect).toEqual(firstReconnect);
    expect(JSON.stringify(firstReconnect)).not.toMatch(
      /playerView|legalActions|pendingChoice/i,
    );
...
function spectatorFixture(): { record: StoredMatch; secrets: string[] } {
  ...
  const hiddenAgendaId = putCorpRootInRemote(
    hiddenState,
    "remote_1",
    "simple_agenda",
  );
  hiddenState.pendingChoice = {
    ...
    options: [
      {
        id: "secret-simple-agenda",
        label: "Simple Agenda",
        value: hiddenAgendaId,
      },
    ],
    minSelections: 1,
    maxSelections: 1,
    ...
    eventRecord(
      matchId,
      {
        ...
        publicPayload: {
          ...
          privatePayload: {
            runner: { cardInstances: hiddenState.cardInstances },
          },
...
function leakFindings(
  projection: SpectatorProjectionV1,
  forbiddenValues: string[],
): string[] {
  const serialized = JSON.stringify(projection);
  const keyFindings = findForbiddenKeys(projection);
  const valueFindings = forbiddenValues.filter((value) =>
...
function findForbiddenKeys(value: unknown, path: string[] = []): string[] {
  ...
  if (Array.isArray(value)) {
    return value.flatMap((entry, index) =>
      findForbiddenKeys(entry, [...path, String(index)]),
    );
  }
