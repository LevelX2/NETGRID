import {
  ...
  type ObservabilityRedactionViolation,
...
export type ModerationAccessLevel =
  | "allow"
  | "redacted"
  | "own_only"
  | "break_glass"
...
export const MODERATION_ROLES: ModerationRole[] = [
  "system",
  "admin",
  "moderator",
  "support_readonly",
...
export const MODERATION_RBAC_MATRIX: Record<
  ModerationDataClass,
  Record<ModerationRole, ModerationAccessLevel>
> = {
  D0_public_lobby_metadata: {
    system: "allow",
    admin: "allow",
...
export function moderationAccessFor(
  role: ModerationRole,
  dataClass: ModerationDataClass,
): ModerationAccessLevel {
  return MODERATION_RBAC_MATRIX[dataClass][role];
}

export function findModerationEvidenceRedactionViolations(
  value: unknown,
): ModerationEvidenceViolation[] {
  return findObservabilityRedactionViolations(value).map((violation) => ({
    ...violation,
    source: "observability_baseline",
  }));
}

export function isStandardModeratorAccessAllowed(
  role: ModerationRole,
  dataClass: ModerationDataClass,
): boolean {
