...
describe("closed account HTTP API", () => {
  it("keeps login neutral and enforces HttpOnly cookie, origin, CSRF, admin invite and one-time acceptance", async () => {
    ...
    try {
      const unknown = await login(baseUrl, "unbekannt", "Falsche geheime Passphrase 2026");
      const wrong = await login(baseUrl, "admin", "Falsche geheime Passphrase 2026");
      ...
      const signedInPayload = JSON.parse(signedInText) as { csrfToken: string; account: { accountId: string; role: string } };
      const setCookie = signedIn.headers.get("set-cookie") ?? "";
      const cookie = setCookie.split(";", 1)[0] ?? "";
      expect(signedIn.status).toBe(200);
      expect(setCookie).toMatch(/^ng_account_session=[^;]+; HttpOnly; SameSite=Lax; Path=\/; Max-Age=1209600$/);
      ...
      const noCsrf = await fetch(`${baseUrl}/api/account/admin/invites`, {
        ...
        body: JSON.stringify({ loginName: "runner-one", displayName: "Runner Eins" }),
      });
      expect(noCsrf.status).toBe(403);

      const foreignOrigin = await fetch(`${baseUrl}/api/account/logout`, {
        method: "POST",
        headers: { origin: "https://attacker.example", cookie, "x-netgrid-csrf": signedInPayload.csrfToken },
      });
      ...
      const accepted = await fetch(`${baseUrl}/api/account/invites/${invitePayload.inviteToken}/accept`, {
        method: "POST",
        headers: { "content-type": "application/json", origin: ORIGIN },
        body: JSON.stringify({ password: "Sehr sichere Runner Passphrase 2026" }),
      });
      expect(accepted.status).toBe(200);
      const acceptedText = await accepted.text();
      expect(acceptedText).not.toContain(invitePayload.inviteToken);
      expect((await fetch(`${baseUrl}/api/account/invites/${invitePayload.inviteToken}`)).status).toBe(404);
  ...
  it("creates one-time admin resets, revokes existing sessions and uses Secure cookies in private internet", async () => {
    ...
    const handle = createNetgridHttpServer(new MultiplayerService(new InMemoryMatchStorage()), { deploymentConfig, accountAuth });
    const baseUrl = await listen(handle);
    try {
      const signedIn = await login(baseUrl, "admin", ADMIN_PASSWORD, "https://play.netgrid.example");
      ...
      const reset = await fetch(`${baseUrl}/api/account/admin/resets`, {
        method: "POST",
        headers: authHeaders(cookie, payload.csrfToken, "https://play.netgrid.example"),
        body: JSON.stringify({ loginName: "admin" }),
      });
      expect(reset.status).toBe(201);
      const resetPayload = await reset.json() as { resetToken: string };
      const accepted = await fetch(`${baseUrl}/api/account/resets/${resetPayload.resetToken}/accept`, {
        method: "POST",
        headers: { "content-type": "application/json", origin: "https://play.netgrid.example" },
        body: JSON.stringify({ newPassword: "Noch sicherere Admin Passphrase 2026" }),
      });
      expect(accepted.status).toBe(200);
      expect((await fetch(`${baseUrl}/api/account/session`, { headers: { cookie } })).status).toBe(401);
      expect((await login(baseUrl, "admin", ADMIN_PASSWORD, "https://play.netgrid.example")).status).toBe(401);
      expect((await login(baseUrl, "admin", "Noch sicherere Admin Passphrase 2026", "https://play.netgrid.example")).status).toBe(200);
      expect((await fetch(`${baseUrl}/api/account/resets/${resetPayload.resetToken}/accept`, {
        method: "POST",
        headers: { "content-type": "application/json", origin: "https://play.netgrid.example" },
        body: JSON.stringify({ newPassword: "Dritte sichere Admin Passphrase 2026" }),
      })).status).toBe(404);
    } finally {
      await handle.close();
    }
...
function login(baseUrl: string, loginName: string, password: string, origin = ORIGIN): Promise<Response> {
...
function authHeaders(cookie: string, csrfToken: string, origin = ORIGIN): Record<string, string> {
  return { "content-type": "application/json", origin, cookie, "x-netgrid-csrf": csrfToken };
}
