...
export const INVITE_LOBBY_FORBIDDEN_KEY_RULES: KeyRule[] = [
  {
    ...
    description:
      "Invite/lobby metadata must not expose raw tokens or token hashes.",
    pattern:
      /^(?:token|.*Token|.*TokenHash|tokenHash|joinToken|sessionToken|reconnectToken|hostSessionToken|hostReconnectToken)$/i,
  },
  {
    ruleId: "session-id-field",
    description:
      "Invite/lobby metadata must not expose internal session identifiers.",
    pattern: /^(?:sessionId|sessionIds|sessions)$/i,
  },
  {
    ruleId: "decklist-or-deckhash-field",
    description:
      "Invite/lobby metadata must not expose decklists, deck snapshots, deck cards, or deck hashes.",
    pattern:
      /^(?:decklist|deckList|deckHash|deckSnapshot|deckSnapshotId|deckCards|cards|privateDeck|privateDeckSnapshots)$/i,
  },
  {
    ruleId: "hidden-card-identity-field",
    description:
      "Invite/lobby metadata must not expose hidden card identities.",
    pattern:
      /^(?:cardInstances|definitionId|cardDefinitionId|cardId|cardIds|hiddenCard|hiddenCards)$/i,
  },
  {
    ruleId: "ai-input-or-decision-debug-field",
    description:
      "Invite/lobby metadata must not expose AIInput or DecisionDebug diagnostics.",
    pattern:
      /^(?:AIInput|aiInput|DecisionDebug|decisionDebug|aiDecisionDebug)$/i,
  },
];
...
export const INVITE_LOBBY_FORBIDDEN_VALUE_RULES: ValueRule[] = [
  {
    ...
    description:
      "Invite/lobby metadata must not embed join-token URLs or query values.",
...
export function findInviteLobbyPayloadRedactionLeaks(
  payload: unknown,
): InviteLobbyRedactionLeak[] {
  const leaks: InviteLobbyRedactionLeak[] = [];
  visitInviteLobbyPayload(payload, [], leaks, new Set<unknown>());
  return leaks;
}

export function assertInviteLobbyPayloadRedacted(
  payload: unknown,
  label = "invite/lobby payload",
): void {
  ...
  const details = leaks
    ...
    .join("\n");
  throw new Error(
    `${label} leaked forbidden invite/lobby metadata:\n${details}`,
  );
}

function visitInviteLobbyPayload(
  payload: unknown,
  path: string[],
  leaks: InviteLobbyRedactionLeak[],
  seen: Set<unknown>,
): void {
  ...
  if (Array.isArray(payload)) {
    payload.forEach((entry, index) =>
      visitInviteLobbyPayload(entry, [...path, String(index)], leaks, seen),
    );
    return;
  }
