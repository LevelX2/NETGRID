...
const SECOND_PASSWORD = "Eine starke zweite Passphrase 2026";
...
describe("private account statistics HTTP API", () => {
  it("serves only the signed-in account, exports v2 and deletes its statistics", async () => {
    ...
    const accountStatistics = new AccountMatchStatisticsService(
      new InMemoryAccountStatisticsStorage("2026-07-19T00:00:00.000Z"),
    );
    const handle = createNetgridHttpServer(
      new MultiplayerService(new InMemoryMatchStorage(), {
        tokenSalt: "account-statistics-http-match",
      }),
      {
    ...
    try {
      ...
      expect((await fetch(`${baseUrl}/api/account/statistics`)).status).toBe(
        401,
      );
      ...
      const invited = await fetch(`${baseUrl}/api/account/admin/invites`, {
        ...
        body: JSON.stringify({
          loginName: "zweiter-spieler",
          displayName: "Zweiter Spieler",
        }),
      });
      ...
      const secondCreatedResponse = await fetch(`${baseUrl}/api/matches`, {
        method: "POST",
        headers: {
          "content-type": "application/json",
          origin: ORIGIN,
        ...
        body: JSON.stringify({
          ...
          playMode: "human_vs_ai",
          humanSide: "corp",
          seed: "account-statistics-http-foreign",
          isPublic: false,
        }),
      });
      const secondCreated = (await secondCreatedResponse.json()) as {
        matchId: string;
        hostSessionToken: string;
        hostSide: "runner" | "corp";
      };
      const secondForfeited = await fetch(
        `${baseUrl}/api/matches/${encodeURIComponent(secondCreated.matchId)}/forfeit`,
        {
          method: "POST",
          headers: { "content-type": "application/json", origin: ORIGIN },
          body: JSON.stringify({
            side: secondCreated.hostSide,
            sessionToken: secondCreated.hostSessionToken,
          }),
        },
      );
      expect(secondForfeited.status).toBe(200);
      ...
      expect(recentResultsText).not.toContain(secondCreated.matchId);
      const secondRecentResultsText = await (
        await fetch(`${baseUrl}/api/account/recent-results?limit=20`, {
          headers: { cookie: secondCookie },
        })
      ).text();
      expect(secondRecentResultsText).toContain(secondCreated.matchId);
      expect(secondRecentResultsText).not.toContain(created.matchId);
      ...
      const statisticsResponse = await fetch(
        `${baseUrl}/api/account/statistics?period=all&opponentKind=ai`,
        { headers: { cookie } },
      );
      ...
      const historyResponse = await fetch(
        `${baseUrl}/api/account/match-history?limit=1`,
        { headers: { cookie } },
      );
      ...
      expect(
        await accountStatistics.gameResultsForAccount(auth.account.accountId),
      ).toEqual([]);
      expect(
        await accountStatistics.seriesResultsForAccount(auth.account.accountId),
      ).toEqual([]);
    } finally {
      await handle.close();
    }
