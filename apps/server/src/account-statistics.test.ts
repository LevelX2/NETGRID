...
describe("AccountMatchStatisticsService participant binding", () => {
  it("binds authenticated slots idempotently, rejects conflicting accounts and inherits bindings", async () => {
    const service = new AccountMatchStatisticsService(
      new InMemoryAccountStatisticsStorage(),
      { now: () => "2026-07-19T01:00:00.000Z" },
    );
    await service.bindAuthenticatedParticipant({
      matchId: "match_a",
    ...
    await expect(
      service.bindAuthenticatedParticipant({
        matchId: "match_a",
        participantSlot: "player_a",
        accountId: "account_b",
        bindingSource: "authenticated_create",
    ...
    await service.inheritMatchParticipants({
      sourceMatchId: "match_a",
      targetMatchId: "match_b",
  ...
  it("records a terminal account game exactly once from persisted match transitions", async () => {
    ...
    const matches = new MultiplayerService(matchStorage, {
      tokenSalt: "account-statistics-ledger",
    });
    const statistics = new AccountMatchStatisticsService(
      new InMemoryAccountStatisticsStorage(),
      { now: () => "2026-07-19T03:00:00.000Z" },
    );
    matches.addPersistenceObserver((record) =>
      statistics.recordTerminalMatch(record),
    );
    ...
    const forfeited = await matches.forfeitMatch({
      matchId: created.matchId,
      side: "runner",
  ...
  it("repairs a missed terminal projection through reconciliation", async () => {
    const matchStorage = new InMemoryMatchStorage();
    const matches = new MultiplayerService(matchStorage, {
      tokenSalt: "account-statistics-reconcile",
    });
    const statistics = new AccountMatchStatisticsService(
      new InMemoryAccountStatisticsStorage(),
    );
    ...
    expect(
      await statistics.gameResultsForAccount("account_reconcile"),
    ).toHaveLength(1);
  });
  ...
  it("reconciles only targeted terminal candidates without falling back to a full list", async () => {
    ...
    const storage = {
      ...
      list: async () => {
        throw new Error(
          "full_list_must_not_be_used_for_startup_reconciliation",
        );
      },
  ...
  it("keeps a terminal result idempotent when a later series transition updates the match timestamp", async () => {
    ...
    await expect(
      statistics.recordTerminalMatch(stored),
  ...
  it("aggregates owner-scoped filters and paginates the private history", async () => {
    const storage = new InMemoryAccountStatisticsStorage(
      "2026-07-01T00:00:00.000Z",
    );
    const statistics = new AccountMatchStatisticsService(storage, {
      now: () => "2026-07-19T12:00:00.000Z",
    });
    await storage.recordGameResult(
      gameRecord({
        accountGameResultId: "result_3",
        originMatchId: "match_3",
    ...
    await storage.recordGameResult(
      gameRecord({
        accountGameResultId: "result_2",
        originMatchId: "match_2",
        completedAt: "2026-07-10T12:00:00.000Z",
        side: "corp",
        outcome: "loss",
        opponentKind: "account",
        finishKind: "forfeit",
        agendaPointsAgainst: 7,
      }),
    );
    await storage.recordGameResult(
      gameRecord({
        accountGameResultId: "result_1",
        originMatchId: "match_1",
    ...
    expect(
      await statistics.statisticsForAccount("account_owner", { period: "30d" }),
    ).toMatchObject({
      totals: { gamesPlayed: 2, wins: 1, losses: 1, draws: 0, forfeitsLost: 1 },
      bySide: {
        runner: { gamesPlayed: 1, wins: 1 },
        corp: { gamesPlayed: 1, losses: 1 },
      },
      byOpponentKind: {
        ai: { gamesPlayed: 1 },
    ...
    expect(
      (
        await statistics.statisticsForAccount("account_owner", {
          side: "runner",
          opponentKind: "ai",
        })
      ).totals.gamesPlayed,
    ...
    const first = await statistics.matchHistoryForAccount("account_owner", {
      limit: 1,
    });
    expect(first.entries.map((entry) => entry.resultId)).toEqual(["result_3"]);
    expect(first.nextCursor).toBe("result_3");
    const second = await statistics.matchHistoryForAccount("account_owner", {
      limit: 1,
  ...
  it("aggregates and keyset-paginates a large SQLite ledger without loading the full account history", async () => {
    const dir = await mkdtemp(
      join(tmpdir(), "netgrid-account-statistics-query-"),
    );
    ...
    const owner = await sessions.createAccount({
      loginName: "query_owner",
      displayName: "Query Owner",
    });
    const other = await sessions.createAccount({
      loginName: "query_other",
      displayName: "Query Other",
    });
    const sqliteStorage = new SqliteAccountStatisticsStorage({
      dbPath,
      backupDir,
    });
    ...
    try {
      for (let index = 0; index < 240; index += 1) {
        ...
        const draft = gameRecord({
          ...
          completedAt: new Date(
            Date.parse(now) - index * 6 * 60 * 60 * 1000,
          ).toISOString(),
          ...
          opponentKind:
            index % 3 === 0 ? "ai" : index % 3 === 1 ? "account" : "guest",
          matchMode:
            index % 2 === 0 ? "human_runner_vs_corp_ai" : "human_vs_human",
        ...
        const record = selfPlay
          ? (({ recordedAt, ...fields }) => ({
              ...fields,
              exclusionReason: "self_play" as const,
              recordedAt,
            }))(draft)
          : draft;
      ...
      await sqliteStorage.recordGameResult(
        gameRecord({
      ...
      const sqliteService = new AccountMatchStatisticsService(sqliteStorage, {
        now: () => now,
      });
      const memoryService = new AccountMatchStatisticsService(memoryStorage, {
        now: () => now,
      });
      const query = {
        period: "30d" as const,
        side: "runner" as const,
        opponentKind: "ai" as const,
      };
      const sqliteStatistics = await sqliteService.statisticsForAccount(
        owner.accountId,
        query,
      );
      const memoryStatistics = await memoryService.statisticsForAccount(
        owner.accountId,
        query,
      );
      expect({ ...sqliteStatistics, statisticsSince: undefined }).toEqual({
        ...memoryStatistics,
        statisticsSince: undefined,
      });

      const firstSqlitePage = await sqliteService.matchHistoryForAccount(
        owner.accountId,
        { ...query, limit: 7 },
      );
      const firstMemoryPage = await memoryService.matchHistoryForAccount(
        owner.accountId,
        { ...query, limit: 7 },
      );
      expect({ ...firstSqlitePage, statisticsSince: undefined }).toEqual({
        ...firstMemoryPage,
        statisticsSince: undefined,
      });
      expect(firstSqlitePage.entries).toHaveLength(7);
      expect(firstSqlitePage.nextCursor).toBeTruthy();
      if (!firstSqlitePage.nextCursor || !firstMemoryPage.nextCursor)
        throw new Error("Missing keyset cursor");

      const secondSqlitePage = await sqliteService.matchHistoryForAccount(
        owner.accountId,
        { ...query, limit: 7, cursor: firstSqlitePage.nextCursor },
      );
      const secondMemoryPage = await memoryService.matchHistoryForAccount(
        owner.accountId,
        { ...query, limit: 7, cursor: firstMemoryPage.nextCursor },
      );
      expect({ ...secondSqlitePage, statisticsSince: undefined }).toEqual({
        ...secondMemoryPage,
        statisticsSince: undefined,
      });
      expect(
        new Set(
          [...firstSqlitePage.entries, ...secondSqlitePage.entries].map(
      ...
      try {
        const plan = (
          queryDb
            .prepare(
              "EXPLAIN QUERY PLAN SELECT account_game_result_id FROM account_game_results WHERE account_id = ? ORDER BY completed_at DESC, account_game_result_id DESC LIMIT 8",
            )
            .all(owner.accountId) as Array<{ detail: string }>
        )
          .map((row) => row.detail)
          .join("\n");
  ...
  it("marks same-account two-slot games as self-play and writes a finished series once", async () => {
    const matchStorage = new InMemoryMatchStorage();
    const matches = new MultiplayerService(matchStorage, {
      tokenSalt: "account-statistics-series",
    });
    const statistics = new AccountMatchStatisticsService(
      new InMemoryAccountStatisticsStorage(),
    );
      ...
      results: [
        {
          ...
          corpAgendaPoints: 0,
          finalStateHash:
            stored.lifecycleResult?.finalEngineStateHash ?? "hash",
    ...
    expect(await statistics.gameResultsForAccount("account_self")).toHaveLength(
      2,
    );
    expect(await statistics.gameResultsForAccount("account_self")).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          participantSlot: "player_a",
          statisticsEligible: false,
          exclusionReason: "self_play",
        }),
        expect.objectContaining({
          participantSlot: "player_b",
          statisticsEligible: false,
    ...
    expect(await statistics.seriesResultsForAccount("account_self")).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          participantSlot: "player_a",
          outcome: "loss",
  ...
  it("persists bindings in a separate account database without copying them into matches", async () => {
    const dir = await mkdtemp(
      join(tmpdir(), "netgrid-account-statistics-binding-"),
    );
    const dbPath = join(dir, "accounts.sqlite");
    const backupDir = join(dir, "backups");
    const accounts = new SqliteAccountStorage({ dbPath, backupDir });
    const sessions = new AccountSessionService(accounts);
    const statisticsStorage = new SqliteAccountStatisticsStorage({
      dbPath,
      backupDir,
    });
    ...
    try {
      ...
      expect(
        await statistics.bindingsForMatch("match_in_other_database"),
      ).toEqual([
        expect.objectContaining({
          accountId: account.accountId,
          participantSlot: "player_a",
  ...
  it("keeps the durable result ledger after raw match retention removes the match", async () => {
    const dir = await mkdtemp(
      join(tmpdir(), "netgrid-account-statistics-retention-"),
    );
    ...
    const matches = new MultiplayerService(matchStorage, {
      tokenSalt: "account-statistics-retention",
    });
    matches.addPersistenceObserver((record) =>
      statistics.recordTerminalMatch(record),
    );
    try {
      ...
      expect(
        await statistics.gameResultsForAccount(account.accountId),
      ).toHaveLength(1);
      expect((await matchStorage.maintenanceSummary()).tableSizes).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
      ...
      const reopened = new AccountMatchStatisticsService(
  ...
  it("preserves and restores the account statistics ledger through SQLite backups", async () => {
    const dir = await mkdtemp(
      join(tmpdir(), "netgrid-account-statistics-backup-"),
    );
    const dbPath = join(dir, "netgrid.sqlite");
    const backupDir = join(dir, "backups");
    const accounts = new SqliteAccountStorage({ dbPath, backupDir });
    const sessions = new AccountSessionService(accounts);
    const account = await sessions.createAccount({
      loginName: "backup_stats",
      displayName: "Backup Stats",
    });
    const statisticsStorage = new SqliteAccountStatisticsStorage({
      dbPath,
      backupDir,
    ...
    const matchStorage = new SqliteMatchStorage({ dbPath, backupDir });
    try {
      await statisticsStorage.recordGameResult(
        gameRecord({
      ...
      const restored = new AccountMatchStatisticsService(
        new SqliteAccountStatisticsStorage({ dbPath, backupDir }),
      );
      try {
...
function gameRecord(
  overrides: Partial<
    import("./account-statistics").AccountGameResultRecord
  > = {},
): import("./account-statistics").AccountGameResultRecord {
  return {
