...
describe("AccountDeckService", () => {
  it("publishes only curated standards and snapshots all of them as valid immutable decks", () => {
    ...
    expect(
      standards.every(
        (deck) =>
          deck.status === "active" &&
          deck.standardDeckId.startsWith("standard_"),
      ),
    ).toBe(true);
    expect(
      standards.every(
        (deck) =>
          deck.guideStatus === "available" &&
          deck.guide?.standardDeckId === deck.standardDeckId,
      ),
    ).toBe(true);
    for (const standard of standards) {
      ...
      expect(service.standardSnapshot(standard.standardDeckId).deckHash).toBe(
        snapshot.deckHash,
      );
      expect(JSON.stringify(snapshot)).not.toMatch(
        /internal_ai|test_fixture|retire|ownerAccountId|cloudDeckId|guideStatus|deckIdea/,
      );
    }
  });
  ...
  it("enforces owner isolation, quota, optimistic versions, copy and immutable snapshots", async () => {
    ...
    const first = await service.copyStandard(
      "acct_a",
      standard.standardDeckId,
      "Eigene Kopie",
    );
    expect(first).toMatchObject({
      ...
      deck: { name: "Eigene Kopie" },
    });
    await expect(
      service.get("acct_b", first.cloudDeckId),
    ).rejects.toMatchObject({ code: "account_deck_not_found" });

    const updated = await service.update("acct_a", first.cloudDeckId, 1, {
      ...draftFromStandard(standard),
      name: "Überarbeitet",
    });
    ...
    ).rejects.toMatchObject({ code: "account_deck_version_conflict" });

    const second = await service.create("acct_a", {
      ...draftFromStandard(standard),
      name: "Zweites Deck",
    });
    await expect(
      service.create("acct_a", {
        ...draftFromStandard(standard),
        name: "51. Deck",
      }),
    ).rejects.toMatchObject({ code: "account_deck_limit_reached" });
    expect((await service.list("acct_a")).quota).toEqual({
      limit: 2,
      used: 2,
      remaining: 0,
    });
  ...
  it("stores invalid drafts but refuses to hand them to match setup", async () => {
    const service = new AccountDeckService(new InMemoryAccountDeckStorage());
    const standard = service.listStandards()[0]!;
    const draft = await service.create("acct_a", {
      ...draftFromStandard(standard),
    ...
    await expect(
      service.snapshot("acct_a", draft.cloudDeckId),
    ).rejects.toMatchObject({ code: "account_deck_invalid" });
  });
  ...
  it("enforces the quota inside the shared SQLite transaction", async () => {
    ...
    const accountStorage = new SqliteAccountStorage({
      dbPath,
      backupDir: join(directory, "backups"),
    });
    const accounts = new AccountSessionService(accountStorage, {
      tokenSalt: "sqlite-account-decks",
    });
    await accounts.createAccount({
      accountId: "acct_sqlite",
      loginName: "sqlite-user",
      displayName: "SQLite User",
    });
    const deckStorage = new SqliteAccountDeckStorage({
      dbPath,
      backupDir: join(directory, "backups"),
    });
    ...
    try {
      ...
      expect((await service.list("acct_sqlite")).quota).toEqual({
        limit: 1,
        used: 1,
        remaining: 0,
      });
...
function draftFromStandard(
  standard: ReturnType<AccountDeckService["listStandards"]>[number],
) {
  return {
    name: standard.name,
    side: standard.side,
