...
describe("V2.6 moderation RBAC and redaction test harness", () => {
  it("covers every role and data class in the RBAC matrix", () => {
    for (const dataClass of MODERATION_DATA_CLASSES) {
      expect(Object.keys(MODERATION_RBAC_MATRIX[dataClass]).sort()).toEqual(
        [...MODERATION_ROLES].sort(),
      );
    }
  });

  it("keeps hidden match data, AI debug and break-glass paths out of standard moderator access", () => {
    expect(moderationAccessFor("moderator", "D0_public_lobby_metadata")).toBe(
      "allow",
    );
    expect(
      moderationAccessFor("moderator", "D3_public_replay_projection"),
    ).toBe("allow");
    expect(moderationAccessFor("moderator", "D4_side_private_projection")).toBe(
      "break_glass",
    );
    expect(moderationAccessFor("moderator", "D5_hidden_match_data")).toBe(
      "deny",
    );
    expect(moderationAccessFor("moderator", "D6_ai_debug_data")).toBe("deny");
    expect(moderationAccessFor("admin", "D5_hidden_match_data")).toBe(
      "break_glass",
    );
    ...
    expect(
      isStandardModeratorAccessAllowed("admin", "D5_hidden_match_data"),
    ).toBe(false);
  });
  ...
  it("rejects copied hidden, token, deck and AI debug material as moderation evidence", () => {
    const violations = findModerationEvidenceRedactionViolations({
      ...
      privateDeckSnapshots: {
        runner: { cards: [{ cardId: "Hidden Priority Agenda", quantity: 3 }] },
      },
