...
describe("ARC-001 maintenance HTTP security", () => {
  it("permits only local loopback GET analysis without a session", () => {
    ...
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        request("127.0.0.1"),
        "/api/storage/maintenance/analysis/matches/match/bundle",
        config,
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        ...
        "/api/storage/maintenance/analysis/matches/match/decisions/1",
        config,
      ),
    ).toBe(true);
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        request("127.0.0.1"),
        "/api/storage/maintenance/ai-decision-traces/matches/match",
        config,
      ),
    ).toBe(true);
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        request("127.0.0.1"),
        "/api/storage/maintenance/analysis/future-endpoint",
        config,
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        request("127.0.0.1"),
        "/api/storage/maintenance/analysis/matches/match/bundle",
        { ...config, profile: "private_internet" },
      ),
    ).toBe(false);
    expect(
      mayAccessLocalReadOnlyAnalysisWithoutMaintenanceAuth(
        request("127.0.0.1", "POST"),
        "/api/storage/maintenance/analysis/matches/match/bundle",
        config,
      ),
    ).toBe(false);
  ...
  it("rate-limits decision analysis before reconstruction without affecting the bundle scope", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "netgrid-decision-analysis-rate-limit-"),
    );
    tempDirs.push(directory);
    const storage = new SqliteMatchStorage({
      dbPath: join(directory, "netgrid.sqlite"),
      backupDir: join(directory, "backups"),
    });
    const service = new MultiplayerService(storage, {
      tokenSalt: "decision-analysis-rate-limit",
    });
    const decisionAnalysis = vi.spyOn(
      service,
      "storageMaintenanceDecisionAnalysis",
    );
    ...
    try {
      const firstDecision = await fetch(
        `${baseUrl}/api/storage/maintenance/analysis/matches/missing/decisions/1`,
      );
      expect(firstDecision.status).toBe(404);
      const limitedDecision = await fetch(
        `${baseUrl}/api/storage/maintenance/analysis/matches/missing/decisions/1`,
      );
      expect(limitedDecision.status).toBe(429);
      expect(await limitedDecision.json()).toMatchObject({
      ...
      expect(
        (
          await fetch(
            `${baseUrl}/api/storage/maintenance/analysis/matches/missing/bundle`,
          )
        ).status,
      ).toBe(404);
    } finally {
  ...
  it("allows loopback analysis without a session while other maintenance routes remain authenticated", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "netgrid-local-analysis-auth-"),
    );
    tempDirs.push(directory);
    const storage = new SqliteMatchStorage({
      dbPath: join(directory, "netgrid.sqlite"),
    ...
    try {
      ...
      const traceIndex = await fetch(
        `${baseUrl}/api/storage/maintenance/ai-decision-traces/matches/missing`,
      );
      expect(traceIndex.status).toBe(200);
      expect(await traceIndex.json()).toEqual({ traces: [] });

      expect(
        (await fetch(`${baseUrl}/api/storage/maintenance/summary`)).status,
      ).toBe(401);
      expect(
        (
          await fetch(`${baseUrl}/api/storage/maintenance/cleanup/preview`, {
            method: "POST",
          })
        ).status,
      ).toBe(401);
  ...
  it("fails closed, enforces cookie, origin, CSRF and reauthentication, and revokes sessions after password change", async () => {
    const service = new MultiplayerService(new InMemoryMatchStorage(), {
      tokenSalt: "maintenance-auth-http",
    });
    const maintenanceAuth = new MaintenanceAuthService(
      new InMemoryMaintenanceCredentialStore(),
      { passwordKdf: TEST_KDF },
    ...
    const baseUrl = await listen(handle);
    try {
      const beforeBootstrap = await fetch(
        `${baseUrl}/api/storage/maintenance/summary`,
      );
      expect(beforeBootstrap.status).toBe(503);
      expect(await beforeBootstrap.text()).toContain(
        "maintenance_auth_uninitialized",
      );
      ...
      expect(setCookie).toMatch(
        /^ng_maintenance_session=[^;]+; HttpOnly; SameSite=Strict; Path=\/; Max-Age=1800$/,
      );
      ...
      const withoutCsrf = await fetch(
        `${baseUrl}/api/storage/maintenance/cleanup/preview`,
        {
          method: "POST",
          headers: {
      ...
      expect(await withoutCsrf.text()).toContain(
      ...
      const foreignOrigin = await fetch(
        `${baseUrl}/api/storage/maintenance/cleanup/preview`,
        {
          ...
          headers: {
            ...
            "x-netgrid-csrf": loginPayload.csrfToken ?? "",
          },
          body: JSON.stringify({ statuses: ["finished"] }),
        },
      );
      ...
      const withoutReauth = await fetch(
        `${baseUrl}/api/storage/maintenance/cleanup/apply`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
      ...
      expect(await withoutReauth.text()).toContain(
        "maintenance_reauthentication_required",
      );

      const changedPassword = "noch sicherere Test-Passphrase";
      const passwordChange = await fetch(
        `${baseUrl}/api/storage/maintenance/auth/password`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
      ...
      expect(await passwordChange.json()).toMatchObject({
        ok: true,
        sessionsRevoked: true,
      });
      expect(
        (
          await fetch(`${baseUrl}/api/storage/maintenance/summary`, {
            headers: { cookie },
          })
        ).status,
      ).toBe(401);
  ...
  it("revalidates cleanup status and refuses a match that became active", async () => {
    const directory = mkdtempSync(
      join(tmpdir(), "netgrid-maintenance-cleanup-"),
    );
    tempDirs.push(directory);
    const storage = new SqliteMatchStorage({
      dbPath: join(directory, "netgrid.sqlite"),
    ...
    const service = new MultiplayerService(storage, {
      tokenSalt: "maintenance-cleanup-revalidation",
    });
    const created = await service.createMatch({
      hostSide: "runner",
      playMode: "human_vs_ai",
      seed: "cleanup-revalidation",
    });
    ...
    record.match.updatedAt = new Date(
      Date.now() - 2 * 60 * 60 * 1000,
    ).toISOString();
    await storage.save(record);
    const filters = {
      statuses: ["abandoned" as const],
      olderThanMinutes: 60,
      limit: 100,
    };
    ...
    await expect(
      storage.maintenanceCleanupApply({
        filters,
        previewId: preview.previewId,
      }),
    ).rejects.toThrow("maintenance_preview_mismatch");
  ...
  it("accepts remote maintenance only through an explicit HTTPS trusted-proxy contract", async () => {
    ...
    const handle = createNetgridHttpServer(
      new MultiplayerService(new InMemoryMatchStorage(), {
        tokenSalt: "private-maintenance-http",
      }),
      {
        deploymentConfig,
        maintenanceAuth,
    ...
    const baseUrl = await listen(handle);
    try {
      const directHttp = await fetch(
        `${baseUrl}/api/storage/maintenance/auth/login`,
        {
          method: "POST",
          headers: {
            "content-type": "application/json",
            origin: "https://admin.netgrid.example",
          },
      ...
      const proxiedHttps = await fetch(
        `${baseUrl}/api/storage/maintenance/auth/login`,
        {
          method: "POST",
          headers: {
      ...
      expect(proxiedHttps.headers.get("access-control-allow-origin")).toBe(
        "https://admin.netgrid.example",
      );
      expect(proxiedHttps.headers.get("access-control-allow-credentials")).toBe(
        "true",
      );

      const adminOriginOnGamePlane = await fetch(`${baseUrl}/health`, {
        headers: { origin: "https://admin.netgrid.example" },
      });
      expect(adminOriginOnGamePlane.status).toBe(403);
      expect(await adminOriginOnGamePlane.text()).toContain(
        "origin_not_allowed",
      );
    } finally {
      await handle.close();
