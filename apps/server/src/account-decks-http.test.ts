...
describe("account deck HTTP API", () => {
  it("exposes only standards and enforces owner, CSRF, version and quota boundaries", async () => {
    ...
    try {
      ...
      const standardsPayload = JSON.parse(standardsText) as {
        catalog: {
          decks: Array<{
            standardDeckId: string;
            side: string;
            guideStatus: string;
            guide?: { standardDeckId: string };
          }>;
          snapshots: unknown[];
        };
      };
      ...
      expect(
        standardsPayload.catalog.decks.every(
          (deck) =>
            deck.guideStatus === "available" &&
            deck.guide?.standardDeckId === deck.standardDeckId,
        ),
      ).toBe(true);
      expect(JSON.stringify(standardsPayload.catalog.snapshots)).not.toMatch(
        /guideStatus|deckIdea/,
      );
      expect(standardsText).not.toMatch(
        /internal_ai|test_fixture|retire|ownerAccountId/,
      );
      ...
      const copied = await fetch(`${baseUrl}/api/account/decks/copy-standard`, {
        ...
        body: JSON.stringify({
          standardDeckId: standard.standardDeckId,
          name: "Alpha Standardkopie",
        }),
      });
      expect(copied.status).toBe(201);
      const copiedPayload = (await copied.json()) as {
        deck: {
          cloudDeckId: string;
          deckVersion: number;
      ...
      expect(
        (
          await fetch(
            `${baseUrl}/api/account/decks/${copiedPayload.deck.cloudDeckId}`,
      ...
      ).json()) as { deck: { deck: Record<string, unknown> } };
      const updated = await fetch(
        `${baseUrl}/api/account/decks/${copiedPayload.deck.cloudDeckId}`,
        {
          ...
          body: JSON.stringify({
            expectedVersion: 1,
            deck: { ...current.deck.deck, name: "Alpha überarbeitet" },
          }),
        },
      );
      ...
      const withoutCsrf = await fetch(
        `${baseUrl}/api/account/decks/${copiedPayload.deck.cloudDeckId}`,
        { method: "DELETE", headers: { cookie: alpha.cookie, origin: ORIGIN } },
      );
      ...
      const preflight = await fetch(`${baseUrl}/api/account/decks`, {
        method: "OPTIONS",
      ...
      expect(exportText).not.toMatch(
        /passwordHash|sessionTokenHash|csrfTokenHash|inviteTokenHash/,
      );
      ...
      expect(
        (
          await fetch(`${baseUrl}/api/account/decks`, {
            headers: { cookie: beta.cookie },
          })
        ).status,
      ).toBe(200);
  ...
  it("returns newly validated standard snapshots that existing match setup accepts", async () => {
    ...
    const handle = createNetgridHttpServer(
      new MultiplayerService(new InMemoryMatchStorage(), {
        tokenSalt: "standard-snapshot-match",
      }),
      {
        deploymentConfig: loadDeploymentConfig({} as NodeJS.ProcessEnv),
        accountAuth,
    ...
    try {
      ...
      const runnerSnapshot = (
        (await (
          await fetch(
            `${baseUrl}/api/decks/standards/${runner.standardDeckId}/snapshot`,
            { method: "POST" },
          )
      ...
      const corpSnapshot = (
        (await (
          await fetch(
            `${baseUrl}/api/decks/standards/${corp.standardDeckId}/snapshot`,
            { method: "POST" },
          )
        ).json()) as { snapshot: unknown }
      ).snapshot;
