...
export function captureServerFrames(page: Page): FrameCapture {
  ...
  page.on("websocket", (socket) => {
    socket.on("framereceived", (event) =>
      capture.received.push(String(event.payload)),
    );
  });
  return capture;
}

export async function expectNoDomOrLocalStorageLeaks(
  page: Page,
  hiddenTitles: string[] = [],
): Promise<void> {
  ...
  for (const pattern of DOM_FORBIDDEN) {
    expect(surface, `DOM/localStorage leak pattern ${pattern}`).not.toMatch(
      pattern,
    );
  }
...
export function expectNoServerPayloadLeaks(capture: FrameCapture): void {
  ...
  for (const pattern of PAYLOAD_FORBIDDEN) {
    expect(
      received,
      `received WebSocket payload leak pattern ${pattern}`,
    ).not.toMatch(pattern);
  }
}

export async function expectRecentSessionsAreSanitized(
  page: Page,
): Promise<void> {
  const recent = await page.evaluate(
    () => window.localStorage.getItem("netgrid.recentSessions") ?? "",
  );
  for (const pattern of DOM_FORBIDDEN) {
    expect(recent, `recent session leak pattern ${pattern}`).not.toMatch(
      pattern,
    );
  }
}
