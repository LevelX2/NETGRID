...
test.describe("V1.0.7 Browser-E2E und Visual QA", () => {
  test("Human-vs-KI Desktop prüft aktives Board, KI-Takt, Cue, Aktionen, Credits und Artefakte", async ({
    page,
  }, testInfo) => {
    ...
    await expect(page.getByTestId("server-run-action").first()).toHaveAttribute(
      "aria-label",
      /Run auf .+ starten/,
    );
    ...
    await page
      .getByRole("button", { name: "Zurück zum aktiven Spiel" })
      .click();
    await page
      .locator('[data-testid="action-button"][data-action-type="gain_credit"]')
      .first()
      .click();
    await expect(page.getByTestId("action-availability")).toContainText(
      "noch 3",
    );
    await page
      .locator('[data-testid="action-button"][data-action-type="end_turn"]')
      .first()
      .click();
    await expect(
      page.getByRole("button", { name: "KI-Schritt" }),
    ).toBeEnabled();
  ...
  test("Human-vs-Human Desktop nutzt zwei getrennte Kontexte für Host, Join, Ready, Countdown und aktives Spiel", async ({
    browser,
  }, testInfo) => {
    ...
    try {
      ...
      await saveFlowScreenshot(
        host.page,
        testInfo,
        "desktop-human-vs-human-host-active",
      );
      await saveFlowScreenshot(
        joiner.page,
        testInfo,
        "desktop-human-vs-human-joiner-active",
      );
    } finally {
  ...
  test("Private Account-Matchstatistik zeigt Account-gegen-KI und zwei getrennte Accounts", async ({
    browser,
  }) => {
    ...
    try {
      ...
      const alphaSecondTab = await alpha.context.newPage();
      await alphaSecondTab.goto(BASE_URL);
      await alphaSecondTab
        .getByRole("button", { name: "Profil", exact: true })
        .click();
      await expect(
        alphaSecondTab.getByRole("heading", {
          name: "Statistik Alpha",
          exact: true,
        }),
      ).toBeVisible();
      ...
      await alpha.page
        ...
        .click();
      await expect(
        alpha.page.getByRole("heading", {
          name: "Statistik Alpha",
          exact: true,
        }),
      ).toBeVisible();
      const betaInvite = await createBrowserAccountInvite(
        alpha.page,
        "stats_beta",
        "Statistik Beta",
      );
      await activateBrowserAccount(
        beta.page,
        betaInvite,
        passwordB,
        "Statistik Beta",
      ...
      await expect(
        alpha.page.getByText("Durch Aufgabe entschieden", { exact: true }),
      ).toBeVisible();

      const joinUrl = await createHumanVsHumanLobby(
        alpha.page,
        "account-statistics-hvh-e2e",
        "runner",
      );
      ...
      await expect(breakdownCard(alpha.page, "Gegen Account")).toContainText(
        "1",
      );
      await expect(breakdownCard(alpha.page, "Gegen KI")).toContainText("1");
      ...
      await expect(breakdownCard(beta.page, "Gegen Account")).toContainText(
        "1",
      );
      await expect(
        beta.page
          .getByRole("article")
          .getByText("Mensch gegen Mensch", { exact: true }),
      ).toBeVisible();
      ...
      await alpha.page
        ...
        .click();
      await expect(
        alpha.page.getByRole("button", { name: "Account", exact: true }),
      ).toBeVisible();
      ...
      await guest.page
        ...
        .click();
      await expect(
        guest.page.getByRole("region", { name: "Deine Matchstatistik" }),
      ).toHaveCount(0);
    } finally {
  ...
  test("Lifecycle/Reconnect prüft Cancel, Recreate, Leave, Wieder verbinden, Forfeit und Verwerfen", async ({
    browser,
    page,
  }, testInfo) => {
    await page.setViewportSize(VIEWPORTS.desktop);
    const joiner = await newContextPage(browser, "desktop");
    try {
      const firstJoinUrl = await createHumanVsHumanLobby(
        page,
        "v1-0-7-lifecycle-cancel",
        "runner",
      );
      await page.getByTestId("cancel-match").click();
      await expect(
        page.getByRole("heading", { name: "Match abgebrochen" }),
      ).toBeVisible();
      ...
      await expect(
        joiner.page.getByText("Du hast die noch nicht aktive Lobby verlassen."),
      ).toBeVisible();
      await expect(joiner.page.getByTestId("setup-screen")).toBeVisible();
      await expect(page.getByTestId("start-lobby")).toBeVisible();
      await expect(
        page.getByText("Gegenüber: Wartet auf Gegenüber"),
      ).toBeVisible();
      await saveFlowScreenshot(
        page,
        testInfo,
        "desktop-lifecycle-host-waiting-after-leave",
      );
      await page.getByTestId("cancel-match").click();
      await expect(
        page.getByRole("heading", { name: "Match abgebrochen" }),
    ...
    await expect(
      page.getByRole("alertdialog", { name: "Spiel aufgeben?" }),
    ).toBeVisible();
    await page
      .getByRole("alertdialog", { name: "Spiel aufgeben?" })
      .getByRole("button", { name: "Aufgeben" })
      .click();
    await expect(
      page.getByText(/Spiel aufgegeben|gewinnt/i).first(),
    ).toBeVisible();
  ...
  test("Tablet-Viewport prüft Board, Run-Ziel, direkte Server-Run-Actions und Card Display", async ({
    page,
  }, testInfo) => {
  ...
  test("Schmaler Viewport prüft Textfit, Actions, Cue-Bereich, RunTimeline und Vorschau", async ({
    page,
  }, testInfo) => {
  ...
  test("Hidden-Info-Scan prüft verdeckte Corp-Installation ohne Titel-, DOM-, Bildpfad- oder Payload-Leak", async ({
    browser,
  }, testInfo) => {
    ...
    try {
      ...
      await expect(
        runner.page.getByTestId("hidden-card").first(),
      ).toBeVisible();
      await expect(
        runner.page.getByTestId("hidden-card").first(),
      ).toContainText("Verdeckte Karte");
      await expectNoDomOrLocalStorageLeaks(runner.page, [hiddenTitle]);
      expectNoServerPayloadLeaks(runnerFrames);
      await saveFlowScreenshot(
        corp.page,
        testInfo,
        "desktop-hidden-install-corp",
      );
      await saveFlowScreenshot(
        runner.page,
        testInfo,
        "desktop-hidden-install-runner-redacted",
      );
    } finally {
  ...
  test("Runtime-Isolation nutzt die temporäre SQLite-Datenbank statt normaler lokaler Runtime-Dateien", async ({
    request,
  }) => {
    ...
    const health = await request.get(
      `${process.env.NETGRID_E2E_SERVER_URL}/health`,
    );
    const body = (await health.json()) as {
      profile?: string;
      realtime?: { ready?: boolean };
      storage?: { kind?: string; database?: string; matchCount?: number };
    };
  ...
  test("V1.3.0 Deckvalidierung prüft legalen und illegalen privaten Formatpfad ohne Decklist-Leak", async ({
    request,
  }) => {
    const legalDeck = {
    ...
    const valid = await request.post("/api/decks/validate", {
      data: { deck: legalDeck },
    });
    ...
    const invalid = await request.post("/api/decks/validate", {
      data: {
        deck: {
          ...legalDeck,
...
async function loginBrowserAccount(
...
async function createBrowserAccountInvite(
  page: import("@playwright/test").Page,
  loginName: string,
  displayName: string,
): Promise<string> {
  ...
  await form
    .getByRole("button", { name: "Einladungslink erzeugen", exact: true })
    .click();
...
async function activateBrowserAccount(
  page: import("@playwright/test").Page,
  inviteUrl: string,
  password: string,
  displayName: string,
  ...
  await page
    .getByRole("button", { name: "Account aktivieren", exact: true })
    .click();
  await expect(
    page.getByRole("heading", { name: displayName, exact: true }),
  ).toBeVisible();
...
async function openAccountStatistics(
  ...
  await expect(
    page.getByRole("region", { name: "Deine Matchstatistik" }),
  ).toBeVisible();
  await expect(
    page.getByText("Matchstatistik wird geladen …", { exact: true }),
  ).toBeHidden();
}

function statisticCard(page: import("@playwright/test").Page, label: string) {
  return page
    .locator(".accountStatisticsKpi")
    .filter({ has: page.getByText(label, { exact: true }) });
}

function breakdownCard(page: import("@playwright/test").Page, title: string) {
  return page
    .locator(".accountStatisticsBreakdown")
    .filter({ has: page.getByRole("heading", { name: title, exact: true }) });
}

async function forfeitAndReturnToStart(
  page: import("@playwright/test").Page,
): Promise<void> {
  ...
  await page
    .getByRole("button", { name: "Board ansehen", exact: true })
    .click();
  await page
    .getByRole("button", { name: "Startbildschirm", exact: true })
    .click();
  await expect(page.getByTestId("setup-screen")).toBeVisible();
}

async function returnToStartAfterOpponentForfeit(
  page: import("@playwright/test").Page,
): Promise<void> {
  await expect(
    page.getByText(/Spiel aufgegeben|gewinnt/i).first(),
